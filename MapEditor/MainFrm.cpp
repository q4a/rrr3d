...
{
	return _activeMapView ? _activeMapView->get_document() : 0;
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	EnablePaneMenu(FALSE, ID_VIEW_CUSTOMIZE, strCustomize, ID_VIEW_TOOLBAR); 
...
void CMainFrame::OnEditMapToolBarCommand(UINT nID)
{
	...
	case IDTB_EDITMAP_CAMLINK:
		GetMapDoc()->SetSelMode(r3d::ISceneControl::smCamLink);		
		m_wndClassView.SetActive(false);
		break;
	...
	case IDTB_EDITMAP_ROTX:
		GetMapDoc()->SetSelMode(r3d::ISceneControl::smRotX);
		m_wndClassView.SetActive(false);
		break;

	case IDTB_EDITMAP_ROTY:
		GetMapDoc()->SetSelMode(r3d::ISceneControl::smRotY);
		m_wndClassView.SetActive(false);
		break;

	case IDTB_EDITMAP_ROTZ:
		GetMapDoc()->SetSelMode(r3d::ISceneControl::smRotZ);
		m_wndClassView.SetActive(false);
		break;
	...
	case ID_DESTROYER:		
		{		
			GetMapDoc()->SetSelMode(r3d::ISceneControl::smDestroyer);	
			m_wndClassView.SetActive(false);
			r3d::IMapObjRef ref = GetMapDoc()->GetSelMapObj();
			if (ref)
			{
				GetMapDoc()->SelectMapObj(0);
				GetMapDoc()->DelMapObj(ref);				
			}	
			GetMapDoc()->SetSelMode(r3d::ISceneControl::smMove);	
		}
		break;
...
void CMainFrame::OnEditMapToolBarUpdateCommandUI(CCmdUI* pCmdUI)
{
	...
	case IDTB_EDITMAP_CAMLINK:
		check = GetMapDoc()->GetSelMode() == r3d::ISceneControl::smCamLink;
		break;
	...
	case IDTB_EDITMAP_ROTX:
		check = GetMapDoc()->GetSelMode() == r3d::ISceneControl::smRotX;
		break;

	case IDTB_EDITMAP_ROTY:
		check = GetMapDoc()->GetSelMode() == r3d::ISceneControl::smRotY;
		break;

	case IDTB_EDITMAP_ROTZ:
		check = GetMapDoc()->GetSelMode() == r3d::ISceneControl::smRotZ;
		break;
	...
	case ID_DESTROYER:
		check = GetMapDoc()->GetSelMode() == r3d::ISceneControl::smDestroyer;  
		break;
