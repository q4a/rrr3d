...
{
	if (INIT_FIRST_TITLE = false)
	{
		_lastTitle = CStringToStdStr(this->GetTitle());
		INIT_FIRST_TITLE = true;
	}
...
const std::string& CMapEditorDoc::GetLastTitle() const
{
	return _lastTitle;
}

void CMapEditorDoc::SetLastTitle(const std::string& value)
{
	_lastTitle = value;
}
...
void CMapEditorDoc::SetMapObjCreated(bool value)
{
	_mObjC = value;
}

bool CMapEditorDoc::IsMapObjCreated() const 
{
	return _mObjC;
}

void CMapEditorDoc::FixedClear() 
{
	_selMapObj = 0;
	_selWayPoint = 0;
}
