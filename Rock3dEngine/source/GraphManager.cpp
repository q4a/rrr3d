...
void GraphManager::InitWaterPlane()
{
	if (!_waterPlane && _graphQuality[goWater] != gqLow)
	{
		...
		_waterPlane->SetColor(D3DXCOLOR(0.0f, 0.1f, 0.3f, 1.0f));
...
void GraphManager::RefreshRefl()
{
	if (_reflRef > 0)
	{
		...
		_reflShader->SetReflectivity(_reflMappMode == rmColorLayer ? 0.3f : 0.3f);
	}
}
