...
void ViewPort3d::SlowAnimProgress(float deltaTime)
{
	D3DXQUATERNION rot;
	D3DXQuaternionSlerp(&rot, &GetBox()->GetRot(), &(_rot3dSpeed * GetBox()->GetRot()), (deltaTime / 8));

	GetBox()->SetRot(rot);
}

void ViewPort3d::ReverseProgress(float deltaTime)
{
	D3DXQUATERNION rot;
	D3DXQuaternionSlerp(&rot, &GetBox()->GetRot(), &(_rot3dSpeed * GetBox()->GetRot()), deltaTime);
...
void ViewPort3d::SetStyle(Style value)
{
	if (_style != value)
	{
		...
		SetProgress(_style == msAnim || _style == msMouseOverAnim || _style == msSlowAnim );
	}
}
