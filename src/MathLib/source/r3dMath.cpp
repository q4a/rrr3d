...
{
	D3DXPLANE res;
	//The negated axes are temporaries, and taking their address is an MSVC
	//extension. Naming them changes nothing but makes it legal.
...
void Frustum::CalculateCorners(Corners& pPoints, const D3DXMATRIX& invViewProj)
{
	...
	{
		const D3DXVECTOR3 ndc(fx, fy, fz);
		D3DXVec3TransformCoord(&pPoints[i], &ndc, &invViewProj);
	}
}
