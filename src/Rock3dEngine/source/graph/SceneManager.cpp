...
{
	D3DXVECTOR3 res;
	const D3DXVECTOR3 firstRow(GetWorldMat().m[0]);
	D3DXVec3Normalize(&res, &firstRow);
	return res;
}
...
{
	const D3DXVECTOR3 minDelta = bb1.min - bb2.min;
	const D3DXVECTOR3 maxDelta = bb1.max - bb2.max;
	return D3DXVec3Length(&minDelta) < floatErrComp && D3DXVec3Length(&maxDelta) < floatErrComp;
}
