...
bool ComputeZBounds(graph::Engine& engine, const graph::CameraCI& camera, const AABB& aabb, float& minZ, float& maxZ)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		{
			const D3DXVECTOR3 nearPoint = rayPos[i] + rayVec[i] * tNear;
			const D3DXVECTOR3 farPoint = rayPos[i] + rayVec[i] * tFar;
		...
		for (LightList::iterator iter = _lightList.begin(); iter != _lightList.end(); ++iter)
		{
			...
			if (light->GetEnable() && shadowMap)
			{
				...
				if (camDesc.style == graph::csOrtho)
				{
					...
					const D3DXVECTOR3 toShadowFar = pos - camDesc.pos;
					maxFar = std::min(D3DXVec3Length(&toShadowFar), camDesc.farDist);
				}
	...
	if (_engine->BeginScene())
	{
		...
		if (_scRenderTexRef)
		{
			...
			if (_sunShaft && _engine->GetContext().GetCamera().GetDesc().style == graph::csPerspective)
			{
				...
				D3DXVECTOR3 radVec;
				const D3DXVECTOR3 fromWorldCenter = lightPos - _actorManager->GetWorldAABB().GetCenter();
				D3DXVec3Normalize(&radVec, &fromWorldCenter);
				const D3DXVECTOR3 zAxis(0, 0, 1);
				D3DXVec3Cross(&radVec, &zAxis, &radVec);
...
void GraphManager::BuildOctree()
{
	...
	for (graph::ActorManager::UserList::const_iterator iter = _actorManager->GetUserList().begin(); iter != _actorManager->GetUserList().end(); ++iter)	
		if ((*iter)->GetActor() != _waterPlaneActor && (*iter)->GetActor() != _grassPlane && (*iter)->GetActor() != _fogPlaneActor)
		{
			...
			{	
				/* vec1() is a D3DXVECTOR4. MSVC reached D3DXVECTOR3 through two
				   user-defined conversions -- operator const FLOAT* then
				   D3DXVECTOR3(const FLOAT*) -- which standard C++ does not allow
				   in one sequence. Written out, it is the same three floats. */
				const D3DXVECTOR4& norm4 = (*iter)->GetActor()->vec1();
				D3DXVECTOR3 norm(norm4.x, norm4.y, norm4.z);
				...
				if (dot < 0.99f)
				{
					...
					const D3DXVECTOR3 fromLight = -lightDir;
					float d2 = D3DXVec3Dot(&binormal, &fromLight);
