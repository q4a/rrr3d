...
void AIDebug::GrActor::DoRender(graph::Engine& engine)
{
	...
	_font->OnResetDevice();
	_font->DrawTextA(0 , sstream.str().c_str(), -1, &destRect, DT_NOCLIP, D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f));
	_font->DrawTextA(0 , sstream2.str().c_str(), -1, &destRect2, DT_NOCLIP, D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f));
	_font->OnLostDevice();
