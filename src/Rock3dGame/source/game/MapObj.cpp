...
{
	//Переходим напрямую к методам загрузки поскольку mapObj является промежуточным звеном и только делегирует вызовы
...
void MapObjects::OnProgress(float deltaTime)
{
	...
	{
		//Вначале прогресс для всех объектов, в том числе и умерших, поскольку они могут выполнять какие нибудь действия. Например помещать умерших потомков в список смертей
		LockCont();
		try
		{
