...
namespace r3d
{
	namespace snd
	{
		...
		Sound::Sound(SoundLib* lib): _lib(lib), _cacheSize(cBufferSize * 10), _isLoad(false), _volume(1.0f),
		                             _unusedBufSize(0)
		{
		}
		...
		Sound::Buffer::Buffer(Sound* sound): _sound(sound), _data(nullptr), _dataSize(0), _pos(0), _endPos(0)
		{
		}
		...
		bool Sound::Buffer::Load(seek_pos pos)
		{
			if (!_data)
			{
				_data = static_cast<char*>(malloc(cBufferSize));
				_dataSize = 0;
		...
		void Sound::DoInit()
		{
			...
			if (_wfopen_s(&_file, GetAppFilePath(_fileName).c_str(), L"rb") != 0)
				throw Error("void Sound::DoInit() " + _fileName);

			if (ov_open_callbacks(_file, &_oggFile, nullptr, 0, OV_CALLBACKS_DEFAULT) < 0)
				throw Error("void Sound::DoInit(). ov_open_callbacks");

			if (ov_seekable(&_oggFile) == 0)
				throw Error("!ov_seekable");
		...
		Sound::Buffer* Sound::FindBufferByPos(seek_pos pos, Buffers::iterator* nearRes)
		{
			...
			auto iter = _buffers.upper_bound(pos);
		...
		Voice::Voice(Engine* engine): _engine(engine), _outMatrix(nullptr), _srcChannels(0), _destChannels(0)
		{
		}
		...
		void Voice::SendReceivers(IXAudio2Voice* voice)
		{
			...
			if (!_receivers.empty())
			{
				...
				int i = 0;
				for (auto iter = _receivers.begin(); iter != _receivers.end(); ++iter, ++i)
				{
		...
		void Voice::ApplyOutputMatrix(IXAudio2Voice* voice)
		{
			...
			if (_receivers.empty())
			{
				...
				if (masterVoice->GetXVoice() && FAILED(
					hr = voice->SetOutputMatrix(masterVoice->GetXVoice(), _srcChannels, _destChannels, _outMatrix)))
			...
			else
			{
				...
				{
					if ((*iter)->GetXVoice() && FAILED(
						hr = voice->SetOutputMatrix((*iter)->GetXVoice(), _srcChannels, _destChannels, _outMatrix)))
						LSL_LOG("_xVoice->SetOutputMatrix");
				}
			}
		}
		...
		{
			auto res = _receivers.erase(iter);
			(*iter)->Release();
		...
		{
			auto matrix = new float[srcChannels * destChannels];
		...
		SubmixVoice::SubmixVoice(Engine* engine): _MyBase(engine), _xVoice(nullptr)
		{
			HRESULT hr;

			if (engine->GetXAudio() && FAILED(
				hr = engine->GetXAudio()->CreateSubmixVoice(&_xVoice, engine->GetDevCaps().OutputFormat.Format.nChannels
					, engine->GetDevCaps().OutputFormat.Format.nSamplesPerSec, 0, 0, 0)))
			{
		...
		MasteringVoice::MasteringVoice(Engine* engine): _MyBase(engine), _xVoice(nullptr)
		{
			if (engine->GetXAudio() && FAILED(
				engine->GetXAudio()->CreateMasteringVoice(&_xVoice, XAUDIO2_DEFAULT_CHANNELS, XAUDIO2_DEFAULT_SAMPLERATE
					, 0, 0, 0)))
			{
		...
		Proxy::Proxy(Engine* engine): _MyBase(engine), _init(false), _sound(nullptr), _pos(0), _playMode(pmOnce),
		                              _volume(1.0f), _frequencyRatio(1.0f), _onStreamEnd(false), _onTerminate(false),
		                              _xVoice(nullptr), _streaming(nullptr), _run(false)
		{
		}
		...
		Proxy::~Proxy()
		{
			...
			for (auto iter = _reportList.begin(); iter != _reportList.end(); ++iter)
				(*iter)->Release();
			_reportList.Clear();
		...
		Proxy::Streaming::Streaming(Proxy* proxy): _proxy(proxy), _play(false), _pos(0), _posChanged(false),
		                                           _bufSize(Sound::cBufferSize * 5), _cacheSize(Sound::cBufferSize * 5),
		                                           _dataSizeBuffers(0), _firstBufPos(0), _cached(false)
		{
		...
		void Proxy::Streaming::PushBackBuffer(Buffer* buffer, bool endOfStream, seek_pos offs)
		{
			...
			{
				throw Error("void Proxy::Buffering()");
			}
		...
		bool Proxy::Streaming::RemoveUnusedBufs()
		{
			...
			try
			{
				...
				else
				{
					...
					try
					{
						for (auto iter = _unusedBuffers.begin(); iter != _unusedBuffers.end();)
						{
		...
		void Proxy::Streaming::Execute(Object* arg)
		{
			do
			{
		...
		void Proxy::SendReports()
		{
			try
			{
				for (ReportList::Position pos = _reportList.First(); Report** iter = _reportList.Current(pos);
				     _reportList.Next(pos))
				{
					Report* report = *iter;
					if (_onStreamEnd)
		...
		void Proxy::Init()
		{
			...
			if (!_init)
			{
				...
				if (GetEngine()->GetXAudio() && FAILED(
					hr = GetEngine()->GetXAudio()->CreateSourceVoice(&_xVoice, &wfm, 0, XAUDIO2_DEFAULT_FREQ_RATIO,
						_voiceCallback, 0, 0)))
				{
		...
		void Proxy::Run()
		{
			if (!_run && IsInit())
			{
				...
				if (_streaming)
				{
					...
					GetSDK()->GetThreadPool()->QueueWork(_streaming, nullptr, flags);
				}
			}
		}
		...
		{
			auto iter = _reportList.Find(report);
		...
		{
			if (IsInit())
				return _sound == value || (_sound->GetVorbisInfo().channels == value->GetVorbisInfo().channels && _sound
					->GetVorbisInfo().rate == value->GetVorbisInfo().rate);
			return false;
		}
		...
		Source::Source(Engine* engine): _MyBase(engine), _sound(nullptr), _pos(0), _playMode(pmOnce), _volume(1.0f),
		                                _frequencyRatio(1.0f), _proxy(nullptr)
		{
		}
		...
		Source3d::Source3d(Engine* engine): _MyBase(engine), _pos3d(NullVector), _distScaler(0), _changed3d(false),
		                                    _play(false), _playTime(0)
		{
		...
		void Source3d::ApplyX3dEffect()
		{
			...
			if (listener)
			{
				...
				case Engine::m3dSurround:
					{
						if (GetEngine()->GetX3dAudio() == nullptr)
							break;
						...
						X3DAudioCalculate(*GetEngine()->GetX3dAudio(), &xList, &_xEmitter, X3DAUDIO_CALCULATE_MATRIX,
						                  &_dspSettings);
				...
				case Engine::m3dFlat:
					{
						SetDefOutputMatrix(nSrcChannels, nDestChannels,
						                   ClampValue(1.0f - listDistLen / distScaller, 0.0f, 1.0f));
		...
		void Source3d::CleanUpX3d()
		{
			if (_dspSettings.pMatrixCoefficients)
			{
				...
				_dspSettings.pMatrixCoefficients = nullptr;
			}
			if (_xEmitter.pChannelAzimuths)
			{
				delete[] _xEmitter.pChannelAzimuths;
				_xEmitter.pChannelAzimuths = nullptr;
			}
		}
		...
		Engine::Engine(): _mainVoice(nullptr), _listener(nullptr), _distScaler(30.0f), _mode3d(m3dFlat),
		                  _changed3d(false), _isComputing(false), _xAudio(nullptr), _poolMaxSize(20),
		                  _initX3dAudio(false)
		{
		...
		void Engine::DeleteProxys()
		{
			...
			for (auto iter = _proxyList.begin(); iter != _proxyList.end(); ++iter)
				DoDeleteProxy(*iter);
		...
		{
			for (auto iter = _srcPool.begin(); iter != _srcPool.end(); ++iter)
				(*iter)->Release();
			_srcPool.clear();
		}
		...
		{
			for (auto iter = _srcCache.begin(); iter != _srcCache.end(); ++iter)
				(*iter)->Release();
		...
		Proxy* Engine::FlushCache(Sound* sound)
		{
			...
			for (auto iter = _srcCache.begin(); iter != _srcCache.end();)
			{
				Proxy* cacheSrc = *iter;
		...
		void Engine::Compute(float deltaTime)
		{
			...
			try
			{
				...
				for (auto iter = _proxy3dList.begin(); iter != _proxy3dList.end(); ++iter)
				{
					if (_changed3d)
					{
						_changed3d = false;
		...
		{
			auto voice = new SubmixVoice(this);
		...
		{
			auto voice = new MasteringVoice(this);
			_voiceList.push_back(voice);
