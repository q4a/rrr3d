...
namespace r3d
{
	namespace video
	{
		Player::Player(IVideoGraphUser* user): _dShowPlayer(nullptr), _size(0, 0), _user(user), _fullScreen(false)
		{
		}
		...
		void Player::Finalize()
		{
			...
			SafeDelete(_dShowPlayer);
		}

		void Player::Open(const string& fileName)
		{
			LSL_ASSERT(_dShowPlayer);

			LSL_LOG("Player::Open opening=" + fileName);

			HRESULT hr = _dShowPlayer->OpenFile(GetAppFilePath(fileName).c_str());
		...
		void Player::UpdateVideoWindow(const Point& size)
		{
			LSL_ASSERT(_dShowPlayer);
