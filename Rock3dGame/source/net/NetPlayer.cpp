...
namespace r3d
{
	namespace game
	{
		...
		NetPlayer::NetPlayer(const Desc& desc): NetModelRPC(desc), _net(NetGame::I()), _player(nullptr),
		                                        _human(nullptr), _aiPlayer(nullptr), _modelOwner(false), _dAlpha(0),
		                                        _raceReady(false), _raceGoWait(false),
		                                        _raceFinish(false) //, _response(NULL)
		{
		...
		{
			if (_player->GetCar().mapObj == nullptr)
			{
		...
		void NetPlayer::SlotsWrite(std::ostream& stream)
		{
			...
			{
				Slot* slot = _player->GetSlotInst(static_cast<Player::SlotType>(i));
				WeaponItem* wpn = slot ? slot->GetItem().IsWeaponItem() : nullptr;
				ArmorItem* armor = slot && slot->GetType() == Slot::stArmor ? &slot->GetItem<ArmorItem>() : nullptr;
		...
		void NetPlayer::SlotsRead(std::istream& stream)
		{
			for (int i = 0; i < Player::cSlotTypeEnd; ++i)
			{
				...
				if (GetCar())
				{
					Slot* slot = recName != "" ? &race()->GetWorkshop().GetSlot(recName) : nullptr;
					race()->GetGarage().InstalSlot(_player, static_cast<Player::SlotType>(i), GetCar(), slot,
					                               chargeCnt);

					slot = _player->GetSlotInst(static_cast<Player::SlotType>(i));
					ArmorItem* armor = slot && slot->GetType() == Slot::stArmor ? &slot->GetItem<ArmorItem>() : nullptr;
				}
			}
		}
		...
		void NetPlayer::OnCarSlotsChanged(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                  std::istream& stream)
		{
			int money;
		...
		void NetPlayer::OnTakeBonus(const net::NetMessage& msg, const net::NetCmdHeader& header, std::istream& stream)
		{
			...
			_player->TakeBonus(&bonus->GetGameObj(), static_cast<Player::BonusType>(type), value, destroy);
		}

		void NetPlayer::OnMineContact1(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                               std::istream& stream)
		{
			unsigned plrId;
			unsigned projId;
			...
			Proj* bonus = bonusPlr ? bonusPlr->model()->GetBonusProj(projId) : nullptr;
		...
		void NetPlayer::OnMineContact2(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                               std::istream& stream)
		{
			unsigned projId;
			...
			if (bonus == nullptr || bonus->GetGameObj().IsProj() == nullptr)
			{
		...
		void NetPlayer::ResponseStream(const net::NetMessage& msg, net::BitStream& stream)
		{
			...
			if (stream.isReading() && !(owner() && _net->isClient()) && !_player->GetFinished())
			{
				...
				car->SetMoveCar(static_cast<GameCar::MoveCarState>(moveState));
				car->SetSteerWheel(static_cast<GameCar::SteerWheelState>(steerState));
				car->SetSteerWheelAngle(steerWheelsAngle);
			}
		}
		...
		void NetPlayer::Shot(MapObj* target, Player::SlotType type)
		{
			...
			if (type != Player::cSlotTypeEnd)
			{
				...
				for (unsigned i = 0; i < slots.count(); ++i)
				{
					...
					WeaponItem* wpn = _player->GetSlotInst(type)
						                  ? _player->GetSlotInst(type)->GetItem().IsWeaponItem()
						                  : nullptr;
		...
		void NetPlayer::TakeBonus(MapObj* bonus, GameObject::BonusType type, float value, bool destroy)
		{
			if (owner())
			{
				LSL_ASSERT(bonus);
				...
				net::Write(stream, type);
				net::Write(stream, value);
				net::Write(stream, destroy);
				CloseRPC();
			}
		}
		...
		int NetPlayer::GenerateGamerId() const
		{
			...
			for (auto iter = tournament.GetGamers().begin(); iter != tournament.GetGamers().end(); ++iter)
		...
		D3DXCOLOR NetPlayer::GenerateColor() const
		{
			...
			for (int i = 0; i < Player::cColorsCount; ++i)
				if (CheckColor(Player::cLeftColors2[i]))
					return Player::cLeftColors2[i];
			...
			for (int i = 0; i < Player::cColorsCount; ++i)
				if (CheckColor(Player::cRightColors2[i]))
					return Player::cRightColors2[i];

			if (CheckColor(clrBlack))
				return clrBlack;
		...
		{
			if (this->GetGamerId() >= SPECTATOR_ID_BEGIN)
				return true;
			return _raceReady;
		}
