...
namespace r3d
{
	namespace game
	{
		...
		NetGame::NetGame(GameMode* game): _game(game), _race(nullptr), _player(nullptr), _port(58213), _started(false),
		                                  _isHost(false), _isClient(false)
		{
			_i = this;
		...
		void NetGame::OnReceiveCmd(const net::NetMessage& msg, const net::NetCmdHeader& header, const void* data,
		                           unsigned size)
		{
			for (Users::Position pos = _users.First(); INetGameUser** iter = _users.Current(pos); _users.Next(pos))
	...
	netService().Ping(_port, 500, 250);
		...
		{
			return mapObj ? GetPlayer(game()->GetRace()->GetPlayerByMapObj(mapObj)) : nullptr;
		}
		...
		bool NetGame::GetAdapterAddresses(StringVec& addrVec)
		{
