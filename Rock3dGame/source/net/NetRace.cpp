...
namespace r3d
{
	namespace game
	{
		NetRace::NetRace(const Desc& desc): NetModelRPC(desc), _net(NetGame::I())
		{
			...
			RegRPC(&NetRace::OnSetSelectedLevel);
		...
		void NetRace::ReadMatch(std::istream& stream)
		{
			...
			net::Read(stream, curdiff);
			net::Read(stream, mode);
			net::Read(stream, upgradeMaxLevel);
			net::Read(stream, selectedLevel);
			...
			GAME_DIFF = curdiff;
			DoStartMatch(static_cast<Race::Mode>(mode), cDifficultyEnd, nullptr);
			...
			race()->GetTournament().SetWheater(static_cast<Environment::Wheater>(wheater));
		}

		void NetRace::WriteMatch(std::ostream& stream)
		{
			net::Write(stream, game()->currentDiff());
			net::Write(stream, race()->GetMode());
			net::Write(stream, garage().GetUpgradeMaxLevel());
			net::Write(stream, garage().GetSelectedLevel());
			...
			net::Write(stream, race()->GetTournament().GetWheater());
		...
		void NetRace::DoStartRace()
		{
			...
			for (auto iter = _net->netPlayers().begin(); iter != _net->netPlayers().end(); ++iter)
				(*iter)->RaceStarted();
		...
		void NetRace::CheckGoWait()
		{
			...
			{
				for (auto iter = _net->netOpponents().begin(); iter != _net->netOpponents().end(); ++iter)
		...
		void NetRace::OnExitRace(const net::NetMessage& msg, const net::NetCmdHeader& header, std::istream& stream)
		{
			...
			for (unsigned i = 0; i < count; ++i)
			{
				...
				//net::Read(stream, playerKills);
				//net::Read(stream, playerDeads);

				net::Read(stream, result.money);
				net::Read(stream, result.pickMoney);
				net::Read(stream, result.place);
				net::Read(stream, result.points);
				//net::Read(stream, result.killstotal);
				//net::Read(stream, result.deadstotal);
		...
		void NetRace::OnDamage1(const net::NetMessage& msg, const net::NetCmdHeader& header, std::istream& stream)
		{
			...
			if (targetPlayer == nullptr || targetPlayer->model()->GetCar().mapObj == nullptr)
			{
				LSL_LOG(
					lsl::StrFmt("NetRace::OnDamage target=NULL target=%d sender=%d value=%0.2f dmgType=%d", target,
						sender, value, damageType));
				return;
			}
			...
			{
				msg.Discard();
				Damage1(senderPlayer, targetPlayer, value, static_cast<GameObject::DamageType>(damageType),
				        net::cNetTargetOthers);
			}
			else
				targetPlayer->model()->GetCar().gameObj->Damage(
					senderPlayer ? senderPlayer->model()->GetId() : cUndefPlayerId, value, targetLife,
					death != 0 ? true : false, static_cast<GameObject::DamageType>(damageType));
		}
		...
		void NetRace::OnDamage2(const net::NetMessage& msg, const net::NetCmdHeader& header, std::istream& stream)
		{
			...
			{
				msg.Discard();
				Damage2(senderPlayer, targetMapObj, value, static_cast<GameObject::DamageType>(damageType),
				        net::cNetTargetOthers);
			}
			else
				targetMapObj->GetGameObj().Damage(senderPlayer ? senderPlayer->model()->GetId() : cUndefPlayerId, value,
				                                  targetLife, death != 0 ? true : false,
				                                  static_cast<GameObject::DamageType>(damageType));
		}

		void NetRace::OnSetUpgradeMaxLevel(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                   std::istream& stream)
		{
			int level;
		...
		void NetRace::OnSetSelectedLevel(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                 std::istream& stream)
		{
			int level;
			...
			garage().SetSelectedLevel(level);

			if (_net->isClient() && level > 0)
			{
				_net->race()->SetTrack(race()->GetTournament().GetCurPlanet().GetTracks()[level - 1]);
				//	race()->GetTournament().SetCurTrack(race()->GetTournament().GetCurPlanet().GetTracks()[level - 1]);
				race()->GetTournament().SetWheater(Environment::ewFair);
			}
		}

		void NetRace::OnSetCurrentDifficulty(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                     std::istream& stream)
		{
			int level;
			...
			_net->game()->currentDiff(static_cast<Difficulty>(level));
		}
		...
		void NetRace::OnSetMaxComputers(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                std::istream& stream)
		{
			net::Read(stream, _maxComputers);
		}

		void NetRace::OnSetWeaponUpgrades(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                  std::istream& stream)
		{
			bool weaponUpgrades;
		...
		void NetRace::OnSetSurvivalMode(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                std::istream& stream)
		{
			bool survivalMode;
		...
		void NetRace::OnSetEnableMineBug(const net::NetMessage& msg, const net::NetCmdHeader& header,
		                                 std::istream& stream)
		{
			bool enableMineBug;
		...
		void NetRace::Damage1(NetPlayer* sender, NetPlayer* target, float value, GameObject::DamageType damageType,
		                      unsigned netTarget)
		{
			...
			net::Write(stream, damageType);
			net::Write(stream, targetLife);
			net::Write(stream, death);
			CloseRPC();
		}

		void NetRace::Damage2(NetPlayer* sender, MapObj* target, float value, GameObject::DamageType damageType,
		                      unsigned netTarget)
		{
			LSL_ASSERT(target);
		...
		bool NetRace::OnConnected(net::INetConnection* sender)
		{
			...
			unsigned maxPlayers = std::min(_net->game()->maxPlayers(),
			                               _net->game()->GetRace()->IsCampaign()
				                               ? static_cast<unsigned>(Race::cCampaignMaxHumans)
				                               : static_cast<unsigned>(Race::cMaxPlayers));
		...
		void NetRace::StartRace()
		{
			...
			int maxComputers = _net->game()->maxComputers();
			int maxPlayers = _net->game()->maxPlayers();
		...
		void NetRace::ExitRace()
		{
			...
			for (auto iter = _net->players().begin(); iter != _net->players().end(); ++iter)
			{
				...
				//net::Write(stream, player->GetKillsTotal());
				//net::Write(stream, player->GetDeadsTotal());
				...
				//net::Write(stream, result.killstotal);
				//net::Write(stream, result.deadstotal);
		...
		int NetRace::GetSelectedLevel() const
		{
			return garage().GetSelectedLevel();
		}

		void NetRace::SetSelectedLevel(int value)
		{
			if (_net->isHost() && _net->game()->SelectedLevel() != value)
			{
				_net->game()->SelectedLevel(value);

				if (value > 0)
				{
					_net->race()->SetTrack(race()->GetTournament().GetCurPlanet().GetTracks()[value - 1]);
					race()->GetTournament().SetWheater(Environment::ewFair);
				}
				//race()->GetTournament().SetCurTrack(race()->GetTournament().GetCurPlanet().GetTracks()[value - 1]);

				std::ostream& stream = NewRPC(net::cNetTargetOthers, &NetRace::OnSetSelectedLevel);
				net::Write(stream, value);
				CloseRPC();
			}
		}
		...
		bool NetRace::GetWeaponUpgrades() const
		{
			return race()->GetWeaponUpgrades();
		}

		void NetRace::SetWeaponUpgrades(bool value)
		{
			if (_net->isHost() && _net->game()->weaponUpgrades() != value)
			{
				_net->game()->weaponUpgrades(value);

				std::ostream& stream = NewRPC(net::cNetTargetOthers, &NetRace::OnSetWeaponUpgrades);
				net::Write(stream, value);
				CloseRPC();
			}
		}

		bool NetRace::GetSurvivalMode() const
		{
			return race()->GetSurvivalMode();
		}

		void NetRace::SetSurvivalMode(bool value)
		{
			if (_net->isHost() && _net->game()->survivalMode() != value)
			{
		...
		bool NetRace::GetOilDestroyer() const
		{
			return race()->GetOilDestroyer();
		}

		void NetRace::SetOilDestroyer(bool value)
		{
			if (_net->isHost() && _net->game()->oilDestroyer() != value)
			{
				_net->game()->oilDestroyer(value);

				std::ostream& stream = NewRPC(net::cNetTargetOthers, &NetRace::OnSetOilDestroyer);
				net::Write(stream, value);
				CloseRPC();
			}
		}
		...
		void NetRace::PushLine(const stringW& text)
		{
		...
		Race::PlayerList NetRace::GetLeaverList() const
		{
			...
			{
				if (_net->GetPlayer(*iter) == nullptr)
