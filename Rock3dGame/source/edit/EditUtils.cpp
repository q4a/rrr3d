...
namespace r3d
{
	namespace edit
	{
		ScNodeCont::ScNodeCont(graph::BaseSceneNode* node): _node(node), _lastPos(_node->GetWorldPos()),
		                                                    _lastRot(_node->GetWorldRot()),
		                                                    _lastScale(_node->GetScale())
		{
			LSL_ASSERT(_node);
		...
		D3DXVECTOR3 ScNodeCont::GetLastPos() const
		{
			return _lastPos;
		}

		void ScNodeCont::SetLastPos(const D3DXVECTOR3& value)
		{
			_lastPos = value;
		}
		...
		D3DXQUATERNION ScNodeCont::GetLastRot() const
		{
			return _lastRot;
		}

		void ScNodeCont::SetLastRot(const D3DXQUATERNION& value)
		{
			_lastRot = value;
		}
		...
		D3DXVECTOR3 ScNodeCont::GetLastScale() const
		{
			return _lastScale;
		}

		void ScNodeCont::SetLastScale(const D3DXVECTOR3& value)
		{
			_lastScale = value;
		}
		...
		graph::BaseSceneNode* ScNodeCont::GetNode() const
		{
			return _node;
		}
	}
