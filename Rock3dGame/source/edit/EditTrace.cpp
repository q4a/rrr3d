...
namespace r3d
{
	namespace edit
	{
		...
		{
			WayPath* path = GetInst()->GetPath() ? new WayPath(GetInst()->GetPath()) : nullptr;
			return {path, path};
		}
		...
		{
			WayNode* node = GetInst()->GetFirst() ? new WayNode(GetInst()->GetFirst()) : nullptr;
			return {node, node};
		}

		void WayPath::Next(IWayNodeRef& ref)
		{
			game::WayNode* next = ref ? ref->GetImpl<WayNode>()->GetInst()->GetNext() : nullptr;
			WayNode* node = next ? new WayNode(next) : nullptr;
			ref.Reset(node, node);
		}
		...
		Trace::~Trace()
		{
			Trace::SelectNode(nullptr);
			Trace::SelectPath(nullptr);
			GetInst()->ClearPathes();

			Trace::EnableVisualize(false);
			delete _traceGfx;
		}

		Trace::NodeControl::NodeControl(Trace* trace, game::WayPoint* wayPoint, const ControlEventRef& mEvent):
			_trace(trace), _wayPoint(wayPoint), _event(mEvent), _link(nullptr), _dragRayPos(NullVector),
			_dragRayVec(NullVector)
		{
		...
		void Trace::NodeControl::FreeLink()
		{
			if (_link)
			{
				if (_trace->_traceGfx->GetPointLink() == _link)
					_trace->_traceGfx->SetPointLink(nullptr);
				SafeDelete(_link);
			}
		}

		bool Trace::NodeControl::CreateWay(game::WayNode* curNode, game::WayPoint* point, game::WayNode* node) const
		{
		...
		{
			_trace->_traceGfx->SetSelPoint(active ? _wayPoint : nullptr);
		}
		...
		void Trace::NodeControl::OnDrag(const D3DXVECTOR3& pos, const D3DXVECTOR3& scrRayPos,
		                                const D3DXVECTOR3& scrRayVec)
		{
			if (_link)
				_link->SetPos(pos);
		}
		...
		void Trace::NodeControl::OnShiftAction(const D3DXVECTOR3& scrRayPos, const D3DXVECTOR3& scrRayVec)
		{
			...
			if (!curNode)
				curNode = (!_wayPoint->GetNodes().empty()) ? *_wayPoint->GetNodes().begin() : nullptr;
			if (curNode)
			{
				game::WayPath* path = curNode->GetPath();
				CreateWay(curNode, newPoint, nullptr);
			}
			...
			if (_event)
			{
				const auto point = new WayPoint(newPoint);
				const ControlEventRef mEvent = _event;
		...
		D3DXVECTOR3 Trace::NodeControl::GetLastPos() const
		{
			return _wayPoint->GetPos();
		}

		void Trace::NodeControl::SetLastPos(const D3DXVECTOR3& value)
		{
			_wayPoint->SetPos(value);
		}
		...
		D3DXQUATERNION Trace::NodeControl::GetLastRot() const
		{
			return NullQuaternion;
		}

		void Trace::NodeControl::SetLastRot(const D3DXQUATERNION& value)
		{
			//Nothing
		}
		...
		D3DXVECTOR3 Trace::NodeControl::GetLastScale() const
		{
			float radius = _wayPoint->GetSize() / sqrt(3.0f);
		...
		void Trace::NodeControl::SetLastScale(const D3DXVECTOR3& value)
		{
			_wayPoint->SetSize(D3DXVec3Length(&value));
		}
		...
		{
			float minDist = 0;
			game::WayPoint* point = nullptr;

			for (const auto iter : GetInst()->GetPoints())
			{
				float t;
				if (RayCastIntersectSphere(rayPos, rayVec, iter->GetPos(), iter->GetSize() / 2.0f, &t))
				{
					if (minDist > t || point == nullptr)
					{
						point = iter;
						minDist = t;
					}
		...
		IWayPointRef Trace::PickPoint(const Point& scrCoord)
		{
			...
			return {point ? new WayPoint(point) : nullptr, this};
		}

		IWayPointRef Trace::AddPoint()
		{
			return {new WayPoint(GetInst()->AddPoint()), this};
		}
		...
		IScNodeContRef Trace::GetPointControl(const IWayPointRef& ref, const ControlEventRef& mEvent)
		{
			...
			return {new NodeControl(this, ref->GetImpl<WayPoint>()->GetInst(), mEvent), this};
		}

		IWayPathRef Trace::AddPath()
		{
			return {new WayPath(GetInst()->AddPath()), this};
		}
		...
		{
			const auto iter = GetInst()->GetPoints().begin();
		...
		{
			auto* point = ref->GetImpl<WayPoint>();

			if (++(point->traceIter) != GetInst()->GetPoints().end())
			{
				const auto newPoint = new WayPoint(static_cast<game::WayPoint*>(*point->traceIter));
		...
		{
			const auto iter = GetInst()->GetPathes().begin();
			WayPath* path = nullptr;
		...
		{
			auto* path = ref->GetImpl<WayPath>();

			if (++(path->traceIter) != GetInst()->GetPathes().end())
			{
				const auto newPath = new WayPath(static_cast<game::WayPath*>(*path->traceIter));
