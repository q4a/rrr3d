...
namespace r3d
{
	namespace game
	{
		namespace n
		{
			GameModeFrame::GameModeFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_mainMenu(mainMenu)
			{
			}
			...
			{
				const string menuItemsStr[cMenuItemEnd] = {_SC(svChampionship), _SC(svSkirmish), _SC(svBack)};

				if (value)
				{
					_mainMenu->SetItems(menuItemsStr, cMenuItemEnd, false, this);
					menu()->SetButtonEnabled(_mainMenu->GetItem(miSkirmish),
					                         menu()->GetRace()->IsTutorialCompletedFirstStage());
				}
			}
			...
			DifficultyFrame::DifficultyFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _mainMenu(mainMenu), _difficulty(gdNormal)
			{
			}
...
#endif
							{
								_mainMenu->StartMatch(Race::rmChampionship, _difficulty, nullptr);
							}
						}
						else
							_mainMenu->StartMatch(Race::rmSkirmish, _difficulty, nullptr);
			...
			NetBrowserFrame::NetBrowserFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _mainMenu(mainMenu), _gridScroll(0)
			{
			...
			void NetBrowserFrame::UpdateGrid()
			{
				...
				auto iter = _grid->GetChildren().begin();
				Vector<Menu::NavElement> navElements;
				Vector<gui::Widget*> gridItems;

				for (unsigned i = 0; i < count; ++i)
				{
					net::Endpoint endpoint = net()->netService().endpointList()[i];

					gui::Button* item = iter != _grid->GetChildren().end() ? static_cast<gui::Button*>(*iter) : nullptr;
					if (item == nullptr)
					{
					...
					item->SetText(StrFmt("%s", endpoint.address.c_str()));
					...
					if (index >= 2)
					{
						Menu::NavElement navElement = {
							gridItems[index - 1], {nullptr, nullptr, gridItems[index - 2], gridItems[index]},
					...
					if (i + 1 == count)
					{
						...
						{
							Menu::NavElement navElement = {
								gridItems[0], {nullptr, nullptr, _upArrow, gridItems[1]},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement);
						}
						...
						Menu::NavElement navElement2 = {
							gridItems[itemCount - 1],
							{nullptr, nullptr, itemCount >= 2 ? gridItems[itemCount - 2] : _upArrow, _downArrow},
							{cVirtualKeyEnd, cVirtualKeyEnd}
						};
				...
				{
					Menu::NavElement navElement = {
						_mainMenu->GetItem(miBack), {nullptr, nullptr, _downArrow, _upArrow}, {vkBack, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					Menu::NavElement navElement = {
						_upArrow,
						{nullptr, nullptr, _mainMenu->GetItem(miBack), itemCount > 0 ? gridItems[0] : nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					Menu::NavElement navElement = {
						_downArrow,
						{
							nullptr, nullptr, itemCount > 0 ? gridItems[itemCount - 1] : nullptr,
							_mainMenu->GetItem(miBack)
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
			...
			void NetBrowserFrame::AdjustGrid(const D3DXVECTOR2& vpSize)
			{
				...
				menu()->SetButtonEnabled(_downArrow, _gridScroll < std::max(
					                         static_cast<int>(ceil(i / static_cast<float>(cellsOnLine))) - cellsGrid /
					                         cellsOnLine, 0));
			...
			{
				const string menuItemsStr[cMenuItemEnd] = {_SC(svBack)};
			...
			bool NetBrowserFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				...
				unsigned i = 0;
				for (auto iter = _grid->GetChildren().begin(); iter != _grid->GetChildren().end(); ++iter, ++i)
				{
					auto item = static_cast<gui::Button*>(*iter);
			...
			NetIPAddressFrame::NetIPAddressFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _mainMenu(mainMenu)
			{
			...
			{
				const string menuItemsStr[cMenuItemEnd] = {_SC(svConnect), _SC(svBack)};
			...
			bool NetIPAddressFrame::OnHandleInput(const InputMessage& msg)
			{
				...
				{
					string text = _labels[mlIPAdress]->GetText();
				...
				if (msg.key == menu()->GetControl()->CharToVirtualKey(VK_OEM_PERIOD))
				{
					string text = _labels[mlIPAdress]->GetText();
					PushLine(text + '.');
					return true;
				...
				if (msg.key == vkChar && isdigit(static_cast<char>(msg.unicode)))
				{
					string text = _labels[mlIPAdress]->GetText();
					string input = ConvertStrWToA(stringW(1, msg.unicode));

					PushLine(text + input);

					return true;
				}
				if (static_cast<char>(msg.unicode) == '.')
				{
					string text = _labels[mlIPAdress]->GetText();
					PushLine(text + '.');
					return true;
			...
			IdentifyFrame::IdentifyFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_mainMenu(mainMenu), _difficulty(gdNormal)
			{
			}
			...
			void IdentifyFrame::OnShow(bool value)
			{
				string menuItems[cMenuItemEnd];
				menuItems[miGamer] = _SC(svIsGamer);
				menuItems[miSpectator] = _SC(svIsSpectator);
				menuItems[miBack] = _SC(svBack);
			...
			bool IdentifyFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				Menu* menu = this->menu();
				...
				if (sender == _mainMenu->GetItem(miGamer))
				{
					menu->ISpectator(false);
					menu->SetGamersState();
					return true;
				}

				if (sender == _mainMenu->GetItem(miSpectator))
				{
					menu->ISpectator(true);
					menu->SetGamersState();
					return true;
				}
			...
			void IdentifyFrame::OnProcessEvent(unsigned id, EventData* data)
			{
				if (id == cVideoStopped)
				{
					menu()->GetGUI()->GetRoot()->SetVisible(true);
...
void SteamBrowserFrame::OnDisconnectedPlayer(NetPlayer* sender)
{
	StartWaiting(false);
			...
			ServerTypeFrame::ServerTypeFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _mainMenu(mainMenu)
			{
			}
	...
	if (sender == _mainMenu->GetItem(miLobby))
	{
		...
		return true;
			...
			ClientTypeFrame::ClientTypeFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _mainMenu(mainMenu)
			{
			}
	...
	if (sender == _mainMenu->GetItem(miConnectLobby))
	{
		...
		return true;
			...
			NetworkFrame::NetworkFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_mainMenu(mainMenu)
			{
				D3DXCOLOR color1 = Menu::cTextColor;
			...
			{
				const string menuItemsStr[cMenuItemEnd] = {_SC(svNetCreate), _SC(svConnect), _SC(svBack)};
			...
			TournamentFrame::TournamentFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _mainMenu(mainMenu)
			{
			}
			...
			{
				const string menuItemsStr[cMenuItemEnd] = {_SC(svContinue), _SC(svNewGame), _SC(svLoad), _SC(svBack)};
				...
				if (value)
				{
					...
					_mainMenu->SetItems(menuItemsStr, cMenuItemEnd, false, this);
					menu()->SetButtonEnabled(_mainMenu->GetItem(miContinue),
					                         menu()->GetLastProfile(netGame) != nullptr);

					for (auto iter = menu()->GetRace()->GetProfiles().begin(); iter != menu()->GetRace()->GetProfiles().
					     end(); ++iter)
			...
			CreditsFrame::CreditsFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_mainMenu(mainMenu), _scrollTime(0)
			{
				_label = menu->CreateLabel(svCredits, root(), "Small", NullVec2, gui::Text::haCenter, gui::Text::vaTop,
				                           D3DXCOLOR(0xffff8a70));
			...
			ProfileFrame::ProfileFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_mainMenu(mainMenu), _gridScroll(0)
			{
				_grid = menu->CreateGrid(root(), nullptr, gui::Grid::gsVertical);
				_grid->SetAlign(gui::Widget::waTop);
			...
			void ProfileFrame::UpdateGrid()
			{
				...
				bool netGame = _mainMenu->ContainsState(MainMenu::msNetwork);
				Vector<Menu::NavElement> navElements;
				Vector<std::pair<gui::Widget*, gui::Widget*>> gridItems;
				...
				for (unsigned i = 0; i < count; ++i)
				{
					...
					if (profile->netGame() == netGame)
					{
						...
						gui::Button* closeBut = menu()->CreateMenuButton(
							svNull, "", "GUI\\buttonBg6.png", "GUI\\buttonBgSel6.png", item, this,
							D3DXVECTOR2(1.8f, 1.8f), gui::Button::bsSelAnim, clrWhite);
						...
						{
							Menu::NavElement navElement = {
								gridItems[index - 1].first,
								{
									gridItems[index - 1].second, gridItems[index - 1].second,
									gridItems[index - 2].first, gridItems[index].first
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement);
							...
							Menu::NavElement navElement2 = {
								gridItems[index - 1].second,
								{
									gridItems[index - 1].first, gridItems[index - 1].first, gridItems[index - 2].second,
									gridItems[index].second
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement2);
					...
					if (i + 1 == count)
					{
						if (itemCount >= 2)
						{
							Menu::NavElement navElement = {
								gridItems[0].first,
								{gridItems[0].second, gridItems[0].second, _upArrow, gridItems[1].first},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement);
							...
							Menu::NavElement navElement2 = {
								gridItems[0].second,
								{gridItems[0].first, gridItems[0].first, _upArrow, gridItems[1].second},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement2);
						}
						...
						{
							Menu::NavElement navElement = {
								gridItems[itemCount - 1].first,
								{
									gridItems[itemCount - 1].second, gridItems[itemCount - 1].second,
									itemCount >= 2 ? gridItems[itemCount - 2].first : _upArrow, _downArrow
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement);
							...
							Menu::NavElement navElement2 = {
								gridItems[itemCount - 1].second,
								{
									gridItems[itemCount - 1].first, gridItems[itemCount - 1].first,
									itemCount >= 2 ? gridItems[itemCount - 2].second : _upArrow, _downArrow
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement2);
				...
				}
				{
					Menu::NavElement navElement = {
						_upArrow,
						{nullptr, nullptr, _mainMenu->GetItem(miBack), itemCount > 0 ? gridItems[0].first : nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					Menu::NavElement navElement = {
						_downArrow,
						{
							nullptr, nullptr, itemCount > 0 ? gridItems[itemCount - 1].first : nullptr,
							_mainMenu->GetItem(miBack)
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
			...
			bool ProfileFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				...
				for (unsigned i = 0; i < count; ++i)
				{
					...
					{
						menu()->ShowAccept(true, GetString("svHintDeleteProfile"), GetString(svYes), GetString(svNo),
						                   uiRoot()->GetVPSize() / 2, gui::Widget::waCenter, this, profile);
						return true;
					}
				}
			...
			MainFrame::MainFrame(Menu* menu, MainMenu* mainMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_mainMenu(mainMenu)
			{
			}
		...
		if (menu()->IsSteamSavingInProcess())
			...
			MainMenu::MainMenu(Menu* menu, gui::Widget* parent): _menu(menu), _state(msMain),
			                                                     _serverType(cServerTypeEnd),
			                                                     _steamHostMode(Race::rmChampionship),
			                                                     _steamHostDifficulty(gdNormal),
			                                                     _steamHostProfile(nullptr), _steamHostStarting(false),
			                                                     _steamConnecting(false)
			{
				_root = _menu->GetGUI()->CreateDummy();
				_root->SetParent(parent);

				_winFrame = _menu->CreatePlane(_root, nullptr, "GUI\\mainFrame.dds", true);
				_winFrame->SetAnchor(gui::Widget::waCenter);
				_winFrame->GetMaterial().GetSampler().SetFiltering(graph::Sampler2d::sfLinear);

				_topPanel = menu->CreatePlane(_root, nullptr, "GUI\\topPanel5.png", true, IdentityVec2,
				                              gui::Material::bmTransparency);
				_topPanel->SetAlign(gui::Widget::waCenter);

				_bottomPanel = menu->CreatePlane(_root, nullptr, "GUI\\bottomPanel5.png", true, IdentityVec2,
				                                 gui::Material::bmTransparency);
				...
				_identifyFrame = new IdentifyFrame(menu, this, _root);
	...
	delete _matchmakingFrame;
			...
			void MainMenu::ApplyState(State state)
			{
				...
				_identifyFrame->Show(state == msIdentify);
			...
			void MainMenu::AdjustMenuItems(const D3DXVECTOR2& vpSize)
			{
				if (_menuItems.size() > 0)
				{
					...
					for (unsigned i = 0; i < _menuItems.size(); ++i)
					{
						...
						_menuItems[i]->SetPos(vpSize.x / 2 + 5,
						                      vpSize.y / 2 - 100.0f + index * (_menuItems[i]->GetSize().y + 5));
						++index;
					}
	...
	case cSteamAuthEnded:
		...
		break;
			...
			void MainMenu::OnDisconnected(net::INetConnection* sender)
			{
				...
				_menu->ShowMessage(_menu->GetString(svWarning), _menu->GetString(svHintHostConnectionFailed),
			...
			void MainMenu::AdjustLayout(const D3DXVECTOR2& vpSize)
			{
				...
				_identifyFrame->AdjustLayout(vpSize);
	...
	_steamBrowser->AdjustLayout(vpSize);
			...
			{
				for (auto iter = _stateStack.begin(); iter != _stateStack.end(); ++iter)
					if (*iter == value)
						return true;
				return false;
			}
			...
			void MainMenu::SetItems(const string menuItemsStr[], unsigned count, bool mainMenu,
			                        gui::Widget::Event* guiEvent)
			{
				...
				for (unsigned i = 0; i < count; ++i)
				{
					gui::Button* item = i < _menuItems.size() ? _menuItems[i] : nullptr;
					...
					{
						Menu::NavElement navElement = {
							_menuItems[i - 1], {nullptr, nullptr, _menuItems[i - 2], _menuItems[i]},
							{cVirtualKeyEnd, cVirtualKeyEnd}
						};
						navElements.push_back(navElement);
					}
					...
					if (i + 1 == count)
					{
						...
						{
							Menu::NavElement navElement = {
								_menuItems[0], {nullptr, nullptr, _menuItems[i], _menuItems[1]},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement);
						}
						...
						Menu::NavElement navElement2 = {
							_menuItems[i],
							{
								nullptr, nullptr, count >= 2 ? _menuItems[i - 1] : nullptr,
								count >= 2 ? _menuItems[0] : nullptr
							},
							{!mainMenu ? vkBack : cVirtualKeyEnd, cVirtualKeyEnd}
						};
						navElements.push_back(navElement2);
					}
				}
	...
	if (steamId.IsValid() && _menu->GetSteamService()->isInit() && _menu->GetSteamService()->InitiateConnection(steamId))
	{
		...
		_menu->ShowMessageLoading();
			...
			gui::Button* MainMenu::CreateMenuButton(const string& name, gui::Widget* parent,
			                                        gui::Widget::Event* guiEvent)
			{
				const auto color1 = D3DXCOLOR(0xffff8a70);

				gui::Button* button = _menu->CreateMenuButton(name, "Header", "", "GUI\\mainItemSel5.png", parent,
				                                              guiEvent, IdentityVec2, gui::Button::bsSimple, color1,
				                                              Menu::ssButton1);
