...
namespace r3d
{
	namespace game
	{
		...
		extern float Y_VPSIZE = 0.0f;
		extern float X_VPSIZE_MMENU = 0.0f;
		extern float Y_VPSIZE_MMENU = 0.0f;
		...
		extern bool CLR_PAGE_CHANGED = false;
		extern bool RIGHT_CLR_PAGE_CHANGED = false;
		...
		//const std::string AchievmentModel::cSingleKill = "singleKill";
		const std::string AchievmentModel::cDoubleKill = "doubleKill";
		const std::string AchievmentModel::cTripleKill = "tripleKill";
		const std::string AchievmentModel::cMegaKill = "megaKill";
		const std::string AchievmentModel::cMonsterKill = "monsterKill";
		...
		const std::string AchievmentModel::cLastLap = "LastLap";
		const std::string AchievmentModel::cLastLoop = "LastLoop";
		const std::string AchievmentModel::cFirstBlood = "firstBlood";
		const std::string AchievmentModel::cBomber = "Bomber";
		...
		void Achievment::SaveTo(SWriter* writer)
		{
			SWriteEnum(writer, "state", _state, cStateStr, cStateEnd);
			writer->WriteValue("price", _price);
		}

		void Achievment::LoadFrom(SReader* reader)
		{
			SReadEnum(reader, "state", _state, cStateStr, cStateEnd);
		...
		void AchievmentMapObj::SaveTo(SWriter* writer)
		{
		...
		void AchievmentMapObj::LoadFrom(SReader* reader)
		{
		...
		{
			return _records.Size() > 0 ? _records.front() : nullptr;
		}
		...
		void AchievmentMapObj::SetRecord(Record* value)
		{
			if (_records.Size() > 0)
			{
				if (ReplaceRef(_records.front(), value))
		...
		void AchievmentGamer::SaveTo(SWriter* writer)
		{
			Achievment::SaveTo(writer);
		...
		void AchievmentGamer::LoadFrom(SReader* reader)
		{
			Achievment::LoadFrom(reader);
		...
		void AchievmentCondition::SaveTo(SWriter* writer)
		{
		...
		void AchievmentCondition::LoadFrom(SReader* reader)
		{
		...
		AchievmentConditionBonus::AchievmentConditionBonus(const Desc& desc): AchievmentCondition(desc), _bonusCount(0),
		                                                                      _bonusTotalCount(0)
		{
		}
		...
		void AchievmentConditionBonus::SaveTo(SWriter* writer)
		{
			AchievmentCondition::SaveTo(writer);

			SWriteEnum(writer, "bonusType", _bonusType, GameObject::cBonusTypeStr, GameObject::cBonusTypeEnd);
		}

		void AchievmentConditionBonus::LoadFrom(SReader* reader)
		{
			AchievmentCondition::LoadFrom(reader);

			SReadEnum(reader, "bonusType", _bonusType, GameObject::cBonusTypeStr, GameObject::cBonusTypeEnd);
		}
		...
		AchievmentConditionSpeedKill::AchievmentConditionSpeedKill(const Desc& desc): AchievmentCondition(desc),
			_killsNum(0), _killsTime(0), _time(0), _curKills(0)
		{
			RegProgressEvent();
		}
		...
		void AchievmentConditionSpeedKill::SaveTo(SWriter* writer)
		{
			AchievmentCondition::SaveTo(writer);
		...
		void AchievmentConditionSpeedKill::LoadFrom(SReader* reader)
		{
			AchievmentCondition::LoadFrom(reader);
		...
		AchievmentConditionBombKill::AchievmentConditionBombKill(const Desc& desc): AchievmentCondition(desc),
			_killzNum(0), _killzTime(0), _timez(0), _curKillz(0)
		{
			RegProgressEvent();
		}

		AchievmentConditionBombKill::~AchievmentConditionBombKill()
		{
			UnregProgressEvent();
		}

		void AchievmentConditionBombKill::OnResetRaceState()
		{
			_timez = 0;
			_curKillz = 0;
		}

		void AchievmentConditionBombKill::OnProcessEvent(unsigned id, EventData* data)
		{
			if (!(data && data->playerId == Race::cHuman))
				return;
		...
		void AchievmentConditionBombKill::OnProgress(float deltaTime)
		{
			if (_timez > 0 && (_timez -= deltaTime) <= 0)
			{
				_timez = 0.0f;
				_curKillz = 0;
			}
		...
		void AchievmentConditionBombKill::SaveTo(SWriter* writer)
		{
			AchievmentCondition::SaveTo(writer);

			writer->WriteValue("killzNum", _killzNum);
			writer->WriteValue("killzTime", _killzTime);
		}

		void AchievmentConditionBombKill::LoadFrom(SReader* reader)
		{
			AchievmentCondition::LoadFrom(reader);

			reader->ReadValue("killzNum", _killzNum);
			reader->ReadValue("killzTime", _killzTime);
		}

		int AchievmentConditionBombKill::killzNum() const
		{
			return _killzNum;
		}

		void AchievmentConditionBombKill::killzNum(int value)
		{
			_killzNum = value;
		}

		float AchievmentConditionBombKill::killzTime() const
		{
			return _killzTime;
		}

		void AchievmentConditionBombKill::killzTime(float value)
		{
			_killzTime = value;
		}

		//
		AchievmentConditionRaceKill::AchievmentConditionRaceKill(const Desc& desc): AchievmentCondition(desc),
			_killsNum(0), _curKills(0)
		{
		}
		...
		void AchievmentConditionRaceKill::SaveTo(SWriter* writer)
		{
			AchievmentCondition::SaveTo(writer);
		...
		void AchievmentConditionRaceKill::LoadFrom(SReader* reader)
		{
			AchievmentCondition::LoadFrom(reader);
		...
		AchievmentConditionLapPass::AchievmentConditionLapPass(const Desc& desc): AchievmentCondition(desc), _place(1),
			_lapCount(0)
		{
		}
		...
		void AchievmentConditionLapPass::SaveTo(SWriter* writer)
		{
			AchievmentCondition::SaveTo(writer);
		...
		void AchievmentConditionLapPass::LoadFrom(SReader* reader)
		{
			AchievmentCondition::LoadFrom(reader);
		...
		AchievmentConditionSurvival::AchievmentConditionSurvival(const Desc& desc): AchievmentCondition(desc),
			_curDeaths(0)
		{
		}
		...
		AchievmentConditionFinalLap::AchievmentConditionFinalLap(const Desc& desc): AchievmentCondition(desc),
			_curDeaths(0)
		{
		}

		void AchievmentConditionFinalLap::OnResetRaceState()
		{
		}

		void AchievmentConditionFinalLap::OnProcessEvent(unsigned id, EventData* data)
		{
			if (!(data && data->playerId == Race::cHuman))
				return;
		...
		AchievmentConditionFirstKill::AchievmentConditionFirstKill(const Desc& desc): AchievmentCondition(desc),
			_curKills(0)
		{
		}
		...
		AchievmentModel::AchievmentModel(Race* race, const string& name): _race(race), _points(0)
		{
			...
			_condClasses.Add<AchievmentConditionBombKill>(cBombKill);
			...
			_condClasses.Add<AchievmentConditionFinalLap>(cFinalLap);
		...
		void AchievmentModel::GenerateLib()
		{
			...
			AchievmentConditionBombKill* condBombKill;
			...
			AchievmentConditionFinalLap* condFinalLap;
			...
			condBonus->reward(400);

			condSpeedKill = AddCond<AchievmentConditionSpeedKill>(cDoubleKill);
			condSpeedKill->reward(700);
			condSpeedKill->killsNum(2);
			condSpeedKill->killsTime(4.0f);

			condSpeedKill = AddCond<AchievmentConditionSpeedKill>(cTripleKill);
			condSpeedKill->reward(800);
			condSpeedKill->killsNum(3);
			condSpeedKill->killsTime(4.0f);

			condSpeedKill = AddCond<AchievmentConditionSpeedKill>(cMegaKill);
			condSpeedKill->reward(1000);
			condSpeedKill->killsNum(4);
			condSpeedKill->killsTime(6.0f);

			condSpeedKill = AddCond<AchievmentConditionSpeedKill>(cMonsterKill);
			condSpeedKill->reward(1500);
			condSpeedKill->killsNum(5);
			condSpeedKill->killsTime(8.0f);

			condBombKill = AddCond<AchievmentConditionBombKill>(cBomber);
			condBombKill->reward(1500);
			condBombKill->killzNum(5);
			condBombKill->killzTime(300.0f);

			condRaceKill = AddCond<AchievmentConditionRaceKill>(cDevastator);
			condRaceKill->reward(800);
			condRaceKill->killsNum(10);

			condLapPass = AddCond<AchievmentConditionLapPass>(cMegaRacer);
			condLapPass->reward(1000);
			condLapPass->place(1);

			condLapBreak = AddCond<AchievmentConditionLapBreak>(cBreakRacer);
			condLapBreak->reward(500);

			condSurvival = AddCond<AchievmentConditionSurvival>(cSurvival);
			condSurvival->reward(700);

			condFinalLap = AddCond<AchievmentConditionFinalLap>(cLastLap);
			condFinalLap->reward(400);
		...
		void AchievmentModel::LoadLib()
		{
			...
			RootNode rootNode("achievmentRoot", _race);
		...
		void AchievmentModel::Save(SWriter* writer)
		{
			writer->WriteValue("points", _points);

			SWriter* items = writer->NewDummyNode("items");
			for (auto iter = _items.begin(); iter != _items.end(); ++iter)
			{
				SWriter* child = items->NewDummyNode(iter->first.c_str());
				child->WriteAttr("classId", iter->second->classId());

				iter->second->SaveTo(child);
			}

			SWriter* conditions = writer->NewDummyNode("conditions");
			for (auto iter = _conditions.begin(); iter != _conditions.end(); ++iter)
			{
				SWriter* child = conditions->NewDummyNode(iter->first.c_str());
				child->WriteAttr("classId", iter->second->classId());
		...
		void AchievmentModel::Load(SReader* reader)
		{
			DeleteAll();

			reader->ReadValue("points", _points);

			SReader* items = reader->ReadValue("items");
			if (items)
			{
				SReader* child = items->FirstChildValue();
				while (child)
				{
					const SerialNode::ValueDesc* attr = child->ReadAttr("classId");
					LSL_ASSERT(attr);
					unsigned classId;
			...
			SReader* conditions = reader->ReadValue("conditions");
			if (conditions)
			{
				SReader* child = conditions->FirstChildValue();
				while (child)
				{
					const SerialNode::ValueDesc* attr = child->ReadAttr("classId");
					LSL_ASSERT(attr);
		...
		bool AchievmentModel::CheckMapObj(Record* record) const
		{
			for (auto iter = _items.begin(); iter != _items.end(); ++iter)
			{
				...
				if (item->state() != Achievment::asOpened && item->classId() == cMapObj && static_cast<AchievmentMapObj
					*>(item)->ContainsRecord(record))
					return false;
			}
		...
		bool AchievmentModel::CheckGamerId(int gamerId) const
		{
			...
			{
				Achievment* item = iter->second;
				if (item->state() != Achievment::asOpened && item->classId() == cGamer && static_cast<AchievmentGamer*>(
					item)->GetGamerId() == gamerId)
					return false;
			}
