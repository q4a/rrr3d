...
namespace r3d
{
	namespace game
	{
		...
		LogicBehaviors* LogicBehavior::GetOwner() const
		{
			return _owner;
		}

		Logic* LogicBehavior::GetLogic() const
		{
			return _owner->GetLogic();
		}

		Map* LogicBehavior::GetMap() const
		{
			return GetLogic()->GetMap();
		}


		LogicEventEffect::LogicEventEffect(LogicBehaviors* owner): _MyBase(owner), _effect(nullptr), _pos(NullVector)
		{
		...
		void LogicEventEffect::DestroyEffObj(MapObj* mapObj) const
		{
		...
		void LogicEventEffect::Save(SWriter* writer)
		{
		...
		void LogicEventEffect::Load(SReader* reader)
		{
		...
		MapObjRec* LogicEventEffect::GetEffect() const
		{
			return _effect;
		}
		...
		{
			const std::pair<ContactMap::iterator, bool> res = _contactMap.insert(ContactMap::value_type(key, nullptr));
			if (res.second)
			{
				LSL_ASSERT(res.first->second == 0);

				const auto node = new ContactNode();
				res.first->second = node;

				if (!node->source && !_sounds.empty())
				{
					auto index = static_cast<unsigned>(_sounds.size() * Random());
		...
		void PairPxContactEffect::InsertContact(ContactMap::iterator iter, NxShape* shape1, NxShape* shape2,
		...
		PairPxContactEffect::ContactMap::iterator PairPxContactEffect::ReleaseContact(
			ContactMap::iterator iter, ContactList::iterator cIter1, ContactList::iterator cIter2, bool death,
			float deltaTime, float cRelTime)
		{
			for (auto cIter = cIter1; cIter != cIter2; ++cIter)
			{
		...
		{
			for (auto iter = _contactMap.begin(); iter != _contactMap.end();)
				iter = ReleaseContact(iter, iter->second->list.begin(), iter->second->list.end(), death);

			LSL_ASSERT(_contactMap.empty());
		}

		void PairPxContactEffect::RemoveContactByEffect(MapObj* effect) const
		{
			for (const auto& iter : _contactMap)
				for (auto& cIter : iter.second->list)
					if (cIter.effect == effect)
					{
						SafeRelease(cIter.effect);
						return;
					}
		}
		...
		void PairPxContactEffect::OnContact(const px::Scene::OnContactEvent& contact1,
		                                    const px::Scene::OnContactEvent& contact2)
		{
#if !_DEBUG
			NxContactStreamIterator streamIter(contact1.stream);
			const ContactNode* contNode = nullptr;
			ContactMap::iterator mapIter;

			while (streamIter.goNextPair())
			{
				NxShape* shapes[2];
				shapes[0] = !streamIter.isDeletedShape(0) ? streamIter.getShape(0) : nullptr;
				shapes[1] = !streamIter.isDeletedShape(1) ? streamIter.getShape(1) : nullptr;

				bool checkShapes = D3DXVec3Length(&contact1.sumFrictionForce) > 10000.0f;
				for (auto& shape : shapes)
				{
					checkShapes &= shape && shape->getType() != NX_SHAPE_WHEEL;
					if (!checkShapes)
						break;
				}
		...
		void LogicBehaviors::OnContact(const px::Scene::OnContactEvent& contact1,
		                               const px::Scene::OnContactEvent& contact2)
		{
		...
		Logic* LogicBehaviors::GetLogic() const
		{
			return _logic;
		}


		Logic::Logic(World* world): _world(world), _initSndCat(false), _touchBorderDamage(0, 0),
		                            _touchBorderDamageForce(0, 0), _touchCarDamage(0, 0), _touchCarDamageForce(0, 0)
		{
		...
		void Logic::PxSceneUser::OnContact(const px::Scene::OnContactEvent& contact1,
		                                   const px::Scene::OnContactEvent& contact2)
		{
		...
		void Logic::FreeSndCat()
		{
			if (_initSndCat)
			{
				...
				for (const auto& i : _sndCat)
					GetAudio()->ReleaseVoice(i);
			}
		}

		void Logic::Save(SWriter* writer)
		{
			writer->WriteValue("behaviors", _behaviors);
		}

		void Logic::Load(SReader* reader)
		{
			reader->ReadValue("behaviors", _behaviors);
		}

		void Logic::RegFixedStepEvent(IFixedStepEvent* user) const
		{
			_world->RegFixedStepEvent(user);
		}

		void Logic::UnregFixedStepEvent(IFixedStepEvent* user) const
		{
			_world->UnregFixedStepEvent(user);
		}

		void Logic::RegProgressEvent(IProgressEvent* user) const
		{
			_world->RegProgressEvent(user);
		}

		void Logic::UnregProgressEvent(IProgressEvent* user) const
		{
			_world->UnregProgressEvent(user);
		}

		void Logic::RegLateProgressEvent(ILateProgressEvent* user) const
		{
			_world->RegLateProgressEvent(user);
		}

		void Logic::UnregLateProgressEvent(ILateProgressEvent* user) const
		{
			_world->UnregLateProgressEvent(user);
		}

		void Logic::RegFrameEvent(IFrameEvent* user) const
		{
			_world->RegFrameEvent(user);
		}

		void Logic::UnregFrameEvent(IFrameEvent* user) const
		{
		...
		{
			for (const auto iter : _gameObjList)
			{
				iter->Release();
				delete iter;
			}
		...
		void Logic::Shot(Player* player, MapObj* target, Player::SlotType type)
		{
			...
			else
			{
				...
				WeaponItem* wpn = player->GetSlotInst(type)
					                  ? player->GetSlotInst(type)->GetItem().IsWeaponItem()
					                  : nullptr;
		...
		void Logic::Shot(Player* player, MapObj* target)
		{
			...
			else
			{
				...
				{
					const auto type = static_cast<Player::SlotType>(i);
					WeaponItem* wpn = player->GetSlotInst(type)
						                  ? player->GetSlotInst(type)->GetItem().IsWeaponItem()
						                  : nullptr;
		...
		void Logic::Damage(GameObject* sender, int senderPlayerId, GameObject* target, float value,
		                   GameObject::DamageType damageType) const
		{
			LSL_ASSERT(target);

			if (target->GetMapObj() == nullptr)
			{
				LSL_LOG("Logic::Damage target->GetMapObj() = NULL")
				return;
			}

			Player* targetPlayer = target->GetMapObj() ? target->GetMapObj()->GetPlayer() : nullptr;
			if (targetPlayer)
			{
				...
				{
					auto& reflector = slot->GetItem<ReflectorItem>();
		                      ...
		                      bool destroy) const
		{
			...
			if (IsNetGame())
			{
				...
				senderPlayer->TakeBonus(bonus->GetMapObj(), type, value, destroy);
			}
			else
			{
				player->TakeBonus(bonus, type, value, destroy);
			}

			return true;
		}

		bool Logic::MineContact(Proj* sender, GameObject* target, const D3DXVECTOR3& point) const
		{
		...
		void Logic::ReleaseSndSource(snd::Source* source) const
		{
		...
		{
			MapObjLib::Category catList[3] = {MapObjLib::ctEffects, MapObjLib::ctCar, MapObjLib::ctBonuz};

			GetMap()->GetMapObjList(MapObjLib::ctDecoration).OnProgressSpecial(deltaTime);

			for (const auto& i : catList)
				GetMap()->GetMapObjList(i).OnProgress(deltaTime);

			//for (int i = 0; i < MapObjLib::cCategoryEnd; ++i)
			//	GetMap()->GetMapObjList((MapObjLib::Category)i).OnProgress(deltaTime);

			for (auto iter = _gameObjList.begin(); iter != _gameObjList.end();)
			{
		...
		bool Logic::IsNetGame() const
		{
			return GetNet()->isStarted();
		}

		Map* Logic::GetMap() const
		{
			return _world->GetMap();
		}

		Race* Logic::GetRace() const
		{
			return _world->GetGame()->GetRace();
		}

		NetGame* Logic::GetNet() const
		{
			return _world->GetGame()->netGame();
		}

		px::Scene* Logic::GetPxScene() const
		{
			return _world->GetPxScene();
		}

		snd::Engine* Logic::GetAudio() const
		{
			return _world->GetAudio();
		}

		LogicBehaviors& Logic::GetBehaviors() const
		{
			return *_behaviors;
		}
	}
