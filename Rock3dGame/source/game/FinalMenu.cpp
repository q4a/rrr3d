...
namespace r3d
{
	namespace game
	{
		FinalMenu::FinalMenu(Menu* menu, gui::Widget* parent): MenuFrame(menu, parent), _time(0), _linesSizeY(0)
		{
			...
			constexpr StringValue strMenuItems[cMenuItemEnd] = {svBack};

			for (int i = 0; i < cMenuItemEnd; ++i)
				_menuItems[i] = menu->CreateMenuButton(strMenuItems[i], "Header", "GUI\\buttonBg2.png",
				                                       "GUI\\buttonBgSel2.png", root(), this, IdentityVec2,
				                                       gui::Button::bsSelAnim, color1);

			_linesRoot = menu->CreateDummy(root(), nullptr);
		}
		...
		FinalMenu::~FinalMenu()
		{
			...
			for (const auto& _menuItem : _menuItems)
				menu()->ReleaseWidget(_menuItem);
		}

		void FinalMenu::AddLineBox(const string& caption, const string& text)
		{
			LineBox line{};

			line.caption = menu()->CreateLabel(caption, _linesRoot, "Small", NullVec2, gui::Text::haCenter,
			                                   gui::Text::vaTop, 0xFFDC0000);
			line.caption->SetAlign(gui::Widget::waTop);
			line.caption->SetWordWrap(true);

			line.text = menu()->CreateLabel(text, line.caption, "Small", NullVec2, gui::Text::haCenter,
			                                gui::Text::vaTop, 0xFFFFD6CD);
		...
		void FinalMenu::AddSlide(const string& image)
		{
			Slide slide{};
		...
		void FinalMenu::OnShow(bool value)
		{
			...
			Menu::NavElement elements[] = {
				{_menuItems[miOk], {nullptr, nullptr, nullptr, nullptr}, {vkBack, cVirtualKeyEnd}}
			};
		...
		{
			for (int i = 0; i < cMenuItemEnd; ++i)
				_menuItems[i]->SetPos(D3DXVECTOR2(_menuItems[i]->GetSize().x / 2,
				                                  vpSize.y - 60.0f + i * (_menuItems[i]->GetSize().y + 10.0f)));

			for (unsigned i = 0; i < _slides.size(); ++i)
			{
				_slides[i].plane->SetPos(D3DXVECTOR2((vpSize.x - 400.0f) / 2.0f, vpSize.y / 2));
				_slides[i].plane->SetSize(menu()->StretchImage(_slides[i].plane->GetMaterial(),
				                                               D3DXVECTOR2(vpSize.x - 500.0f, vpSize.y - 300.0f), true,
				                                               false));
			}
			...
			for (unsigned i = 0; i < _lineBoxes.size(); ++i)
			{
				...
				_linesSizeY += _lineBoxes[i].caption->GetTextAABB().GetSize().y + _lineBoxes[i].text->GetTextAABB().
					GetSize().y + 40.0f + 10.0f;
			}
		}
		...
		void FinalMenu::OnInvalidate()
		{
			...
			const string credits = GetString(svCredits);
			StringVec creditsVec;
			StrExtractValues(credits, creditsVec, "\n\n");

			for (unsigned i = 0; i < creditsVec.size(); ++i)
			{
				string credits = creditsVec[i];
				const int pos = credits.find('\n');

				AddLineBox(pos >= 0 ? credits.substr(0, pos) : credits,
				           pos >= 0 ? credits.substr(pos + 1, credits.size() - pos - 1) : "");
			}
		...
		{
			constexpr float cDuration = 107.0f;

			const D3DXVECTOR2 vpSize = uiRoot()->GetVPSize();

			_time += deltaTime;
			const float alpha = ClampValue(_time / cDuration, 0.0f, 1.0f);

			_linesRoot->SetPos(D3DXVECTOR2(vpSize.x - 250.0f, vpSize.y - alpha * (_linesSizeY + vpSize.y)));

			for (unsigned i = 0; i < _slides.size(); ++i)
			{
				const float alpha1 = i / static_cast<float>(_slides.size());
				const float alpha2 = (i + 1) / static_cast<float>(_slides.size());
				float duration = (alpha2 - alpha1) * cDuration;
				const float time = ClampValue((alpha - alpha1) * cDuration, 0.0f, duration);
				const float mAlpha = ClampValue(time / 1.0f, 0.0f, 1.0f) - ClampValue(
					(time + 0.0f - duration) / 1.0f, 0.0f, 1.0f);
