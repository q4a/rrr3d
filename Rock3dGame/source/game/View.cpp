...
namespace r3d
{
	namespace game
	{
		...
		void View::Reset(const Desc& desc2)
		{
			...
			_desc = desc2;
			Point resolution = _desc.resolution;
			...
			if (resolution.x == 0 && resolution.y == 0)
			{
				...
				if (_world->GetGraph()->FindNearMode(
					Point(graph::RenderDriver::cOptimalResWidth, graph::RenderDriver::cOptimalResHeight), dispMode,
					screenMode.height != 0 ? screenMode.width / static_cast<float>(screenMode.height) : 0.0f))
				{
			...
			{
				graph::DisplayMode screenMode = _world->GetGraph()->GetScreenMode();
				resolution = Point(screenMode.width, screenMode.height);
				fullscreen = false;
			}
			else if (fullscreen)
			{
				...
				if (_world->GetGraph()->FindNearMode(resolution, mode))
					_desc.resolution = resolution = Point(mode.width, mode.height);
			}

			if (_world->GetEdit() == nullptr)
			{
				SetWindowLong(_desc.handle, GWL_STYLE,
				              _desc.fullscreen ? WS_POPUP | WS_VISIBLE : WS_OVERLAPPEDWINDOW | WS_VISIBLE);
		...
		bool View::OnMouseMoveEvent(const Point& coord, bool shift, bool ctrl)
		{
			...
			Point newCoord = ScreenToView(coord);
		...
		void View::OnKeyChar(unsigned key, KeyState state, bool repeat)
		{
			if (!_world->GetGraph())
				return;
		...
		D3DXVECTOR2 View::GetVPSize() const
		{
			...
			return D3DXVECTOR2(static_cast<float>(engine.GetParams().BackBufferWidth),
			                   static_cast<float>(engine.GetParams().BackBufferHeight));
		}

		Point View::ScreenToView(const Point& point)
		{
			Point wndRc = GetWndSize();
			const D3DPRESENT_PARAMETERS& params = _world->GetGraph()->GetEngine().GetParams();
			auto wndSize = D3DXVECTOR2(static_cast<float>(wndRc.x), static_cast<float>(wndRc.y));

			D3DXVECTOR2 viewSize = GetVPSize();

			return Point(static_cast<int>(Round(point.x * viewSize.x / wndSize.x)),
			             static_cast<int>(Round(point.y * viewSize.y / wndSize.y)));
		}

		D3DXVECTOR2 View::ViewToProj(const Point& point)
		{
			return graph::CameraCI::ViewToProj(D3DXVECTOR2(static_cast<float>(point.x), static_cast<float>(point.y)),
			                                   GetVPSize());
		}
		...
		Point View::GetMousePos() const
		{
			return _mMove.coord;
		}

		void View::SetWindowSize(HWND handle, const Point& size, bool fullScreen)
		{
			Point wndSize = size;
			...
			SetWindowPos(handle, nullptr, 0, 0, wndSize.x, wndSize.y, 0);
		}
	}
}
