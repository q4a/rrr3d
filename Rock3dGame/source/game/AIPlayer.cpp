...
namespace r3d
{
	namespace game
	{
		AIPlayer::AIPlayer(Player* player): _player(player), _car(nullptr)
		{
			LSL_ASSERT(_player);
		...
		void AIPlayer::OnProgress(float deltaTime) const
		{
			if (_car)
		...
		void AIPlayer::FreeCar()
		{
			if (_car && _car->GetCar().mapObj)
			{
				...
				//Блокируем машину, чтобы не было бага вращения, после финиша:
				_car->GetCar().gameObj->SetSteerWheelAngle(0.0f);
				_car->GetCar().gameObj->SetSteerWheel(GameCar::smManual);
				_car->GetCar().gameObj->LockClutch(0.0f);
				_car->GetCar().gameObj->SetDamageStop(false);
			}

			SafeDelete(_car);
		}

		World* AIPlayer::GetWorld() const
		{
		...
		AICar* AIPlayer::GetCar() const
		{
			return _car;
		}


		AISystem::AISystem(Race* race): _race(race), _aiDebug(nullptr)
		{
		}
		...
		template <class Iter>
		void AISystem::LockChainTrack(const Iter& iter1, const Iter& iter2, AICar* ignoreCar, unsigned track)
		{
		...
		void AISystem::ComputeTracks(float deltaTime)
		{
			...
			using CarStore = List<Link*>;
			using CarList = Container<Link*>;
			using ChainList = List<Chain*>;

			CarStore carStore;
			CarList carList;
			ChainList chainList;
			const unsigned cTrackCnt = _race->GetMap()->GetTrace().cTrackCnt;
			if (_aiDebug)
				_aiDebug->chainList.clear();

			//Заполняем список
			for (const auto iter : _playerList)
			{
				if (iter->GetCar() && iter->GetCar()->GetCar().mapObj && iter->GetCar()->GetCar().curTile)
				{
					...
					auto link = new Link(iter->GetCar(), Line2DistToPoint(dirLine, car.pos), nullptr);
			...
			for (CarList::Position pos = carList.First(); Link** iter = carList.Current(pos); carList.Next(pos))
			{
				...
				const WayNode* curNode = car.curNode;
				...
				for (CarList::Position tPos = carList.First(); Link** iter = carList.Current(tPos); carList.Next(tPos))
				{
					...
					const float normDist = Line2DistToPoint(car.trackNormLine, tCar.pos);
					const bool lowRange = normDist + tCar.radius + car.radius > 0;
					const bool heighRange = normDist - tCar.radius - car.radius < 0;
			...
			//Обрабатываем цепи
			for (const auto tChain : chainList)
			{
				//LSL_ASSERT(!tChain->empty() && tChain->size() <= cTrackCnt);
				LSL_ASSERT(!tChain->empty());


				struct Pred : std::binary_function<Link*, Link*, bool>
				{
					bool operator()(Link* link1, Link* link2) const
					{
				...
				AIDebug::Chain* debChain = nullptr;
				if (_aiDebug)
				{
				...
				unsigned i = 0;
				//
				const AICar* lastAICar = nullptr;
				unsigned lastSTrack = 0;
				for (auto tIter = tChain->begin(); tIter != tChain->end(); ++tIter, ++sTrack, ++i)
				{
					//
					AICar* tAICar = (*tIter)->car;
					auto& tCar = const_cast<AICar::CarState&>(tAICar->GetCar());

					//максимально допустимая дорожка с учетом требуемых для оставшихся целей мест			
					sTrack = ClampValue(tCar.track, std::min(sTrack, cTrackCnt - 1),
					                    cTrackCnt - (std::min(tCount, cTrackCnt) - i % cTrackCnt));
			...
			for (const auto& iter : chainList)
				delete iter;
			for (const auto& iter : carStore)
				delete iter;
		}
		...
		AIPlayer* AISystem::FindAIPlayer(Player* player) const
		{
			for (const auto& iter : _playerList)
		...
		{
			constexpr int nPointSize = 9;
			//char strFontName[] = "Arial";
			const HDC hDC = GetDC(nullptr);
			const int nHeight = -MulDiv(nPointSize, GetDeviceCaps(hDC, LOGPIXELSY), 72);
			ReleaseDC(nullptr, hDC);
			// Create a font for statistics and help output
			HRESULT hr = D3DXCreateFont(_debug->GetWorld()->GetGraph()->GetEngine().GetDriver().GetDevice(), nHeight, 0,
			                            FW_BOLD, 0, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, DEFAULT_QUALITY,
			                            DEFAULT_PITCH | FF_DONTCARE, TEXT("Arial"), &_font);
		}
		...
		void AIDebug::GrActor::DoRender(graph::Engine& engine)
		{
			...
			if (_debug->_aiPlayer->GetPlayer()->GetCar().mapObj)
			{
				...
				if (car && nextNode)
				{
					...
					float dist = Line2DistToPoint(nextNode->GetTile().GetEdgeLine(),
					                              _debug->_aiPlayer->_car->GetCar().pos);
			...
			sstream << "Chains \n";
			unsigned i = 0;
			for (auto iter = _debug->chainList.begin(); iter != _debug->chainList.end(); ++iter, ++i)
			{
				sstream << "Chain " << i << " = ";

				const Chain& tChain = *iter;
				for (unsigned int tIter : tChain)
				{
					sstream << tIter << " ";
				}
				sstream << '\n';
			...
			if (dbgDumpTime > 0.5f)
			{
				...
				sstream << "nReac=" << Round(normReaction * 100.0f) / 100.0f << "\nnReacMin=" <<
					Round(minNormReaction * 100.0f) / 100.0f << "\nnReacMax=" << Round(maxNormReaction * 100.0f) /
					100.0f;
			...
			CarWheel* wheel = &gameObj->GetWheels().front();
			...
			int ind = 0;
			for (CarWheels::const_iterator iter = gameObj->GetWheels().begin(); iter != gameObj->GetWheels().end(); ++
			     iter, ++ind)
				sstream2 << "wheel" << ind << " lat=" << Round((*iter)->GetLatSlip() * 10.0f) / 10.0f << " long=" <<
					Round((*iter)->GetLongSlip() * 10.0f) / 10.0f << '\n';
			sstream2 << '\n';
			...
			_font->OnResetDevice();
			_font->DrawText(nullptr, sstream.str().c_str(), -1, &destRect, DT_NOCLIP,
			                D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f));
			_font->DrawText(nullptr, sstream2.str().c_str(), -1, &destRect2, DT_NOCLIP,
			                D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f));
			_font->OnLostDevice();

			constexpr int cParamKeysEnd = 10;
			const int paramKeys[cParamKeysEnd] = {
			...
			for (auto iter : gameObj->GetWheels())
			{
				CarWheel* wheel;
				wheel = iter;
		...
		void AIDebug::OnProgress(float deltaTime) const
		{
			_traceGfx->SetSelNode(_aiPlayer->GetPlayer()->GetCar().curNode);
		}

		World* AIDebug::GetWorld() const
		{
			return _aiPlayer->GetWorld();
		}

		AIPlayer* AIDebug::GetAI() const
		{
			return _aiPlayer;
		}
	}
