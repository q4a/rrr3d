		...
		void GameObject::Damage(int senderPlayerId, float value, float newLife, bool death, DamageType damageType)
		{
			...
			if (_liveState != lsDeath)
			{
				...
				if (death)
				{
					...
					{
						//attackbonus указываем здесь сумму атакбонусов.
						if (GetMapObj()->GetPlayer() != nullptr)
						{
							...
							//если жертва не агрессор
							if (victim->GetId() != plr->GetId())
							{
								//для самонаводок и фрострея награда 100$
								...
								//для обычного оружия, а также тарана награда 500$
								...
								//для необычного оружия награда 750$
								...
								//для остального нет награды, но в статистику фраг добавим.
							...
							{
								//SendEvent(cPlayerSuicide, senderPlayerId, &data); //massmod идея на будущее! новое игровое событие и фраза для коментатора.
							}
						}
					}
