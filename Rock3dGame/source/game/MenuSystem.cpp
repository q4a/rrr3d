...
SteamService* MenuFrame::steamService() const
{
	return _menu->GetSteamService();
		...
		void MenuFrame::SetPos(const D3DXVECTOR2& pos, gui::Widget::Anchor align, const D3DXVECTOR2& size)
		{
			...
			pos2.x = ClampValue(pos2.x, size.x / 2 + 15.0f, root()->GetManager().GetVPSize().x - size.x / 2 - 15.0f);
			pos2.y = ClampValue(pos2.y, size.y / 2 + 15.0f, root()->GetManager().GetVPSize().y - size.y / 2 - 15.0f);
