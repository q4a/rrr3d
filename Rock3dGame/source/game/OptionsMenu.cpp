...
namespace r3d
{
	namespace game
	{
		namespace n
		{
			GameFrame::GameFrame(Menu* menu, OptionsMenu* optionsMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_optionsMenu(optionsMenu), _gridScroll(0)
			{
				...
				const StringValue cMMStyleStr[GameMode::cMMStyleEnd] = {
				...
				StringList mMapStr;
				for (int i = 0; i < GameMode::cMMStyleEnd; ++i)
					mMapStr.push_back(GetString(cMMStyleStr[i]));

				StringList SHudStr;
				for (int i = 0; i < GameMode::cHUDStyleEnd; ++i)
					SHudStr.push_back(GetString(cHudStyleStr[i]));

				StringList upgradeMaxLevel;
				for (int i = 0; i < Garage::cUpgCntLevel; ++i)
					upgradeMaxLevel.push_back(StrFmt("%d", i + 1));

				StringList SelectedLevel;
				for (int i = 0; i <= Garage::cMaxLevel; ++i)
					SelectedLevel.push_back(StrFmt("%d", i));

				StringList numPlayersStr;
				for (int i = 2; i <= Race::cMaxPlayers; ++i)
					numPlayersStr.push_back(StrFmt("%d", i));

				StringList numComputersStr;
				for (int i = 0; i <= Race::cMaxPlayers - 1; ++i)
					numComputersStr.push_back(StrFmt("%d", i));

				StringList lapsCountStr;
				for (int i = 1; i <= 8; ++i)
					lapsCountStr.push_back(StrFmt("%d", i));
				...
				gui::Text::HorAlign horLabels[cLabelEnd] = {
					...
					gui::Text::haLeft, gui::Text::haLeft, gui::Text::haLeft
				};
				...
				{
					_itemsBg[i] = menu->CreatePlane(_grid, nullptr, "GUI\\labelBg1.png", true, IdentityVec2,
					                                gui::Material::bmTransparency);
					_itemsBg[i]->SetAlign(gui::Widget::waLeft);
				}

				for (int i = 0; i < cLabelEnd; ++i)
					_labels[i] = menu->CreateLabel(GetString(strLabels[i]), _itemsBg[i], fontLabels[i], NullVec2,
					                               horLabels[i], vertLabels[i], colorLabels[i]);
			...
			void GameFrame::AdjustGrid(const D3DXVECTOR2& vpSize)
			{
				...
				for (auto iter = _grid->GetChildren().begin(); iter != _grid->GetChildren().end(); ++iter)
				{
					...
					if (i >= _gridScroll * cellsOnLine && i < _gridScroll * cellsOnLine + cellsGrid)
					{
						...
						for (auto iter2 = (*iter)->GetChildren().begin(); iter2 != (*iter)->GetChildren().end(); ++
						     iter2)
						{
							if ((*iter2)->GetTag() == 1)
							{
								gui::Material* fon = static_cast<gui::StepperBox*>(*iter2)->GetArrow();
								(*iter2)->SetEnabled(true && (fon == nullptr || fon->GetAlpha() == 1.0f));
							}
							else
				...
				menu()->SetButtonEnabled(
					_downArrow, _gridScroll < std::max(
						static_cast<int>(ceil(i / (float)cellsOnLine)) - cellsGrid / cellsOnLine, 0));
			}
			...
			void GameFrame::LoadCfg()
			{
				...
				_steppers[dbStyleHUD]->SetSelIndex(menu()->StyleHUD());
				_steppers[dbMinimapStyle]->SetSelIndex(menu()->MinimapStyle());
				_steppers[dbLapsCount]->SetSelIndex(menu()->lapsCount() - 1);
				_steppers[dbUpgradeMaxLevel]->SetSelIndex(menu()->GetUpgradeMaxLevel());
				_steppers[dbSelectedLevel]->SetSelIndex(menu()->GetSelectedLevel());
			...
			void GameFrame::OnShow(bool value)
			{
				...
				{
					_gridScroll = 0;
					_optionsMenu->RegNavElements(_steppers, cStepperEnd, nullptr, 0, nullptr, 0, _upArrow, _downArrow);
				}
			}
			...
			bool GameFrame::ApplyChanges()
			{
				for (int i = 0; i < cStepperEnd; ++i)
				{
					...
					case dbSelectedLevel:
						menu()->SetSelectedLevel(val);
						break;
					...
					case dbWeaponUpgrades:
						menu()->weaponUpgrades(val == 1);
						break;
			...
			CameraFrame::CameraFrame(Menu* menu, OptionsMenu* optionsMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _optionsMenu(optionsMenu), _gridScroll(0)
			{
				D3DXCOLOR color1(0xffafafaf);
				...
				for (int i = 0; i < GameMode::cFovLevelEnd; ++i)
					FOVStr.push_back(GetString(cFovLevelStr[i]));

				StringList SubjectStr;
				for (int i = 0; i < GameMode::cSubjectLevelEnd; ++i)
					SubjectStr.push_back(GetString(cSubjectLevelStr[i]));
				...
				for (int i = 0; i < cLabelEnd; ++i)
					_labels[i] = menu->CreateLabel(GetString(strLabels[i]), _itemsBg[i], fontLabels[i],
					                               NullVec2, horLabels[i], vertLabels[i], colorLabels[i]);
				...
				_downArrow->SetRot(-D3DX_PI / 2);
				D3DXVECTOR2 size = _downArrow->GetSize();
				_downArrow->SetSize(menu->StretchImage(*_downArrow->GetFon(), D3DXVECTOR2(30.0f, 30.0f), true,
				                                       true, true, false));
				_downArrow->SetSelSize(
					menu->GetImageSize(*_downArrow->GetSel()) * _downArrow->GetSize().x / size.x);

				_upArrow = menu->CreateArrow(root(), this);
				_upArrow->SetRot(D3DX_PI / 2);
				size = _upArrow->GetSize();
				_upArrow->SetSize(menu->StretchImage(*_upArrow->GetFon(), D3DXVECTOR2(30.0f, 30.0f), true, true,
				                                     true, false));
				_upArrow->SetSelSize(menu->GetImageSize(*_upArrow->GetSel()) * _upArrow->GetSize().x / size.x);
			...
			void CameraFrame::AdjustGrid(const D3DXVECTOR2& vpSize)
			{
				_grid->SetPos(-235.0f, -178.0f);
				_grid->cellSize(D3DXVECTOR2(0, 50.0f));
				...
				menu()->SetButtonEnabled(_upArrow, _gridScroll > 0);
				menu()->SetButtonEnabled(_downArrow, _gridScroll < std::max(
					                         static_cast<int>(ceil(i / (float)cellsOnLine)) - cellsGrid /
					                         cellsOnLine, 0));
			}

			void CameraFrame::ScrollGrid(int step)
			{
				_gridScroll += step;
				AdjustGrid(menu()->GetGUI()->GetVPSize());
			}
			...
			void CameraFrame::LoadCfg()
			{
				...
				_steppers[dbCamFov]->SetSelIndex(menu()->camFov());
				_steppers[dbCamProection]->SetSelIndex(menu()->camProection());
				_steppers[dbAutoCamera]->SetSelIndex(menu()->autoCamera());
				_steppers[dbSubjectView]->SetSelIndex(menu()->subjectView());
			}

			void CameraFrame::OnShow(bool value)
			{
				if (value)
				{
					_gridScroll = 0;
					_optionsMenu->RegNavElements(_steppers, cStepperEnd, nullptr, 0, nullptr, 0, _upArrow,
					                             _downArrow);
				}
			}

			void CameraFrame::OnAdjustLayout(const D3DXVECTOR2& vpSize)
			{
				AdjustGrid(vpSize);
			...
			void CameraFrame::OnInvalidate()
			{
			}

			bool CameraFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				if (sender == _downArrow)
				{
					ScrollGrid(1);
			...
			bool CameraFrame::ApplyChanges()
			{
				for (int i = 0; i < cStepperEnd; ++i)
				{
					...
					case dbCamProection:
						menu()->camProection(val == 1);
						break;

					case dbAutoCamera:
						menu()->autoCamera(val == 1);
						break;

					case dbSubjectView:
						menu()->subjectView(val);
						break;
					}
				}
			...
			void CameraFrame::CancelChanges()
			{
			}


			MediaFrame::MediaFrame(Menu* menu, OptionsMenu* optionsMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _optionsMenu(optionsMenu)
			{
				StringValue gfxLevel[] = {svLow, svMiddle, svHigh, svUltra};
				StringValue postLevel[] = {svOff, svOn};
				std::string msLevel[] = {"off", "2x", "4x", "6x", "8x", "16x"};
				std::string aaLevel[] = {"off", "2x", "4x", "8x"};

				StringList shadowLevel;
				for (unsigned i = 0; i < ARRAY_LENGTH(gfxLevel); ++i)
					if (menu->GetEnv()->IsShadowQualitySupported(static_cast<Environment::Quality>(i)))
						shadowLevel.push_back(GetString(gfxLevel[i]));

				StringList lightLevel;
				for (unsigned i = 0; i < ARRAY_LENGTH(gfxLevel); ++i)
					if (menu->GetEnv()->IsLightQualitySupported(static_cast<Environment::Quality>(i)))
						lightLevel.push_back(GetString(gfxLevel[i]));

				StringList postEffLevel;
				for (unsigned i = 0; i < ARRAY_LENGTH(postLevel); ++i)
					if (menu->GetEnv()->IsPostEffQualitySupported(static_cast<Environment::Quality>(i)))
						postEffLevel.push_back(GetString(postLevel[i]));

				StringList envLevel;
				for (unsigned i = 0; i < ARRAY_LENGTH(gfxLevel) - 1; ++i)
					if (menu->GetEnv()->IsEnvQualitySupported(static_cast<Environment::Quality>(i)))
				...
				for (int i = 0; i < ARRAY_LENGTH(msLevel); ++i)
					if (menu->GetEnv()->IsFilteringSupported(static_cast<Environment::Filtering>(i)))
						fltLevel.push_back(msLevel[i]);

				StringList msFltLevel;
				for (int i = 0; i < ARRAY_LENGTH(aaLevel); ++i)
					if (menu->GetEnv()->IsMultisamplingSupported(static_cast<Environment::Multisampling>(i)))
						msFltLevel.push_back(aaLevel[i]);

				StringList resolutionStr;
				graph::DisplayModes modes = menu->GetDisplayModes();
				for (unsigned i = 0; i < modes.size(); ++i)
					resolutionStr.push_back(StrFmt("%d x %d", modes[i].width, modes[i].height));

				StringList itemsStepper[cStepperEnd] = {
					resolutionStr, fltLevel, msFltLevel, shadowLevel, envLevel, lightLevel, postEffLevel,
					onOffLevel
				};
				...
				string strLabels[cLabelEnd] = {
					_SC(svResolution), _SC(svFiltering), _SC(svMultisampling), _SC(svShadow), _SC(svEnv),
					_SC(svLight), _SC(svPostProcess), "svDisableVideo"
				}; //, "svWindowMode"};
				std::string fontLabels[cLabelEnd] = {
					"Small", "Small", "Small", "Small", "Small", "Small", "Small", "Small"
				}; //, "Small"};
				...
				for (int i = 0; i < cLabelEnd; ++i)
				{
					_itemsBg[i] = menu->CreatePlane(root(), nullptr, "GUI\\labelBg1.png", true, IdentityVec2,
					                                gui::Material::bmTransparency);
					_itemsBg[i]->SetAlign(gui::Widget::waLeft);
				}
				...
				for (int i = 0; i < cLabelEnd; ++i)
					_labels[i] = menu->CreateLabel(GetString(strLabels[i]), labelsParent[i], fontLabels[i],
					                               NullVec2, horLabels[i], vertLabels[i], colorLabels[i]);
			...
			void MediaFrame::OnShow(bool value)
			{
				if (value)
				{
					_optionsMenu->RegNavElements(_steppers, cStepperEnd, nullptr, 0);
				}
			}
				...
				for (int i = 0; i < cStepperEnd; ++i)
				{
					...
					case dbResolution:
						if (val >= 0 && static_cast<unsigned>(val) < menu()->GetDisplayModes().size())
						{
							...
							menu()->SetDisplayMode(Point(mode.width, mode.height));
						}
						break;

					case dbFiltering:
						menu()->GetEnv()->SetFiltering(static_cast<Environment::Filtering>(val));
						break;

					case dbMultisampling:
						menu()->GetEnv()->SetMultisampling(static_cast<Environment::Multisampling>(val));
						break;

					case dbShadow:
						menu()->GetEnv()->SetShadowQuality(static_cast<Environment::Quality>(val));
						break;

					case dbEnv:
						menu()->GetEnv()->SetEnvQuality(static_cast<Environment::Quality>(val));
						break;

					case dbLight:
						menu()->GetEnv()->SetLightQuality(static_cast<Environment::Quality>(val));
						break;

					case dbPostProcess:
						menu()->GetEnv()->SetPostEffQuality(static_cast<Environment::Quality>(val));
						break;
					...
					NetworkTab::NetworkTab(Menu* menu, OptionsMenu* optionsMenu, gui::Widget* parent):
						MenuFrame(menu, parent), _optionsMenu(optionsMenu)
					{
						D3DXCOLOR color1(0xffafafaf);

						StringList languageStr;
						for (auto iter = menu->GetGame()->GetLanguages().begin(); iter != menu->GetGame()->
						     GetLanguages().end(); ++iter)
							languageStr.push_back(iter->name);

						StringList commentatorStr;
						for (auto iter = menu->GetGame()->GetCommentatorStyles().begin(); iter != menu->
						     GetGame()->GetCommentatorStyles().end(); ++iter)
						...
						StringValue strLabels[cLabelEnd] = {
							svLanguage, svCommentator, svMusic, svSound, svSoundDicter
						};
						...
						for (int i = 0; i < cLabelEnd; ++i)
						{
							_itemsBg[i] = menu->CreatePlane(root(), nullptr, "GUI\\labelBg1.png", true,
							                                IdentityVec2, gui::Material::bmTransparency);
							_itemsBg[i]->SetAlign(gui::Widget::waLeft);
						}

						for (int i = 0; i < cLabelEnd; ++i)
							_labels[i] = menu->CreateLabel(strLabels[i], labelsParent[i], fontLabels[i],
							                               NullVec2, horLabels[i], vertLabels[i],
							                               colorLabels[i]);
					...
					void NetworkTab::LoadCfg()
					{
						...
						_steppers[dbCommentator]->SetSelIndex(
							menu()->GetGame()->FindCommentatorStyleIndex(
								menu()->GetGame()->GetCommentatorStyle()));
					}
					...
					bool NetworkTab::ApplyChanges()
					{
						...
						for (int i = 0; i < cStepperEnd; ++i)
						{
							...
							case dbLanguage:
								{
									...
									if (val != -1 && val != curVal)
									{
										menu()->GetGame()->SetLanguage(
											menu()->GetGame()->GetLanguages()[val].name);
										res |= true;
									}
									break;
								}
							...
							case dbCommentator:
								{
									if (val != -1)
										menu()->GetGame()->SetCommentatorStyle(
											menu()->GetGame()->GetCommentatorStyles()[val].name);
									break;
								}
							}
						}
					...
					ControlsFrame::ControlsFrame(Menu* menu, OptionsMenu* optionsMenu, gui::Widget* parent):
						MenuFrame(menu, parent), _optionsMenu(optionsMenu), _gridScroll(0)
					{
						D3DXCOLOR color1(0xffafafaf);
						...
						_downArrow->SetRot(-D3DX_PI / 2);
						D3DXVECTOR2 size = _downArrow->GetSize();
						_downArrow->SetSize(menu->StretchImage(*_downArrow->GetFon(), D3DXVECTOR2(30.0f, 30.0f),
						...
						_upArrow->SetRot(D3DX_PI / 2);
						size = _upArrow->GetSize();
						_upArrow->SetSize(menu->StretchImage(*_upArrow->GetFon(), D3DXVECTOR2(30.0f, 30.0f),
						                                     true, true, true, false));
						_upArrow->SetSelSize(
							menu->GetImageSize(*_upArrow->GetSel()) * _upArrow->GetSize().x / size.x);

						for (int i = 0; i < cControllerTypeEnd; ++i)
						{
							_controllerIcons[i] = menu->CreatePlane(
								root(), nullptr, StrFmt("GUI\\%s.png", cControllerTypeStr[i].c_str()), true,
								IdentityVec2, gui::Material::bmTransparency);
						}
					...
					void ControlsFrame::LoadCfg()
					{
						...
						auto iter = _boxes.begin();

						for (unsigned i = 0; i < cGameActionUserEnd; ++i)
						{
							auto action = static_cast<GameAction>(i);

							InputBox* item = iter != _boxes.end() ? &(*iter) : nullptr;
							if (item == nullptr)
							{
								InputBox newItem;
								newItem.sprite = menu()->CreatePlane(
									_grid, nullptr, "GUI\\labelBg2.png", true, IdentityVec2,
									gui::Material::bmTransparency);
								newItem.label = menu()->CreateLabel(
									"", newItem.sprite, "Small", NullVec2, gui::Text::haLeft,
									gui::Text::vaCenter, 0xffafafaf);

								for (int j = 0; j < cControllerTypeEnd; ++j)
								{
									newItem.keys[j] = menu()->CreateMenuButton(
										svNull, "Small", "GUI\\keyBg.png", "GUI\\keyBgSel.png", newItem.sprite,
										this, IdentityVec2, gui::Button::bsSimple, clrBlack, Menu::ssButton1);
							...
							for (int j = 0; j < cControllerTypeEnd; ++j)
							{
								auto controller = static_cast<ControllerType>(j);
								VirtualKey virtualKey = menu()->GetControl()->GetGameKey(controller, action);
								VirtualKeyInfo info = menu()->GetControl()->GetVirtualKeyInfo(
									controller, virtualKey);
					...
					void ControlsFrame::OnShow(bool value)
					{
						if (value)
						{
							...
							_optionsMenu->RegNavElements(nullptr, 0, nullptr, 0, &buttons.front(),
							                             buttons.size(), _upArrow, _downArrow);
						}
						else
						{
							menu()->GetControl()->RemoveEvent(this);
					...
					bool ControlsFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
					{
								...
								for (int j = 0; j < cGameActionUserEnd; ++j)
								{
									if (_boxes[j].keys[i]->IsSelected())
									{
										if (menu()->GetAcceptResultYes())
										{
											...
											_boxes[j].keys[i]->SetText(
												menu()->GetControl()->GetVirtualKeyInfo(
													static_cast<ControllerType>(i), cVirtualKeyEnd).name);
										}
...

						{
							...
							menu()->ShowAccept(true, menu()->GetString("svPressKey"),
							                   menu()->GetString("svDeleteKey"), menu()->GetString(svCancel),
							                   menu()->GetGUI()->GetVPSize() / 2, gui::Widget::waCenter, this,
						...
						if (msg.key != cVirtualKeyEnd && msg.state == ksDown)
						{
							for (int j = 0; j < cGameActionUserEnd; ++j)
							{
								if (_boxes[j].keys[msg.controller]->IsSelected())
								{
									...
									_boxes[j].keys[msg.controller]->SetText(
										menu()->GetControl()->GetVirtualKeyInfo(msg.controller, msg.key).name);
					...
					bool ControlsFrame::ApplyChanges()
					{
						for (int i = 0; i < cControllerTypeEnd; ++i)
							for (int j = 0; j < cGameActionUserEnd; ++j)
								menu()->GetControl()->SetGameKey(
									static_cast<ControllerType>(i), static_cast<GameAction>(j),
									_boxes[j].virtualKeys[i]);
						return false;
					}
					...
					OptionsMenu::OptionsMenu(Menu* menu, gui::Widget* parent): _menu(menu), _state(msGame)
					{
						...
						StringValue strStateItems[cStateEnd] = {
							svGame, svCamera, svGraphic, svNetwork, svControls
						};
						...
						_menuBgMask = _menu->CreatePlane(_root, this, "", false, IdentityVec2,
						                                 gui::Material::bmTransparency);
						_menuBgMask->GetMaterial().SetColor(D3DXCOLOR(0, 0, 0, 0.8f));
						_menuBgMask->SetAnchor(gui::Widget::waCenter);

						_menuBg = _menu->CreatePlane(_menuBgMask, this, "GUI\\optionsBg.png", true,
						                             IdentityVec2, gui::Material::bmTransparency);

						_gameFrame = new GameFrame(menu, this, _menuBg);
						_cameraFrame = new CameraFrame(menu, this, _menuBg);
					...
					void OptionsMenu::SetSelection(State state, bool instant)
					{
						for (int i = 0; i < cStateEnd; ++i)
						{
							...
							auto label = static_cast<gui::Label*>(_stateItems[i]->GetChildren().front());
					...
					void OptionsMenu::ApplyState()
					{
						...
						_cameraFrame->Show(_state == msCamera);
					...
					bool OptionsMenu::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
					{
						...
						if (sender == _menuItems[miApply])
						{
							...
							if (prevSyncFrameRate == Environment::sfrFixed && _menu->GetEnv()->
								GetSyncFrameRate() != Environment::sfrFixed)
							{
								std::string message;
								if (needReload)
								...
								_menu->ShowMessage(_menu->GetString(svWarning), message, _menu->GetString(svOk),
								                   _menu->GetGUI()->GetVPSize() / 2, gui::Widget::waCenter,
								                   0.0f, this);
							}
							else if (needReload)
							{
								_menu->ShowMessage(_menu->GetString(svWarning),
								                   _menu->GetString(svHintNeedReload), _menu->GetString(svOk),
								                   _menu->GetGUI()->GetVPSize() / 2, gui::Widget::waCenter,
								                   0.0f, this);
							}
							else
						...
						for (int i = 0; i < cStateEnd; ++i)
						{
							...
							{
								SetState(static_cast<State>(i));
								return true;
							}

							if (sender->GetData() != nullptr)
							{
								auto button = lsl::StaticCast<gui::Button*>(sender->GetData());
					...
					bool OptionsMenu::OnMouseEnter(gui::Widget* sender, const gui::MouseMove& mMove)
					{
						if (sender->GetData() != nullptr)
						{
							...
							auto label = static_cast<gui::Label*>(button->GetChildren().front());
							label->GetMaterial().SetColor(0xffafafaf);
						}
					...
					void OptionsMenu::RegSteppers(Vector<_T2>& navElements, _T1 steppers[], int stepperCount,
					                              gui::Widget* upWidgetLeft, gui::Widget* upWidgetRight,
					                              gui::Widget* downWidgetLeft, gui::Widget* downWidgetRight)
					{
						for (int i = 0; i < stepperCount; ++i)
						{
							if (i >= 2)
							{
								Menu::NavElement navElement = {
									steppers[i - 1]->GetLeft(),
									{
										steppers[i - 1]->GetRight(), steppers[i - 1]->GetRight(),
										steppers[i - 2]->GetLeft(), steppers[i]->GetLeft()
									},
								...
								Menu::NavElement navElement2 = {
									steppers[i - 1]->GetRight(),
									{
										steppers[i - 1]->GetLeft(), steppers[i - 1]->GetLeft(),
										steppers[i - 2]->GetRight(), steppers[i]->GetRight()
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
							...
							if (i + 1 == stepperCount)
							{
								if (stepperCount >= 2)
								{
									Menu::NavElement navElement = {
										steppers[0]->GetLeft(),
										{
											steppers[0]->GetRight(), steppers[0]->GetRight(), upWidgetLeft,
											steppers[1]->GetLeft()
										},
										{cVirtualKeyEnd, cVirtualKeyEnd}
									};
									...
									Menu::NavElement navElement2 = {
										steppers[0]->GetRight(),
										{
											steppers[0]->GetLeft(), steppers[0]->GetLeft(), upWidgetRight,
											steppers[1]->GetRight()
										},
										{cVirtualKeyEnd, cVirtualKeyEnd}
									};
								...
								{
									Menu::NavElement navElement = {
										steppers[stepperCount - 1]->GetLeft(),
										{
											steppers[stepperCount - 1]->GetRight(),
											steppers[stepperCount - 1]->GetRight(),
											stepperCount >= 2
												? steppers[stepperCount - 2]->GetLeft()
												: upWidgetLeft,
											downWidgetLeft
										},
										{cVirtualKeyEnd, cVirtualKeyEnd}
									...
									Menu::NavElement navElement2 = {
										steppers[stepperCount - 1]->GetRight(),
										{
											...
											steppers[stepperCount - 1]->GetLeft(),
											stepperCount >= 2
												? steppers[stepperCount - 2]->GetRight()
												: upWidgetRight,
											downWidgetRight
										},
										{cVirtualKeyEnd, cVirtualKeyEnd}
					...
					void OptionsMenu::RegNavElements(gui::StepperBox* steppers[], int stepperCount,
					                                 gui::VolumeBar* volumes[], int volumeCount,
					                                 ButtonLR buttons[], int buttonsCount, gui::Widget* upArrow,
					                                 gui::Widget* downArrow)
					{
						Vector<Menu::NavElement> navElements;

						gui::Widget* firstStepperLeft = nullptr;
						gui::Widget* firstStepperRight = nullptr;
						gui::Widget* lastStepperLeft = nullptr;
						gui::Widget* lastStepperRight = nullptr;

						if (buttons)
						{
							RegSteppers(navElements, buttons, buttonsCount, upArrow, upArrow, downArrow,
							            downArrow);

							{
								Menu::NavElement navElement = {
									upArrow, {nullptr, nullptr, _menuItems[miBack], buttons[0].GetLeft()},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
								navElements.push_back(navElement);
							}
							{
								Menu::NavElement navElement = {
									downArrow,
									{nullptr, nullptr, buttons[buttonsCount - 1].GetLeft(), _menuItems[miBack]},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
								navElements.push_back(navElement);
							}
						...
						else
						{
							...
							if (steppers)
								RegSteppers(navElements, steppers, stepperCount, upElement, upElement,
								            volumes ? volumes[0]->GetLeft() : downElement,
								            volumes ? volumes[0]->GetRight() : downElement);
							if (volumes)
								RegSteppers(navElements, volumes, volumeCount,
								            steppers ? steppers[stepperCount - 1]->GetLeft() : upElement,
								            steppers ? steppers[stepperCount - 1]->GetRight() : upElement,
								            downElement, downElement);

							firstStepperLeft = steppers ? steppers[0]->GetLeft() : nullptr;
							firstStepperRight = steppers ? steppers[0]->GetRight() : nullptr;
							lastStepperLeft = volumes
								                  ? volumes[volumeCount - 1]->GetLeft()
								                  : (steppers
									                     ? steppers[stepperCount - 1]->GetLeft()
									                     : nullptr);
							lastStepperRight = volumes
								                   ? volumes[volumeCount - 1]->GetRight()
								                   : (steppers
									                      ? steppers[stepperCount - 1]->GetRight()
									                      : nullptr);
							...
							{
								Menu::NavElement navElement = {
									upArrow, {nullptr, nullptr, _menuItems[miBack], firstStepperLeft},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
								navElements.push_back(navElement);
							}
							...
							{
								Menu::NavElement navElement = {
									downArrow, {nullptr, nullptr, lastStepperLeft, _menuItems[miBack]},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
								navElements.push_back(navElement);
							}
						...
						for (int i = 0; i < cStateEnd; ++i)
						{
							if (i >= 2)
							{
								Menu::NavElement navElement = {
									_stateItems[i - 1],
									{
										_menuItems[miApply], _menuItems[miBack], _stateItems[i - 2],
										_stateItems[i]
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
							...
							if (i + 1 == cStateEnd)
							{
								if (cStateEnd >= 2)
								{
									Menu::NavElement navElement = {
										_stateItems[0],
										{
											_menuItems[miApply], _menuItems[miBack], _stateItems[cStateEnd - 1],
											_stateItems[1]
										},
										{cVirtualKeyEnd, cVirtualKeyEnd}
									};
								...
								Menu::NavElement navElement2 = {
									_stateItems[cStateEnd - 1],
									{
										_menuItems[miApply], _menuItems[miBack],
										cStateEnd >= 2 ? _stateItems[cStateEnd - 2] : nullptr, _stateItems[0]
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
								navElements.push_back(navElement2);
...

						{
							Menu::NavElement navElement = {
								_menuItems[miBack],
								{_stateItems[0], _menuItems[miApply], lastStepperLeft, firstStepperLeft},
						...
						}
						{
							Menu::NavElement navElement = {
								_menuItems[miApply],
								{_menuItems[miBack], _stateItems[0], lastStepperLeft, firstStepperLeft},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement);
						}
					...
					gui::Button* OptionsMenu::CreateMenuButton(StringValue name, gui::Widget* parent,
					                                           gui::Widget::Event* guiEvent)
					{
						return _menu->CreateMenuButton2(name, parent, guiEvent);
					}

					gui::Button* OptionsMenu::CreateMenuButton2(StringValue name, gui::Widget* parent,
					                                            gui::Widget::Event* guiEvent)
					{
						gui::Button* button = _menu->CreateMenuButton(
							svNull, "", "GUI\\buttonBg4.png", "GUI\\buttonBgSel4.png", parent, guiEvent,
							IdentityVec2, gui::Button::bsSelAnim, clrWhite, Menu::ssButton1);
						button->SetAlign(gui::Widget::waLeft);

						gui::Label* label = _menu->CreateLabel(name, button, "Header", NullVec2,
						                                       gui::Text::haLeft, gui::Text::vaCenter,
						                                       0xffafafaf);
					...
					{
						return _menu->CreatePlane(parent, guiEvent, "GUI\\labelBg1.png", true, IdentityVec2,
						                          gui::Material::bmTransparency);
					}

					gui::StepperBox* OptionsMenu::CreateStepper(const StringList& items, gui::Widget* parent,
					                                            gui::Widget::Event* guiEvent)
					{
						return _menu->CreateStepper(items, parent, guiEvent);
					}

					gui::VolumeBar* OptionsMenu::CreateVolumeBar(
						gui::Widget* parent, gui::Widget::Event* guiEvent)
					{
						...
						volumeBar->GetOrCreateArrow()->GetSampler().
						           SetTex(_menu->GetTexture("GUI\\arrow3.png"));
						volumeBar->GetOrCreateArrow()->SetBlending(gui::Material::bmTransparency);

						volumeBar->GetOrCreateArrowSel()->GetSampler().SetTex(
							_menu->GetTexture("GUI\\arrowSel3.png"));
					...
					void OptionsMenu::AdjustLayout(const D3DXVECTOR2& vpSize)
					{
						...
						for (int i = 0; i < cStateEnd; ++i)
							_stateItems[i]->SetPos(-558.0f, -150.0f + i * 85.0f);

						_gameFrame->AdjustLayout(vpSize);
						_cameraFrame->AdjustLayout(vpSize);
					...
					StartOptionsMenu::StartOptionsMenu(Menu* menu, gui::Widget* parent):
						MenuFrame(menu, parent), _lastCameraIndex(-1)
					{
						D3DXCOLOR color1(0xffafafaf);
						StringValue strMenuItems[cMenuItemEnd] = {svApply};

						const StringValue cPrefCameraStr[GameMode::cPrefCameraEnd] = {
							svCameraSecView, svCameraOrtho
						};
						...
						StringList itemsStepper[cStepperEnd] = {
							cameraStr, resolutionStr, languageStr, commentatorStr
						};

						StringValue strLabels[cLabelEnd] = {
							svCamera, svResolution, svLanguage, svCommentator, svStartOptionsInfo
						};
						std::string fontLabels[cLabelEnd] = {"Small", "Small", "Small", "Small", "Small"};
						gui::Text::HorAlign horLabels[cLabelEnd] = {
							gui::Text::haLeft, gui::Text::haLeft, gui::Text::haLeft, gui::Text::haLeft,
						...
						_menuBgMask = menu->CreatePlane(root(), this, "", false, IdentityVec2,
						                                gui::Material::bmTransparency);
						_menuBgMask->GetMaterial().SetColor(D3DXCOLOR(0, 0, 0, 0.8f));
						_menuBgMask->SetAnchor(gui::Widget::waCenter);

						_menuBg = menu->CreatePlane(_menuBgMask, this, "GUI\\startMenuBg.png", true,
						                            IdentityVec2, gui::Material::bmTransparency);
						...
						for (int i = 0; i < cStepperEnd; ++i)
						{
							_itemsBg[i] = menu->CreatePlane(_menuBg, nullptr, "GUI\\labelBg1.png", true,
							                                IdentityVec2, gui::Material::bmTransparency);
							_itemsBg[i]->SetAlign(gui::Widget::waLeft);
						}
					...
					void StartOptionsMenu::OnShow(bool value)
					{
						if (value)
						{
							...
							for (int i = 0; i < cStepperEnd; ++i)
							{
								...
								{
									Menu::NavElement navElement = {
										_steppers[i - 1]->GetLeft(),
										{
											nullptr, _steppers[i - 1]->GetRight(), _steppers[i - 2]->GetLeft(),
											_steppers[i]->GetLeft()
										},
										{cVirtualKeyEnd, cVirtualKeyEnd}
									};
									...
									Menu::NavElement navElement2 = {
										_steppers[i - 1]->GetRight(),
										{
											_steppers[i - 1]->GetLeft(), nullptr, _steppers[i - 2]->GetRight(),
											_steppers[i]->GetRight()
										},
										{cVirtualKeyEnd, cVirtualKeyEnd}
									};
									navElements.push_back(navElement2);
								...
								if (i + 1 == cStepperEnd)
								{
									if (cStepperEnd >= 2)
									{
										Menu::NavElement navElement = {
											_steppers[0]->GetLeft(),
											{
												_steppers[0]->GetRight(), _steppers[0]->GetRight(),
												_menuItems[miApply], _steppers[1]->GetLeft()
											},
											{cVirtualKeyEnd, cVirtualKeyEnd}
										};
										navElements.push_back(navElement);
										...
										Menu::NavElement navElement2 = {
											_steppers[0]->GetRight(),
											{
												_steppers[0]->GetLeft(), _steppers[0]->GetLeft(),
												_menuItems[miApply], _steppers[1]->GetRight()
											},
											{cVirtualKeyEnd, cVirtualKeyEnd}
										};
										navElements.push_back(navElement2);
									...
									{
										Menu::NavElement navElement = {
											_steppers[cStepperEnd - 1]->GetLeft(),
											{
												_steppers[cStepperEnd - 1]->GetRight(),
												_steppers[cStepperEnd - 1]->GetRight(),
												cStepperEnd >= 2
													? _steppers[cStepperEnd - 2]->GetLeft()
													: _menuItems[miApply],
												_menuItems[miApply]
											},
											{cVirtualKeyEnd, cVirtualKeyEnd}
										...
										Menu::NavElement navElement2 = {
											_steppers[cStepperEnd - 1]->GetRight(),
											{
												...
												_steppers[cStepperEnd - 1]->GetLeft(),
												cStepperEnd >= 2
													? _steppers[cStepperEnd - 2]->GetRight()
													: _menuItems[miApply],
												_menuItems[miApply]
											},
											{cVirtualKeyEnd, cVirtualKeyEnd}
...

							{
								Menu::NavElement navElement = {
									_menuItems[miApply],
									{
										nullptr, nullptr, _steppers[cStepperEnd - 1]->GetLeft(),
										_steppers[0]->GetLeft()
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
							...
							menu()->SetNavElements(_menuItems[miApply], true, &navElements[0],
							                       navElements.size());
						}
						else
							menu()->UnregNavElements(_menuItems[miApply]);
					...
					{
						auto offs = D3DXVECTOR2(-160.0f, 45.0f);
					...
					bool StartOptionsMenu::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
					{
						...
						if (sender == _menuItems[miApply])
						{
							...
							if (prevSyncFrameRate == Environment::sfrFixed && menu()->GetEnv()->
								GetSyncFrameRate() != Environment::sfrFixed)
							{
								std::string message;
								if (needReload)
								...
								menu()->ShowMessage(menu()->GetString(svWarning), message,
								                    menu()->GetString(svOk), menu()->GetGUI()->GetVPSize() / 2,
								                    gui::Widget::waCenter, 0.0f, this);
							}
							else if (needReload)
							{
								menu()->ShowMessage(menu()->GetString(svWarning), GetString(svHintNeedReload),
								                    GetString(svOk), menu()->GetGUI()->GetVPSize() / 2,
								                    gui::Widget::waCenter, 0.0f, this);
							}
							else
							{
