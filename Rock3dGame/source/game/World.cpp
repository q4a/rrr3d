...
namespace r3d
{
	namespace game
	{
		...
		World::World(): _terminateResult(EXIT_SUCCESS), _terminate(false), _pause(false), _syncFreq(60), _firstTick(0),
		                _lastTick(0), _lastTick2(0), _lastSyncTick(0), _timeAccum(0), _lastDrawTime(0), _curTimeIter(1),
		                _warmIterations(0), _time(0), _dTimeReal(0), _inputWasReset(false), _videoMode(false),
		                _timeResolution(0), _control(nullptr), _view(nullptr), _graph(nullptr), _audio(nullptr),
		                _videoPlayer(nullptr), _pxManager(nullptr), _pxScene(nullptr), _resManager(nullptr),
		                _camera(nullptr), _logic(nullptr), _db(nullptr), _map(nullptr), _env(nullptr), _edit(nullptr),
		                _game(nullptr)
		{
		...
		void World::Init(IView::Desc viewDesc)
		{
			...
			if (viewDesc.resolution.x == 0 && viewDesc.resolution.y == 0)
			{
				...
				{
					SerialFileXML file;
					RootNode node("root", this);

					file.LoadNodeFromFile(node, "user.xml");

					node.BeginLoad();

					SReadValue(&node, "resolution", viewDesc.resolution);
					SReadValue(&node, "fullScreen", viewDesc.fullscreen);
			...
			viewDesc.resolution = Point(_graph->GetEngine().GetParams().BackBufferWidth,
			                            _graph->GetEngine().GetParams().BackBufferHeight);
		...
		void World::Free()
		{
			...
			SafeDelete(_pxManager);
			SafeDelete(_audio);
			SafeDelete(_videoPlayer);
			SafeDelete(_graph);
			SafeDelete(_view);
			SafeDelete(_control);
		...
		{
			for (FixedStepEvents::const_iterator iter = _fixedStepEvents.begin(); iter != _fixedStepEvents.end(); ++
			     iter)
		...
		{
			for (LateProgressEvents::const_iterator iter = _lateProgressEvents.begin(); iter != _lateProgressEvents.
			     end(); ++iter)
		...
		bool World::OnMouseClickEvent(const MouseClick& mClick)
		{
			...
			res = res || _graph->GetGUI().OnMouseClickEvent(mClick.key, mClick.state, mClick.coord, mClick.shift1,
			                                                false);
		...
		void World::OnKeyChar(unsigned key, KeyState state, bool repeat)
		{
			_control->OnKeyChar(key, state, repeat);
		}

		void World::OnReset(HWND window, Point resolution, bool fullScreen)
		{
	...
	__int64 renderTick = newTick - tick;
			...
			{
				int syncIter = std::max(static_cast<int>(_curTimeIter - syncFreq * (newTick - _lastTick2) / _cpuFreq),
				                        1);
...
#endif
				__int64 fixTick = _curTimeIter * _cpuFreq / syncFreq;
				...
				if (downTimeIterAvg > _curTimeIter + iterDownThreshold)
				{
					if (gpuTimeAvg > 0.004f)
					{
						...
						for (int i = 0; i < _curTimeIter; ++i)
						{
							...
							_iterBuf[i].gpuTime = std::min(curIter + std::max(newIter - curIter, 0.0f) * 0.5f,
							                               std::max(curIter, newIter));
						}
					}
					...
					_curTimeIter = ClampValue(static_cast<int>(ceil(downTimeIterAvg - iterDownThreshold)), 1,
					                          cMaxSimIter);
				}
				//up
				else if (upTimeIterAvg < _curTimeIter - iterUpThreshold)
				{
					int upTimeIter = ClampValue(static_cast<int>(ceil(upTimeIterAvg + iterUpThreshold)), 1,
					                            cMaxSimIter);
					for (; upTimeIter < _curTimeIter; ++upTimeIter)
					{
						if (upTimeIterAvg < upTimeIter - (iterUpThreshold + std::max(
							_iterBuf[upTimeIter - 1].gpuTime - gpuTimeAvg / syncStep, 0.0f)))
						{
	...
	_game->Run(false);
		...
		{
			RootNode rootNode("root", this);

			SWriter* writer = rootNode.BeginSave();
			writer->WriteValue("map", _map);
			//writer->WriteValue("logic", _logic);
			rootNode.EndSave();

			SerialFileXML xmlFile;
		...
		void World::LoadLevel(const std::string& level)
		{
			...
			SReader* reader = rootNode.BeginLoad();
