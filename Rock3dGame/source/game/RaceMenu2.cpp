	...
	namespace game
	{
		...
		namespace n
		{
			CarFrame::CarFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_raceMenu(raceMenu), _car(nullptr), _camStyle(cCamStyleEnd), _carMapObj(nullptr)
			{
			...
			void CarFrame::OnShow(bool value)
			{
				...
				if (value)
				{
					...
					menu()->GetEnv()->SetLampRot(D3DXQUATERNION(0.19748747f, 0.34095559f, -0.46066701f, 0.79532242f),
					                             0);
					...
					menu()->GetEnv()->SetLampRot(D3DXQUATERNION(-0.17059785f, 0.045529708f, 0.95100683f, 0.25379914f),
					                             1);
			...
			{
				if (ReplaceRef(_car, value) || (value && !secret && _carMapObj == nullptr))
				{
					if (_carMapObj)
					...
					if (_car && !secret)
					{
						...
						for (GameCar::Wheels::const_iterator iter = gameCar.GetWheels().begin(); iter != gameCar.
						     GetWheels().end(); ++iter)
						{
							(*iter)->SetPos(
								(*iter)->GetPos() + D3DXVECTOR3(0, 0, -0.5f) * (*iter)->GetShape()->
								GetSuspensionTravel() + (*iter)->GetOffset());
							...
							wheelOffs = abs((*iter)->GetPos().z) + (*iter)->GetShape()->GetRadius() + (*iter)->
								GetOffset().z;
						}
			...
			void CarFrame::SetSlots(Player* player, bool includeDefSlots)
			{
				...
				for (int i = Player::stHyper; i <= Player::stWeapon4; ++i)
				{
					...
					if (includeDefSlots)
					{
						const Garage::PlaceItem* placeItem = placeSlot.defItem != nullptr
							                                     ? placeSlot.FindItem(placeSlot.defItem)
							                                     : nullptr;
						if (placeItem)
						{
						...
						else if (player)
						{
							...
							if (playerSlot && placeSlot.FindItem(playerSlot) == nullptr)
								continue;
						}
					}
					...
					if (player)
					{
						if (slot == nullptr)
							slot = player->GetSlotInst(type)
								       ? &player->GetSlotInst(type)->GetItem<WeaponItem>()
								       : nullptr;
					}
			...
			void CarFrame::SetCamStyle(CamStyle value, bool instant)
			{
				...
				{
					camera->FlyTo(pos, rot, 0.5f, true, true);
				}
				...
				if (lastCamStyle == csCar)
				{
					...
					camera->GetObserverCoord(D3DXVECTOR3(target.x, target.y, target.z), target.w, &_camLastPos,
					                         _camLastRot, NullVec2, 0, false, false, true, nullptr, nullptr, nullptr);
				}
			}


			SpaceshipFrame::SpaceshipFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _raceMenu(raceMenu), _redLampTime(0.0f)
			{
				{
					auto center = D3DXVECTOR3(63.0f, 0.0f, 23.0f);
					...
					graph::PlaneNode* spaceSprite = &static_cast<graph::PlaneNode&>(_spaceMapObj->GetGameObj().
						GetGrActor().GetNodes().front());

					D3DXVECTOR2 imageSize = static_cast<graph::Sampler2d&>(spaceSprite->material.Get()->samplers.
						front()).GetSize();
			...
			void SpaceshipFrame::OnProgress(float deltaTime)
			{
				...
				float redIntens = ClampValue((_redLampTime - 1.5f) / 0.15f, 0.0f, 1.0f) - ClampValue(
					(_redLampTime - 2.85f) / 0.15f, 0.0f, 1.0f);
			...
			void SpaceshipFrame::OnShow(bool value)
			{
				...
				if (value)
				{
					...
					menu()->GetEnv()->SetLampRot(D3DXQUATERNION(-0.47304764f, 0.077942163f, 0.86590993f, 0.14267041f),
					                             0);
					menu()->GetEnv()->SetLampColor(clrWhite, 0);
					menu()->GetEnv()->SwitchOnLamp(0, true);

					menu()->GetEnv()->SetLampPos(D3DXVECTOR3(-20.881384f, -21.184746f, 26.121809f), 1);
					menu()->GetEnv()->SetLampRot(D3DXQUATERNION(-0.16464995f, 0.34524971f, 0.39772648f, 0.83397770f),
					                             1);
					...
					menu()->GetEnv()->SetLampRot(D3DXQUATERNION(0.21948183f, 0.18329506f, -0.73549527f, 0.61423278f),
					                             2);
					...
					camera->SetClampAngle(D3DXVECTOR4(40.0f * D3DX_PI / 180, 30.0f * D3DX_PI / 180,
					                                  45.0f * D3DX_PI / 180, 80.0f * D3DX_PI / 180));
			...
			GarageFrame::GarageFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_raceMenu(raceMenu), _lastCarIndex(-1)
			{
				...
				_bottomPanel = menu->CreatePlane(root(), this, "GUI\\bottomPanel2.png", true, IdentityVec2,
				                                 gui::Material::bmTransparency);
				_bottomPanel->SetAnchor(gui::Widget::waBottom);
				_bottomPanel->SetAlign(gui::Widget::waBottom);

				_rightPanel = menu->CreatePlane(root(), this, "GUI\\rightPanel2.png", true, IdentityVec2,
				                                gui::Material::bmTransparency);
				_rightPanel->SetAlign(gui::Widget::waRight);

				_leftPanel = menu->CreatePlane(root(), this, "GUI\\rightPanel2.png", true, IdentityVec2,
				                               gui::Material::bmTransparency);
				_leftPanel->SetAlign(gui::Widget::waRight);
				_leftPanel->SetRot(D3DX_PI);

				_headerBg = menu->CreatePlane(_topPanel, this, "GUI\\header2.png", true, IdentityVec2,
				                              gui::Material::bmTransparency);
				_headerBg->SetAlign(gui::Widget::waCenter);
				_headerBg->SetVisible(false);

				_moneyBg = menu->CreatePlane(_bottomPanel, this, "GUI\\moneyBg.png", true, IdentityVec2,
				                             gui::Material::bmTransparency);
				_moneyBg->SetAlign(gui::Widget::waRightBottom);

				_stateBg = menu->CreatePlane(_bottomPanel, this, "GUI\\statFrame2.png", true, IdentityVec2,
				                             gui::Material::bmTransparency);
				_stateBg->SetAlign(gui::Widget::waLeftTop);

				_speedBar = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar2.png", "");
				_armorBar = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar2.png", "");
				_damageBar = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar2.png", "");
				_speedBar->SetAlign(gui::Widget::waLeftTop);
				_armorBar->SetAlign(gui::Widget::waLeftTop);
				_damageBar->SetAlign(gui::Widget::waLeftTop);

				_armorBarValue = menu->CreateLabel("", _armorBar, "Small", NullVec2, gui::Text::haRight,
				                                   gui::Text::vaTop, color2);
				_armorBarValue->SetPos(D3DXVECTOR2(_armorBar->GetSize().x - 4.0f, 1.0f));

				_damageBarValue = menu->CreateLabel("", _damageBar, "Small", NullVec2, gui::Text::haRight,
				                                    gui::Text::vaTop, color2);
				_damageBarValue->SetPos(D3DXVECTOR2(_damageBar->GetSize().x - 4.0f, 1.0f));

				_speedBarValue = menu->CreateLabel("", _speedBar, "Small", NullVec2, gui::Text::haRight,
				                                   gui::Text::vaTop, color2);
				...
				_menuItems[miBuy] = menu->CreateMenuButton(svNull, "Header", buyButton, buyButtonSel, _bottomPanel,
				                                           this, IdentityVec2, gui::Button::bsSelAnim, color2);
				...
				_UPbtn = _raceMenu->CreateUpArrow(root(), this);
				_UPbtn->SetAlign(gui::Widget::waLeft);
				_DWNbtn = _raceMenu->CreateDWNArrow(root(), this);
				_DWNbtn->SetAlign(gui::Widget::waLeft);

				_UPbtnR = _raceMenu->CreateUpArrowR(root(), this);
				...
				L_COLORINDEX = 1;
				R_COLORINDEX = 1;
				_leftColorGrid = menu->CreateGrid(_leftPanel, nullptr, gui::Grid::gsVertical);
				...
				_rightColorGrid = menu->CreateGrid(_rightPanel, nullptr, gui::Grid::gsVertical);
			...
			GarageFrame::~GarageFrame()
			{
				...
				menu()->ReleaseWidget(_UPbtn);
				menu()->ReleaseWidget(_DWNbtn);
				menu()->ReleaseWidget(_UPbtnR);
				menu()->ReleaseWidget(_DWNbtnR);
			...
			{
				gui::PlaneFon* plane = menu()->CreatePlane(
					grid, this, !locked ? "GUI\\Cars\\" + car->GetRecord()->GetName() + ".png" : "GUI\\lock.png", true,
					IdentityVec2, gui::Material::bmTransparency);
				plane->GetMaterial().SetColor(clrWhite);
				plane->SetData(car);

				plane = menu()->CreatePlane(plane, this, "GUI\\carBox.png", true, IdentityVec2,
				                            gui::Material::bmAdditive);
			...
			void GarageFrame::UpdateCarList(gui::Grid* grid)
			{
				...
				for (auto iter = garage().GetCars().begin(); iter != garage().GetCars().end(); ++iter)
				{
					bool isSecret = true;
					for (auto iterPlanet = tournament().GetPlanets().begin(); iterPlanet != tournament().GetPlanets().
					     end(); ++iterPlanet)
			...
			void GarageFrame::AdjustCarList()
			{
				...
				for (auto iter = _carGrid->GetChildren().begin(); iter != _carGrid->GetChildren().end(); ++iter, ++i)
					if ((*iter)->GetData() == car())
					{
						selIndex = i;
				...
				{
					for (auto iter = _carGrid->GetChildren().begin(); iter != _carGrid->GetChildren().end(); ++iter, ++
					     i)
					{
						if (_lastCarIndex == -1)
						{
							int indexRight = ClampValue(selIndex + numCars / 2, numCars - 1, totalCars - 1);
							int indexLeft = std::max(indexRight - numCars + 1, 0);
							(*iter)->SetVisible(i >= indexLeft && i <= indexRight);
						}
						else if (selIndex - _lastCarIndex >= 0 && i > selIndex - numCars && i <= std::max(
							selIndex, numCars - 1))
							(*iter)->SetVisible(true);
						else if (selIndex - _lastCarIndex < 0 && i >= std::min(selIndex, totalCars - numCars) && i <
							selIndex + numCars)
							(*iter)->SetVisible(true);
						else
			...
			{
				gui::Widget* box = menu()->CreateMenuButton("", "", "GUI\\colorBoxBg.png", "GUI\\colorBoxBgSel.png",
				                                            grid, this, IdentityVec2, gui::Button::bsSelAnim, clrWhite,
				                                            Menu::ssButton4);

				gui::PlaneFon* plane = menu()->CreatePlane(box, nullptr, "GUI\\colorBox.png", true, IdentityVec2,
				                                           gui::Material::bmOpaque, Menu::ssButton4);
			...
			void GarageFrame::RefreshColorList()
			{
				...
				{
					RefreshColorList(_leftColorGrid, Player::cLeftColors2, Player::cColorsCount);
				}
				...
				{
					RefreshColorList(_rightColorGrid, Player::cRightColors2, Player::cColorsCount);
				}
			}
			...
			void GarageFrame::FixColorPos()
			{
				...
				{
					_leftColorGrid->SetPos(D3DXVECTOR2(-20.0f, 168.0f));
				}
				if (RIGHT_CLR_PAGE_CHANGED == true)
				{
					_rightColorGrid->SetPos(D3DXVECTOR2(-20.0f, -168.0f));
				}
			}
			...
			void GarageFrame::ShowMessage(StringValue message, gui::Widget* sender, const D3DXVECTOR2& slotSize)
			{
				...
				menu()->ShowMessage(GetString(svWarning), GetString(message), GetString(svOk),
			...
			void GarageFrame::ShowAccept(const std::string& message, gui::Widget* sender, const D3DXVECTOR2& slotSize)
			{
				...
				menu()->ShowAccept(true, message, GetString(svYes), GetString(svNo), uiRoot()->GetVPSize() / 2,
			...
			void GarageFrame::OnShow(bool value)
			{
				...
				if (value)
				{
					Vector<Menu::NavElement> navElements;
					...
					for (auto iter = _leftColorGrid->GetChildren().begin(); iter != _leftColorGrid->GetChildren().end();
					     ++iter, ++index)
					{
						...
						if (index >= 2)
						{
							Menu::NavElement navElement = {
								gridItemsLeft[index - 1],
								{
									_rightColorGrid->GetChildren().front(), _leftArrow, gridItemsLeft[index - 2],
									gridItemsLeft[index]
								},
						...
						if (index + 1 == itemCount)
						{
							if (itemCount >= 2)
							{
								Menu::NavElement navElement = {
									gridItemsLeft[0],
									{
										_rightColorGrid->GetChildren().front(), _leftArrow, _menuItems[miBuy],
										gridItemsLeft[1]
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
							...
							Menu::NavElement navElement2 = {
								gridItemsLeft[itemCount - 1],
								{
									_rightColorGrid->GetChildren().front(), _leftArrow,
									itemCount >= 2 ? gridItemsLeft[itemCount - 2] : _menuItems[miBuy], _menuItems[miBuy]
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
					...
					index = 0;
					for (auto iter = _rightColorGrid->GetChildren().begin(); iter != _rightColorGrid->GetChildren().
					     end(); ++iter, ++index)
					{
						...
						{
							Menu::NavElement navElement = {
								gridItemsRight[index - 1],
								{
									_rightArrow, _leftColorGrid->GetChildren().front(), gridItemsRight[index - 2],
									gridItemsRight[index]
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
							navElements.push_back(navElement);
						...
						if (index + 1 == itemCount)
						{
							if (itemCount >= 2)
							{
								Menu::NavElement navElement = {
									gridItemsRight[0],
									{
										_rightArrow, _leftColorGrid->GetChildren().front(), _menuItems[miBuy],
										gridItemsRight[1]
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
							...
							Menu::NavElement navElement2 = {
								gridItemsRight[itemCount - 1],
								{
									_rightArrow, _leftColorGrid->GetChildren().front(),
									itemCount >= 2 ? gridItemsRight[itemCount - 2] : _menuItems[miBuy],
									_menuItems[miBuy]
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
...

					{
						Menu::NavElement navElement = {
							_menuItems[miExit],
							{_menuItems[miBuy], _menuItems[miBuy], _menuItems[miBuy], _menuItems[miBuy]},
					...
					}
					{
						Menu::NavElement navElement = {
							_menuItems[miBuy],
							{
								gridItemsLeft[gridItemsLeft.size() - 1], gridItemsRight[gridItemsRight.size() - 1],
								_leftArrow, _menuItems[miExit]
							},
							{cVirtualKeyEnd, cVirtualKeyEnd}
						};
						navElements.push_back(navElement);
					...
					{
						Menu::NavElement navElement = {
							_leftArrow, {gridItemsLeft[0], _rightArrow, _menuItems[miBuy], _menuItems[miBuy]},
							{vkShoulderLeft, cVirtualKeyEnd}
						};
						navElements.push_back(navElement);
					}
					{
						Menu::NavElement navElement = {
							_rightArrow, {_leftArrow, gridItemsRight[0], _menuItems[miBuy], _menuItems[miBuy]},
							{vkShoulderRight, cVirtualKeyEnd}
						};
						navElements.push_back(navElement);
					}
			...
			{
				D3DXVECTOR2 size = menu()->GetImageSize(_topPanel->GetMaterial());
				X_VPSIZE_MMENU = size.x;
				Y_VPSIZE_MMENU = size.y;

				//23.0f
				_topPanel->SetSize(size.x, size.y);
				_topPanel->SetPos(vpSize.x / 2, 0.0f);

				_rightPanel->SetPos(D3DXVECTOR2(vpSize.x + 1,
				                                (_topPanel->GetSize().y + vpSize.y - _bottomPanel->GetSize().y + 0.0f) /
				                                2.0f));
				_leftPanel->SetPos(
					D3DXVECTOR2(-1, (_topPanel->GetSize().y + vpSize.y - _bottomPanel->GetSize().y + 0.0f) / 2.0f));
				...
				_UPbtn->SetPos(D3DXVECTOR2(35.5f, (vpSize.y / 2) - 220.0f));
				_DWNbtn->SetPos(D3DXVECTOR2(35.5f, (vpSize.y / 2) + 180.0f));

				_UPbtnR->SetPos(D3DXVECTOR2((vpSize.x - 96.5f), (vpSize.y / 2) - 220.0f));
				_DWNbtnR->SetPos(D3DXVECTOR2((vpSize.x - 96.5f), (vpSize.y / 2) + 180.0f));
			...
			void GarageFrame::OnInvalidate()
			{
				...
				for (auto iter = _carGrid->GetChildren().begin(); iter != _carGrid->GetChildren().end(); ++iter, ++i)
				{
					auto carPlane = static_cast<gui::PlaneFon*>(*iter);
					auto carBox = static_cast<gui::PlaneFon*>(carPlane->GetChildren().front());

					bool isSel = carPlane->GetData() == car();
					if (isSel)
						carIndex = i;
					carPlane->GetMaterial().SetColor(
						isSel ? D3DXCOLOR(255.0f, 150.0f, 0.0f, 255.0f) / 255.0f : clrWhite);
					carBox->GetMaterial().GetSampler().SetTex(
						isSel ? GetTexture("GUI\\carBoxSel.png") : GetTexture("GUI\\carBox.png"));
					carBox->GetMaterial().
					        SetBlending(isSel ? gui::Material::bmAdditive : gui::Material::bmTransparency);
				}
				AdjustCarList();

				_leftArrow->SetVisible(carIndex > 0);
				_rightArrow->SetVisible(carIndex < static_cast<int>(_carGrid->GetChildren().size()) - 1);
				_UPbtn->SetVisible(L_COLORINDEX > 1);
				_DWNbtn->SetVisible(L_COLORINDEX < 2);
			...
			bool GarageFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				...
				{
					PrevCar();
					CLR_PAGE_CHANGED = true;
					RIGHT_CLR_PAGE_CHANGED = true;
					RefreshColorList();
					//LEFT
					if (L_COLORINDEX == 1)
					{
					...
					else if (L_COLORINDEX == 2)
					{
						UpdateColorList(_leftColorGrid, Player::cLeftColors2, Player::cColorsCount);
					}
					//RIGHT
					if (R_COLORINDEX == 1)
					{
					...
					else if (R_COLORINDEX == 2)
					{
						UpdateColorList(_rightColorGrid, Player::cRightColors2, Player::cColorsCount);
					}
					FixColorPos();
					return true;
				}
					...
					|| sender->GetParent() == _rightColorGrid))
				{
					SelectColor(D3DXCOLOR(
						static_cast<gui::PlaneFon*>(sender->GetChildren().front())->GetMaterial().GetColor()));
					return true;
				}
			...
			WorkshopFrame::WorkshopFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_raceMenu(raceMenu), _goodScroll(0), _overGood(nullptr), _overGood2(nullptr), _infoId(-1),
				_numSelectedSlots(0)
			{
				const D3DXCOLOR color2 = D3DXCOLOR(214.0f, 214.0f, 214.0f, 255.0f) / 255.0f;
				...
				gui::Text::HorAlign horLabels[cLabelEnd] = {
					gui::Text::haCenter, gui::Text::haRight, gui::Text::haCenter
				};
				gui::Text::VertAlign vertLabels[cLabelEnd] = {
					gui::Text::vaCenter, gui::Text::vaCenter, gui::Text::vaTop
				};
				D3DXCOLOR colorLabels[cLabelEnd] = {color2, clrWhite, color2};

				_topPanel = menu->CreatePlane(root(), this, "GUI\\topPanel3.png", true, IdentityVec2,
				                              gui::Material::bmTransparency);
				_topPanel->SetAnchor(gui::Widget::waTop);
				_topPanel->SetAlign(gui::Widget::waTop);

				_bottomPanel = menu->CreatePlane(root(), this, "GUI\\bottomPanel3.png", true, IdentityVec2,
				                                 gui::Material::bmTransparency);
				_bottomPanel->SetAnchor(gui::Widget::waBottom);
				_bottomPanel->SetAlign(gui::Widget::waBottom);

				_leftPanel = menu->CreatePlane(root(), this, "GUI\\leftPanel3.png", true, IdentityVec2,
				                               gui::Material::bmTransparency);
				...
				_speedBarBonus = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar2Plus.png", "");
				_armorBarBonus = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar2Plus.png", "");
				_damageBarBonus = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar2Plus.png", "");
				...
				_speedBar = menu->CreateBar(_speedBarBonus, nullptr, "GUI\\statBar2.png", "");
				_armorBar = menu->CreateBar(_armorBarBonus, nullptr, "GUI\\statBar2.png", "");
				_damageBar = menu->CreateBar(_damageBarBonus, nullptr, "GUI\\statBar2.png", "");
				_speedBar->SetAlign(gui::Widget::waLeftTop);
				_armorBar->SetAlign(gui::Widget::waLeftTop);
				_damageBar->SetAlign(gui::Widget::waLeftTop);
				...
				_downArrow->SetSize(menu->StretchImage(*_downArrow->GetFon(), D3DXVECTOR2(30.0f, 30.0f), true, true,
				                                       true, false));
				...
				_upArrow->SetSize(menu->StretchImage(*_upArrow->GetFon(), D3DXVECTOR2(30.0f, 30.0f), true, true, true,
				                                     false));
				_upArrow->SetSelSize(menu->GetImageSize(*_upArrow->GetSel()) * _upArrow->GetSize().x / size.x);
				//иконка слота в мастерской:
				const std::string slotIcons[Player::cSlotTypeEnd] = {
					"", "", "", "", "GUI\\hyperSlot.png", "GUI\\mineSlot.png", "GUI\\wpnSlot.png", "GUI\\wpnSlot.png",
					"GUI\\wpnSlot.png", "GUI\\wpnSlot.png", ""
				};
				...
				for (int i = 0; i < Player::cSlotTypeEnd; ++i)
				{
					...
					slot.plane = menu->CreatePlane(root(), this, "GUI\\slot2.png", true, IdentityVec2,
					                               gui::Material::bmTransparency);

					if (slotIcons[i] != "")
						slot.slotIcon = menu->CreatePlane(slot.plane, nullptr, slotIcons[i], true, IdentityVec2,
						                                  gui::Material::bmTransparency);
					else
						slot.slotIcon = nullptr;

					slot.viewport = menu->CreateViewPort3d(slot.plane, nullptr, "", gui::ViewPort3d::msMouseOverAnim);
					...
					slot.mesh3d = menu->CreateMesh3d(slot.viewport, nullptr, nullptr);
					...
					slot.chargeBox = menu->CreatePlane(slot.plane, nullptr, "GUI\\chargeBox.png", true);
					slot.chargeBox->SetAlign(gui::Widget::waLeftTop);
					slot.chargeBox->SetPos(68.0f, -12.0f);

					slot.chargeBar = menu->CreateBar(slot.chargeBox, nullptr, "GUI\\chargeBar.png", "",
					                                 gui::ProgressBar::psVertical);
					...
					slot.UpgBtn = _raceMenu->CreateUpgBtn(slot.plane, this);
					slot.UpgBtn->SetAlign(gui::Widget::waCenter);
					slot.UpgBtn->SetPos(0.0f, 65.0f);
				}
				...
				_dragItem.viewport = menu->CreateViewPort3d(root(), nullptr, "", gui::ViewPort3d::msStatic);
				_dragItem.mesh3d = menu->CreateMesh3d(_dragItem.viewport, nullptr, nullptr);
				_dragItem.mesh3d->GetMaterial()->GetSampler().SetFiltering(graph::BaseSampler::sfAnisotropic);
				_dragItem.slot = nullptr;
			}
			...
			{
				gui::ViewPort3d* viewPort = menu()->CreateMesh3dBox(_goodGrid, this,
				                                                    slot != nullptr
					                                                    ? slot->GetItem().GetMesh()
					                                                    : nullptr,
				                                                    slot != nullptr
					                                                    ? slot->GetItem().GetTexture()
					                                                    : nullptr, gui::ViewPort3d::msMouseOverAnim);
				viewPort->SetData(slot);

				if (slot)
				{
					auto mesh3d = static_cast<gui::Mesh3d*>(viewPort->GetBox()->GetChildren().front());
				...
				gui::PlaneFon* box = menu()->CreatePlane(viewPort, nullptr, "GUI\\slot2Frame.png", true, IdentityVec2,
				                                         gui::Material::bmTransparency);
			...
			void WorkshopFrame::UpdateGoods()
			{
				...
				struct Pred
				{
					...
					{
						if (slot1->GetItem().GetId() < slot2->GetItem().GetId())
							return true;
						return false;
					}
				};
				...
				for (Workshop::Items::const_iterator iter = items.begin(); iter != items.end(); ++iter)
				{
					...
					if (slot != nullptr && slot->GetItem().IsHide() == false)
					{
				...
				for (; i < cellsCount; ++i)
				{
					gui::ViewPort3d* viewPort = AddGood(nullptr, i, cellsCount);
					size = viewPort->GetSize();
				}
			...
			void WorkshopFrame::AdjustGood()
			{
				...
				int i = 0;
				for (auto iter = _goodGrid->GetChildren().begin(); iter != _goodGrid->GetChildren().end(); ++iter)
				{
					...
					if (i >= _goodScroll * _goodGrid->maxCellsOnLine() && i < _goodScroll * _goodGrid->maxCellsOnLine()
						+ 12)
				...
				_downArrow->SetVisible(
					_goodScroll < std::max(static_cast<int>(ceil(i / (float)_goodGrid->maxCellsOnLine())) - 4, 0));
			}
			...
			void WorkshopFrame::StartDrag(Slot* slot, Player::SlotType slotType, int chargeCount)
			{
				...
				if (ReplaceRef(_dragItem.slot, slot))
				{
					_dragItem.slot = slot;
			...
			{
				if (_dragItem.slot == nullptr)
					return true;

				if (dropOut)
				{
					if (!intoGood && _dragItem.slotType != Player::cSlotTypeEnd && player()->GetSlot(_dragItem.slotType)
						== nullptr)
					{
					...
					else
					{
						if (menu()->IsCampaign() && _dragItem.slotType != Player::cSlotTypeEnd)
						{
							...
							if (HasString(svSellWeapon))
								message = StrFmt(message.c_str(),
								                 menu()->FormatCurrency(
									                 workshop().GetCostItem(
										                 _dragItem.slot, true, _dragItem.chargeCount)).c_str());
			...
			void WorkshopFrame::ResetDrag()
			{
				...
				SafeRelease(_dragItem.slot);
				_dragItem.slot = nullptr;
				_dragItem.mesh3d->SetMesh(nullptr);
			...
			void WorkshopFrame::UpdateSlot(Player::SlotType type, Slot* slot)
			{
				...
				if (!car()->GetSlot(type).active ||
					(menu()->IsSkirmish() && wpnSlotIndex >= 0 && wpnSlotIndex < Garage::cWeaponMaxLevel && wpnSlotIndex
						>= Garage::cWeaponCurLevel))
				{
				...
				if (slot != nullptr)
				{
					...
					box.chargeBox->SetVisible(wpn != nullptr);
					box.chargeButton->SetVisible(wpn != nullptr);
					//показывать кнопку апгрейда оружия только для тех слотов, которые указаны в воркшопе:
					if (wpn != nullptr && wpn->GetModify() == true && menu()->weaponUpgrades())
						box.UpgBtn->SetVisible(true);
					else
						box.UpgBtn->SetVisible(false);

					if (wpn != nullptr && wpn->GetLimit())
					{
						if (wpn->GetCntCharge() == wpn->GetMaxCharge() && menu()->weaponUpgrades())
							box.UpgBtn->SetVisible(true);
						else
							box.UpgBtn->SetVisible(false);
					}
					...
					if (wpn)
					{
						...
						//здесь можно заблокировать кнопку по другому принципу (например после прокачки, сейчас же блокируются только кнопки которые невидимые по своей сути)
						//возможно позже сделать чтобы кнопка после прокачки не исчезала? это сделать легко, надо просто добавить bool переменную для wpn(item) - не указывать значение в воркшопе(по дефолту сделать false, а в случае прокачки true). И заменить этой переменной modify в данной части функции из трех строк, которая приведена ниже:
						bool modify = wpn->GetModify();
						box.UpgBtn->SetEnabled(box.UpgBtn->GetVisible());
						box.UpgBtn->Select(!modify, true);
						if (!menu()->weaponUpgrades())
							box.UpgBtn->SetEnabled(false);
					}

					box.level->SetVisible(wpn == nullptr);

					if (wpn)
					{
						box.upLevel = wpn->GetCntCharge();
						box.chargeBar->SetProgress(box.upLevel / static_cast<float>(wpn->GetMaxCharge()));
					}
					else
					{
						box.upLevel = garage().GetUpgradeCarLevel(car(), type, slot->GetRecord());
						//*чтобы добавить иконку прокачки апгрейдов нужно изменить две строки кода ниже:
						int level = ClampValue(box.upLevel, 0, 3); //3*
						const std::string imgs[4] = {
							"GUI\\upLevel1.png", "GUI\\upLevel2.png", "GUI\\upLevel3.png", "GUI\\upLevel4.png"
						}; //[4]*
			...
			void WorkshopFrame::SelectSlots(Slot* slot, bool select)
			{
				...
				for (int i = 0; i < Player::cSlotTypeEnd; ++i)
				{
					...
					case Player::stWeapon4:
						...
						_slots[i].slotIcon->GetMaterial().GetSampler().SetTex(
							GetTexture(selectSlot ? "GUI\\wpnSlotSel.png" : "GUI\\wpnSlot.png"));
						_slots[i].slotIcon->SetSize(menu()->GetImageSize(_slots[i].slotIcon->GetMaterial()));
						_slots[i].slotIcon->SetVisible(selectSlot || plrSlot == nullptr);
						break;
					}

					if (checkSlot)
					{
						WeaponItem* wpn = plrSlot ? plrSlot->GetItem().IsWeaponItem() : nullptr;
						...
						else if (slotType == Player::cSlotTypeEnd || plrSlot == nullptr)
						{
							slotType = type;
			...
			void WorkshopFrame::UpdateSelection()
			{
				if (_dragItem.slot)
					...
					SelectSlots(nullptr, false);
			}

			void WorkshopFrame::SetOverGood(gui::Widget* value)
			{
				if (ReplaceRef(_overGood, value))
				{
					_overGood = value;
					...
					{
						WEAPON_INDEXX = 0;
						auto slot = static_cast<Slot*>(_overGood->GetData());
			...
			{
				if (ReplaceRef(_overGood2, value))
				{
					if (_overGood2)
						lsl::StaticCast<gui::ViewPort3d*>(_overGood2)->SetStyle(
							_overGood2->GetParent() == _goodGrid
								? gui::ViewPort3d::msMouseOverAnim
								: gui::ViewPort3d::msStatic);
			                             ...
			                             int infoId)
			{
				...
				if (wpn)
					damage = StrFmt("%0.0f", wpn->GetDamage());

				D3DXVECTOR2 cellOffs = slotSize / 4;
				cellOffs.y = -cellOffs.y;

				if (WEAPON_INDEXX > 3 && WEAPON_INDEXX < 9)
					menu()->ShowWeaponDialog(GetString(slot->GetItem().GetName()), GetString(slot->GetItem().GetInfo()),
					                         menu()->FormatCurrency(static_cast<int>(inf_cost)),
					                         menu()->FormatCurrency(slot->GetItem().GetDamage()),
					                         sender->GetWorldPos() + cellOffs, gui::Widget::waLeftBottom, 0.0f);
				else
					menu()->ShowWeaponDialog(GetString(slot->GetItem().GetName()), GetString(slot->GetItem().GetInfo()),
					                         menu()->FormatCurrency(cost),
					                         menu()->FormatCurrency(slot->GetItem().GetDamage()),
					                         sender->GetWorldPos() + cellOffs, gui::Widget::waLeftBottom, 0.0f);
			}
			...
			bool WorkshopFrame::UpdateSlotInfo(gui::Widget* sender, const SlotBox& slotBox, Slot* slot,
			                                   Player::SlotType type)
			{
				bool isSender = slotBox.level == sender || slotBox.plane == sender || slotBox.chargeButton == sender ||
					slotBox.UpgBtn == sender;
				if (!isSender)
					return false;
				...
				{
					ShowInfo(
						slot, slot->GetItem<WeaponItem>().GetChargeCost() * slot->GetItem<WeaponItem>().GetChargeStep(),
						sender, sender->GetSize() * 4.0f, infoId);
				}
				...
				else if (slotBox.UpgBtn->IsMouseEnter())
				{
					...
					if (slot && slot != nullptr && slot->GetItem().GetModify() == true)
					{
						...
						for (Workshop::Items::const_iterator iter = items.begin(); iter != items.end(); ++iter)
						{
							...
							if (newSlot != nullptr && newSlot->GetItem().GetId() == linkID)
							{
								int cost = static_cast<int>((cntcharge * newSlot->GetItem().GetCost()) * inf);
								if (cntcharge <= 1)
									cost = cntcharge * newSlot->GetItem().GetCost();

								if (slot->GetItem().GetLimit() == false)
									menu()->ShowWeaponDialog(GetString(newSlot->GetItem().GetName()),
									                         GetString(newSlot->GetItem().GetInfo()),
									                         menu()->FormatCurrency(cost),
									                         menu()->FormatCurrency(newSlot->GetItem().GetDamage()),
									                         sender->GetWorldPos() - D3DXVECTOR2(0.0f, 60.0f),
									                         gui::Widget::waLeftBottom, 0.0f);
								else
									menu()->ShowWeaponDialog(GetString(newSlot->GetItem().GetName()),
									                         GetString(newSlot->GetItem().GetInfo()),
									                         menu()->FormatCurrency(newSlot->GetItem().GetCost()),
									                         menu()->FormatCurrency(newSlot->GetItem().GetDamage()),
									                         sender->GetWorldPos() - D3DXVECTOR2(0.0f, 60.0f),
									                         gui::Widget::waLeftBottom, 0.0f);
							}
			...
			void WorkshopFrame::UpdateBonusStats(Player::SlotType type, Slot* slot)
			{
				...
				{
					_raceMenu->UpdateStats(nullptr, nullptr, _armorBarBonus, _speedBarBonus, _damageBarBonus, nullptr,
					                       nullptr, nullptr);
					UpdateStats();
				}
			}
			...
			void WorkshopFrame::ShowAccept(const std::string& message, gui::Widget* sender, const D3DXVECTOR2& slotSize,
			                               Slot* slot)
			{
				HideInfo(cUndefInfo);
				...
				menu()->ShowAccept(true, message, GetString(svYes), GetString(svNo), sender->GetWorldPos() + cellOffs,
				                   gui::Widget::waLeftBottom, this, slot);
			}
			...
			{
				_leftPanel->SetPos(
					D3DXVECTOR2(30, (_topPanel->GetSize().y - 30.0f + vpSize.y - _bottomPanel->GetSize().y) / 2));
				...
				_stateBg->SetPos(D3DXVECTOR2(vpSize.x / 2 - _stateBg->GetSize().x - 15.0f,
				                             -_bottomPanel->GetSize().y + 25.0f));
				...
				for (int i = 0; i < cMenuItemEnd; ++i)
					_menuItems[i]->SetPos(D3DXVECTOR2(-vpSize.x / 2,
					                                  -_bottomPanel->GetSize().y + 40.0f + i * (_menuItems[i]->GetSize()
						                                  .y + 10.0f)));

				_labels[mlHeader]->SetPos(D3DXVECTOR2(20.0f, 40.0f));
				_labels[mlMoney]->SetPos(D3DXVECTOR2(-53.0f, -29.0f));

				_labels[mlHint]->SetPos(D3DXVECTOR2(
					(_menuItems[miExit]->GetPos().x + _menuItems[miExit]->GetSize().x + _stateBg->GetPos().x) / 2,
					-72.0f));
				...
				float leftOffset = std::max(centerX - 330.0f * scale,
				                            _leftPanel->GetPos().x + _leftPanel->GetSize().x + 80.0f);
				...
				//размещение слотов в мастерской:
				D3DXVECTOR2 slotOffset[Player::cSlotTypeEnd] = {
					D3DXVECTOR2(leftOffset + 20.0f, (centerY + slotSize.y / 2 + slotSpace) - 20.0f), //wheel
					D3DXVECTOR2(leftOffset + 20.0f, (centerY - slotSize.y / 2 - slotSpace) - 20.0f), //truba
					D3DXVECTOR2(centerX, bottomOffset), //armor
					D3DXVECTOR2(centerX + slotSize.x / 2 + 6.0f * slotSpace, bottomOffset), //motor
					D3DXVECTOR2(rightOffset, centerY - slotSize.y / 2 - slotSpace), //hyper
					D3DXVECTOR2(rightOffset, centerY + slotSize.y / 2 + slotSpace), //mine
					D3DXVECTOR2(centerX - (slotSize.x + 4.5f * slotSpace) * 1.5f, topOffset), //wpn1
					D3DXVECTOR2(centerX - (slotSize.x + 4.5f * slotSpace) * 0.5f, topOffset), //wpn2
					D3DXVECTOR2(centerX + (slotSize.x + 4.5f * slotSpace) * 0.5f, topOffset), //wpn3
					D3DXVECTOR2(centerX + (slotSize.x + 4.5f * slotSpace) * 1.5f, topOffset), //wpn4
					D3DXVECTOR2(leftOffset + 20.0f, (centerY + (slotSize.y + slotSpace * 6)) - 20.0f), //trans
				};
			...
			void WorkshopFrame::OnInvalidate()
			{
				...
				for (int i = 0; i < Player::cSlotTypeEnd; ++i)
				{
					...
					gui::Button* level = _slots[type].level && _slots[type].level->GetVisible()
						                     ? _slots[type].level
						                     : nullptr;
					gui::Button* chargeButton = _slots[type].chargeButton && _slots[type].chargeButton->GetVisible()
						                            ? _slots[type].chargeButton
						                            : nullptr;
				...
				//настройка навигации стрелками в мастерской.
				//Здесь работает следующая логика: выбраный элемент, соседний элемент (слева), соседний элемент (справа), соседний элемент (сверху), соседний элемент снизу, клавиши ввода.

				{
					//armor
					Menu::NavElement navElement = {
						slots[Player::stArmor],
						{slots[Player::stTrans], slots[Player::stMotor], slots[Player::stTrans], _menuItems[miExit]},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					Menu::NavElement navElement = {
						slots[Player::stMotor],
						{slots[Player::stArmor], slots[Player::stMine], slots[Player::stMine], _menuItems[miExit]},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					Menu::NavElement navElement = {
						slots[Player::stMine],
						{
							slots[Player::stMotor], slots[Player::stHyper], slots[Player::stHyper],
							slots[Player::stMotor]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						slots[Player::stHyper],
						{slots[lastWeapon], slots[Player::stMine], slots[lastWeapon], slots[Player::stMine]},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					Menu::NavElement navElement = {
						slots[Player::stWeapon4],
						{
							slots[Player::stWeapon3], slots[Player::stHyper], slots[Player::stWeapon3],
							slots[Player::stHyper]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						slots[Player::stWeapon3],
						{
							slots[Player::stWeapon2], slots[Player::stWeapon4], slots[Player::stWeapon2],
							slots[Player::stWeapon4]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						slots[Player::stWeapon2],
						{
							slots[Player::stWeapon1], slots[Player::stWeapon3], slots[Player::stWeapon1],
							slots[Player::stWeapon3]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						slots[Player::stWeapon1],
						{
							slots[Player::stTruba], slots[Player::stWeapon2], slots[Player::stTruba],
							slots[Player::stTruba]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					Menu::NavElement navElement = {
						slots[Player::stTruba],
						{
							slots[Player::stWheel], slots[Player::stWeapon1], slots[Player::stWeapon1],
							slots[Player::stWheel]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					...
					Menu::NavElement navElement = {
						slots[Player::stWheel],
						{
							slots[Player::stTruba], slots[Player::stTrans], slots[Player::stTruba],
							slots[Player::stTrans]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
				{
					...
					Menu::NavElement navElement = {
						slots[Player::stTrans],
						{
							slots[Player::stWheel], slots[Player::stArmor], slots[Player::stWheel],
							slots[Player::stArmor]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
...

				{
					Menu::NavElement navElement = {
						_menuItems[miExit],
						{
							slots[Player::stArmor], slots[Player::stArmor], slots[Player::stArmor],
							slots[Player::stArmor]
						},
						{vkBack, cVirtualKeyEnd}
					};
			...
			bool WorkshopFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				...
				if (sender == menu()->GetAcceptSender())
				{
					if (menu()->GetAcceptResultYes())
					{
						...
						{
							auto slot = static_cast<Slot*>(menu()->GetAcceptData());
				...
				{
					auto slot = static_cast<Slot*>(sender->GetData());
					...
					else if (slot)
					{
						...
						else if (menu()->IsCampaign())
						{
							...
							if (HasString(svBuyWeapon))
								message = StrFmt(message.c_str(),
								                 menu()->FormatCurrency(slot->GetItem().GetCost()).c_str());
				...
				for (int i = 0; i < Player::cSlotTypeEnd; ++i)
				{
					...
					if (_slots[i].UpgBtn == sender && !IsDragItem())
					{
						if (slot && slot != nullptr)
						{
							...
							for (Workshop::Items::const_iterator iter = items.begin(); iter != items.end(); ++iter)
							{
								Slot* newSlot = *iter;
								++i;
								//заменяем оружие на то, которое вказано в linkId.
								if (newSlot != nullptr && newSlot->GetItem().GetId() == linkID)
								{
									int cost = static_cast<int>((cntcharge * newSlot->GetItem().GetCost()) * inf);
									...
									if (menu()->IsCampaign())
									{
										//апгрейд доступен с конкретной планеты указаной в воркшопе.
										if (tournament().GetCurPlanet().GetIndex() >= reqIndex)
										{
											//если установлен лимит, то слот сначала надо полностью прокачать.
											if (slot->GetItem().GetLimit() == true)
											{
												if (cntcharge == maxcharge)
												{
													if (money >= limitCost)
													{
														...
														UpdateStats();
														UpdateMoney();
														menu()->PlaySound("Sounds\\Reload.ogg");
													}
													else
													{
														ShowMessage(svHintCantMoney, sender,
														            D3DXVECTOR2(-30.0f, -18.0f));
													}
												}
												else
												{
													ShowMessage(svHintCantWpnUpg, sender, D3DXVECTOR2(-30.0f, -18.0f));
												}
											}
											else
											{
												...
												{
													ShowMessage(svHintCantMoney, sender, D3DXVECTOR2(-30.0f, -18.0f));
												}
											}
										}
										else
					...
					{
						if (!place.lock && !(IsDragItem() && place.FindItem(_dragItem.slot) == nullptr))
						{
							InstalSlot(type, IsDragItem() ? _dragItem.slot : nullptr, _dragItem.chargeCount);
							if (IsDragItem())
			...
			bool WorkshopFrame::OnMouseMove(gui::Widget* sender, const gui::MouseMove& mMove)
			{
				...
				{
					if (_dragItem.slot)
						UpdateDragPos(D3DXVECTOR2(mMove.worldCoord.x, mMove.worldCoord.y));
				}
			...
			GamersFrame::GamersFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_raceMenu(raceMenu), _planetIndex(-1)
			{
				const D3DXCOLOR color2 = D3DXCOLOR(214.0f, 214.0f, 214.0f, 255.0f) / 255.0f;
				...
				_viewport = menu->CreateViewPort3d(root(), nullptr, "", gui::ViewPort3d::msAnim, false);
				D3DXQUATERNION quat;
				...
				_mesh3d = menu->CreateMesh3d(_viewport, nullptr, nullptr);
				_mesh3d->GetMaterial()->GetSampler().SetFiltering(graph::BaseSampler::sfAnisotropic);

				_bottomPanel = menu->CreatePlane(root(), this, "GUI\\bottomPanel4.png", true, IdentityVec2,
				                                 gui::Material::bmTransparency);
				_bottomPanel->SetAnchor(gui::Widget::waBottom);
				_bottomPanel->SetAlign(gui::Widget::waBottom);
				...
				_photoLight = menu->CreatePlane(_bottomPanel, this, "GUI\\wndLight4.png", true, IdentityVec2,
				                                gui::Material::bmTransparency);
			...
			int GamersFrame::GetNextPlanetIndex(int sIndex)
			{
				...
				if (menu()->ISpectator() && menu()->IsNetGame() && menu()->GetNet()->isClient())
				{
					for (int i = sIndex + 1; i < static_cast<int>(tournament().GetGamers().size()); ++i)
					{
						...
						if (achievment().CheckGamerId(gamerId) && !(menu()->IsNetGame() && !net()->player()->
							CheckGamerId(gamerId)))
							return i;
					}

					return -1;
				}
				for (int i = sIndex + 1; i < static_cast<int>(tournament().GetGamers().size()) - spectators; ++i)
				{
					int gamerId = tournament().GetGamers()[i]->GetId();
			...
			void GamersFrame::OnAdjustLayout(const D3DXVECTOR2& vpSize)
			{
				...
				auto planetPos = D3DXVECTOR2(vpSize.x / 2 - 25.0f, planetRadius);
			...
			void GamersFrame::OnShow(bool value)
			{
				if (value)
				{
					...
					if (menu()->ISpectator() && menu()->IsNetGame() && menu()->GetNet()->isClient())
					{
						planet = tournament().GetGamer(SPECTATOR_ID_BEGIN);
					}
				...
				Menu::NavElement elements[] = {
					{_nextArrow, {nullptr, nullptr, _rightArrow, _rightArrow}, {cVirtualKeyEnd, cVirtualKeyEnd}},
			...
			void GamersFrame::OnInvalidate()
			{
				...
				_photo->SetSize(menu()->StretchImage(_photo->GetMaterial(), D3DXVECTOR2(190.0f, 190.0f), true, false,
				                                     true, false));
			...
			AngarFrame::AngarFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_raceMenu(raceMenu), _planetIndex(-1), _planetPrevIndex(-1), _doorTime(-1.0f)
			{
				const auto color = D3DXCOLOR(0xff76cef2);
				const D3DXCOLOR color2 = D3DXCOLOR(214.0f, 214.0f, 214.0f, 255.0f) / 255.0f;
				const StringValue menuItemsStr[cMenuItemEnd] = {svBack};
				...
				gui::Text::HorAlign horLabels[cLabelEnd] = {
					gui::Text::haCenter, gui::Text::haCenter, gui::Text::haLeft
				};
				gui::Text::VertAlign vertLabels[cLabelEnd] = {
					gui::Text::vaCenter, gui::Text::vaCenter, gui::Text::vaTop
				};
				...
				_planetGridBg = menu->CreateGrid(_bottomPanelBg, nullptr, gui::Grid::gsHorizontal);
				_planetGridBg->SetAlign(gui::Widget::waLeft);

				_bottomPanel = menu->CreatePlane(_bottomPanelBg, this, "GUI\\bottomPanel6.png", true, IdentityVec2,
				                                 gui::Material::bmTransparency);
				_bottomPanel->SetAlign(gui::Widget::waBottom);

				_planetGrid = menu->CreateGrid(_bottomPanel, nullptr, gui::Grid::gsHorizontal);
				_planetGrid->SetAlign(gui::Widget::waLeft);

				_planetInfo = menu->CreatePlane(_bottomPanel, this, "GUI\\planetInfo.png", true, IdentityVec2,
				                                gui::Material::bmTransparency);
				_planetInfo->SetAlign(gui::Widget::waBottom);

				_planetInfoClose = menu->CreateMenuButton(svNull, "", "GUI\\buttonBg6.png", "GUI\\buttonBgSel6.png",
				                                          _planetInfo, this, IdentityVec2, gui::Button::bsSelAnim,
				                                          clrWhite);

				_planetBoss = menu->CreatePlane(_planetInfo, this, "", true, IdentityVec2,
				                                gui::Material::bmTransparency);

				_planetBossCar = menu->CreateViewPort3d(_planetInfo, nullptr, "", gui::ViewPort3d::msAnim);
				D3DXQUATERNION quat;
				D3DXQuaternionRotationAxis(&quat, &ZVector, D3DX_PI / 2.0f);
			...
			AngarFrame::PlanetBox& AngarFrame::AddPlanet(Planet* planet, int index)
			{
				...
				PlanetBox* box = static_cast<unsigned>(index) < _planets.size() ? &(_planets[index]) : nullptr;
				...
				if (box == nullptr)
				{
					...
					box->viewport = menu()->CreateMesh3dBox(_planetGrid, this, planet->GetMesh(), planet->GetTexture(),
					                                        gui::ViewPort3d::msStatic, Menu::ssButton5);
					D3DXQUATERNION quat;
					D3DXQuaternionRotationAxis(&quat, &ZVector, D3DX_PI / 24.0f);
					...
					auto mesh3d = static_cast<gui::Mesh3d*>(box->viewport->GetBox()->GetChildren().front());
					mesh3d->GetMaterial()->GetSampler().SetFiltering(graph::BaseSampler::sfAnisotropic);

					box->slot = menu()->CreateMenuButton(svNull, "Item", "GUI\\doorSlot.png", "GUI\\doorSlotSel.png",
					                                     _planetGridBg, this, IdentityVec2, gui::Button::bsSelAnim,
					                                     clrWhite);

					box->slotClip = menu()->CreateDummy(box->slot, nullptr);
					box->slotClip->SetFlag(gui::Widget::wfClientClip, true);
					box->slotClip->SetSize(box->slot->GetSize() - D3DXVECTOR2(8.0f, 8.0f));

					box->doorDown = menu()->CreatePlane(box->slotClip, nullptr, "GUI\\doorDown.png", true, IdentityVec2,
					                                    gui::Material::bmTransparency);
					box->doorDown->SetAlign(gui::Widget::waTop);

					box->doorUp = menu()->CreatePlane(box->slotClip, nullptr, "GUI\\doorUp.png", true, IdentityVec2,
					                                  gui::Material::bmTransparency);
					box->doorUp->SetAlign(gui::Widget::waBottom);
				}

				if ((planet->GetState() == Planet::psOpen) || (race->GetPlanetChampion() && tournament().GetNextPlanet()
					== planet))
				{
				...
				{
					box->slot->SetText(StrFmt(GetString(svRequestPoints).c_str(),
					                          planet->GetRequestPoints(planet->GetPass())));
				}
			...
			void AngarFrame::UpdatePlanets()
			{
				...
				for (auto iter = tournament().GetPlanets().begin(); iter != tournament().GetPlanets().end(); ++iter, ++
				     index)
				{
			...
			void AngarFrame::ShowMessage(StringValue message, gui::Widget* sender, const D3DXVECTOR2& slotSize)
			{
				...
				menu()->ShowMessage(GetString(svWarning), GetString(message), GetString(svOk),
				                    sender->GetWorldPos() + cellOffs, gui::Widget::waBottom, 0.0f);
			}

			void AngarFrame::ShowAccept(const std::string& message, gui::Widget* sender, const D3DXVECTOR2& slotSize,
			                            Object* data)
			{
				D3DXVECTOR2 cellOffs = slotSize / 2;
				cellOffs.y = -cellOffs.y;
				...
				menu()->ShowAccept(true, message, GetString(svYes), GetString(svNo),
				                   (sender ? sender->GetWorldPos() : uiRoot()->GetVPSize() / 2) + cellOffs,
			...
			void AngarFrame::OnShow(bool value)
			{
				if (value)
				{
					bool selectCurPlanet = menu()->GetRace()->GetPlanetChampion() && tournament().GetNextPlanet() !=
						nullptr;
					...
					for (unsigned i = 0; i < count; ++i)
					{
						...
						if (i + 1 == count)
						{
							if (count >= 2)
							{
								Menu::NavElement navElement = {
									_planets[0].viewport,
									{
										_planets[i].viewport, _planets[1].viewport, _menuItems[miExit],
										_menuItems[miExit]
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
...

							{
								Menu::NavElement navElement = {
									_planets[i].viewport,
									{
										count >= 2 ? _planets[i - 1].viewport : nullptr,
										count >= 2 ? _planets[0].viewport : nullptr, _menuItems[miExit],
										_menuItems[miExit]
									},
									{cVirtualKeyEnd, cVirtualKeyEnd}
								};
						...
						{
							Menu::NavElement navElement = {
								_planets[i - 1].viewport,
								{
									_planets[i - 2].viewport, _planets[i].viewport, _menuItems[miExit],
									_menuItems[miExit]
								},
								{cVirtualKeyEnd, cVirtualKeyEnd}
							};
					...
					{
						Menu::NavElement navElement = {
							_menuItems[miExit], {nullptr, nullptr, _planets[0].viewport, _planets[0].viewport},
							{vkBack, cVirtualKeyEnd}
						};
						navElements.push_back(navElement);
					}
			...
			void AngarFrame::OnAdjustLayout(const D3DXVECTOR2& vpSize)
			{
				...
				_planetGrid->SetPos(D3DXVECTOR2(-_bottomPanel->GetSize().x / 2 + 35.0f,
				                                -_bottomPanel->GetSize().y / 2));
				_planetGridBg->SetPos(_planetGrid->GetPos() + D3DXVECTOR2(0.0f, -124.0f));

				if (_planetIndex != -1)
				{
					D3DXVECTOR2 pos = _planets[_planetIndex].slot->GetPos() + D3DXVECTOR2(0.0f, -37.0f) + _planetGridBg
						->GetPos();
					pos.x = ClampValue(pos.x, -vpSize.x / 2 + _planetInfo->GetSize().x / 2,
					                   -vpSize.x / 2 + vpSize.x - _planetInfo->GetSize().x / 2);
			...
			void AngarFrame::OnInvalidate()
			{
				if (_planetIndex != -1)
				{
					...
					_planetBoss->SetSize(menu()->StretchImage(_planetBoss->GetMaterial(), D3DXVECTOR2(110.0f, 85.0f),
					                                          true, false, true, false));

					if (planet->GetBoss().cars.size() > 0)
						_raceMenu->CreateCar(_planetBossCar, garage().FindCar(planet->GetBoss().cars.front().record),
						                     clrWhite, nullptr);
					else
						_raceMenu->CreateCar(_planetBossCar, nullptr);
				...
				for (unsigned index = 0; index < _planets.size(); ++index)
				{
					...
					box.viewport->SetStyle(index == _planetIndex
						                       ? gui::ViewPort3d::msAnim
						                       : gui::ViewPort3d::msMouseOverAnim);
			...
			bool AngarFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				if (sender == _menuItems[miExit])
				{
					if (menu()->GetRace()->GetPlanetChampion())
						ShowAccept(GetString(svYouReadyStayPlanet), nullptr, NullVec2, &tournament().GetCurPlanet());
					else
						_raceMenu->SetState(RaceMenu::msMain);
					return true;
				}
				...
				if (sender == menu()->GetAcceptSender())
				{
					if (menu()->GetAcceptResultYes())
					{
						auto planet = lsl::StaticCast<Planet*>(menu()->GetAcceptData());
						bool newPlanet = planet->GetState() == Planet::psClosed || planet->GetState() ==
							Planet::psUnavailable;
				...
				for (unsigned i = 0; i < _planets.size(); ++i)
				{
					...
					if (sender == planetBox.slot && i == _planetIndex)
					{
						...
						if (race->GetPlanetChampion() || race->GetDevMode() && TEST_BUILD == true)
						{
							...
							if (planet == tournament().GetNextPlanet())
							{
		...
		if (steamService()->isInit() && menu()->IsCampaign())
			...
			void AngarFrame::OnProgress(float deltaTime)
			{
				if (_doorTime >= 0.0f)
				{
					float alpha = ClampValue(_doorTime / 0.25f, 0.0f, 1.0f);
					_doorTime += deltaTime;
			...
			AchievmentFrame::AchievmentFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent):
				MenuFrame(menu, parent), _raceMenu(raceMenu)
			{
				const auto color = D3DXCOLOR(0xFFFA5800);
				...
				_bottomPanel = menu->CreatePlane(root(), this, "GUI\\achievmentBottomPanel.png", true, IdentityVec2,
				                                 gui::Material::bmTransparency);
				_bottomPanel->SetAlign(gui::Widget::waBottom);
				_bottomPanel->GetMaterial().GetSampler().SetFiltering(graph::Sampler2d::sfLinear);

				_bg = menu->CreatePlane(root(), this, "GUI\\achievmentBg.dds", true, IdentityVec2,
				                        gui::Material::bmTransparency);
				...
				_panel = menu->CreatePlane(root(), this, "GUI\\achievmentPanel.png", true, IdentityVec2,
				                           gui::Material::bmTransparency);
			...
			const AchievmentFrame::AchievmentBox* AchievmentFrame::AddAchievment(
				unsigned index, const std::string& lockImg, const std::string& img, const D3DXVECTOR2& pos,
				Achievment* model)
			{
				LSL_ASSERT(model);

				AchievmentBox* box = index < _achievments.size() ? &_achievments[index] : nullptr;
				if (box == nullptr)
				{
					AchievmentBox newBox;
					newBox.image = menu()->CreatePlane(_panel, this, "", false, IdentityVec2,
					                                   gui::Material::bmTransparency);
					newBox.image->GetMaterial().GetSampler().SetFiltering(graph::Sampler2d::sfLinear);

					newBox.price = menu()->CreateLabel(svNull, newBox.image, "Item", NullVec2, gui::Text::haCenter,
					                                   gui::Text::vaCenter, D3DXCOLOR(0xFFC3C2C0));
					newBox.button = menu()->CreateMenuButton(svNull, "", "", "", newBox.image, this, IdentityVec2,
				...
				box->image->GetMaterial().GetSampler().SetTex(
					GetTexture(model->state() == Achievment::asOpened ? img : lockImg));
				...
				box->price->SetText(StrFmt("%d", model->price()));
			...
			{
				const std::string nameList[cMenuBoxEnd] = {
					AchievmentModel::cViper, AchievmentModel::cBuggi, AchievmentModel::cAirblade,
					AchievmentModel::cReflector, AchievmentModel::cDroid, AchievmentModel::cTankchetti,
					AchievmentModel::cPhaser, AchievmentModel::cMustang, AchievmentModel::cArmor4
				};
				const std::string lockImgList[cMenuBoxEnd] = {
					"GUI\\Rewards\\viperLock.png", "GUI\\Rewards\\buggiLock.png", "GUI\\Rewards\\airbladeLock.png",
					"GUI\\Rewards\\reflectorLock.png", "GUI\\Rewards\\droidLock.png",
					"GUI\\Rewards\\tankchettiLock.png", "GUI\\Rewards\\phaserLock.png", "GUI\\Rewards\\mustangLock.png",
					"GUI\\Rewards\\musicTrackLock.png"
				};
				const std::string imgList[cMenuBoxEnd] = {
					"GUI\\Rewards\\viper.png", "GUI\\Rewards\\buggi.png", "GUI\\Rewards\\airblade.png",
					"GUI\\Rewards\\reflector.png", "GUI\\Rewards\\droid.png", "GUI\\Rewards\\tankchetti.png",
					"GUI\\Rewards\\phaser.png", "GUI\\Rewards\\mustang.png", "GUI\\Rewards\\musicTrack.png"
				};
				const D3DXVECTOR2 posList[cMenuBoxEnd] = {
					D3DXVECTOR2(200.0f, 105.0f), D3DXVECTOR2(405.0f, 155.0f), D3DXVECTOR2(0.0f, 245.0f),
					D3DXVECTOR2(-190.0f, 125.0f), D3DXVECTOR2(-380.0f, 95.0f), D3DXVECTOR2(-325.0f, 265.0f),
					D3DXVECTOR2(-190.0f, 400.0f), D3DXVECTOR2(205.0f, 375.0f), D3DXVECTOR2(445.0f, 315.0f)
				};

				const Box sortBox[cMenuBoxEnd] = {
					mbMusicTrack, mbBuggi, mbViper, mbPhaser, mbMustang, mbAirblade, mbReflector, mbDroid, mbTankchetti
				};
				...
				for (int i = 0; i < cMenuBoxEnd; ++i)
				{
					...
					if (model)
					{
						achievments[box] = *AddAchievment(count++, lockImgList[box], imgList[box],
						                                  posList[box] + D3DXVECTOR2(-30.0f, 0.0f), model);
					}
				}
				...
				{
					Menu::NavElement navElement = {
						achievments[mbPhaser].button,
						{
							achievments[mbTankchetti].button, achievments[mbMustang].button,
							achievments[mbAirblade].button, _menuItems[miExit]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				...
				{
					Menu::NavElement navElement = {
						achievments[mbMustang].button,
						{
							achievments[mbPhaser].button, achievments[mbMusicTrack].button,
							achievments[mbAirblade].button, _menuItems[miExit]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						achievments[mbMusicTrack].button,
						{
							achievments[mbMustang].button, achievments[mbTankchetti].button,
							achievments[mbBuggi].button, _menuItems[miExit]
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				...
				{
					Menu::NavElement navElement = {
						achievments[mbTankchetti].button,
						{
							achievments[mbMusicTrack].button, achievments[mbAirblade].button,
							achievments[mbDroid].button, achievments[mbPhaser].button
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				...
				{
					Menu::NavElement navElement = {
						achievments[mbAirblade].button,
						{
							achievments[mbTankchetti].button, achievments[mbMusicTrack].button,
							achievments[mbReflector].button, achievments[mbPhaser].button
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				...
				{
					Menu::NavElement navElement = {
						achievments[mbDroid].button,
						{
							achievments[mbBuggi].button, achievments[mbReflector].button, _menuItems[miExit],
							achievments[mbTankchetti].button
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						achievments[mbReflector].button,
						{
							achievments[mbDroid].button, achievments[mbViper].button, _menuItems[miExit],
							achievments[mbTankchetti].button
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						achievments[mbViper].button,
						{
							achievments[mbReflector].button, achievments[mbBuggi].button, _menuItems[miExit],
							achievments[mbAirblade].button
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
				...
				{
					Menu::NavElement navElement = {
						achievments[mbBuggi].button,
						{
							achievments[mbViper].button, achievments[mbDroid].button, _menuItems[miExit],
							achievments[mbMusicTrack].button
						},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					};
...

				{
					Menu::NavElement navElement = {
						_menuItems[miExit],
						{nullptr, nullptr, achievments[mbPhaser].button, achievments[mbPhaser].button},
						{vkBack, cVirtualKeyEnd}
					};
					navElements.push_back(navElement);
				}
			...
			void AchievmentFrame::UpdateSelection(gui::Widget* sender, bool select)
			{
				...
				for (Achievments::const_reverse_iterator iter = _achievments.rbegin(); iter != _achievments.rend(); ++
				     iter)
				{
			...
			{
				menu()->ShowAccept(true, message, GetString(svYes), GetString(svNo), uiRoot()->GetVPSize() / 2,
				                   gui::Widget::waCenter, this, achievment);
			}
			...
			void AchievmentFrame::OnAdjustLayout(const D3DXVECTOR2& vpSize)
			{
				...
				for (int i = 0; i < cMenuItemEnd; ++i)
					_menuItems[i]->SetPos(
						D3DXVECTOR2(0.0f, _bottomPanel->GetPos().y + 17.0f + i * (_menuItems[i]->GetSize().y + 10.0f)));
			}
			...
			bool AchievmentFrame::OnClick(gui::Widget* sender, const gui::MouseClick& mClick)
			{
				...
				{
					auto achievment = lsl::StaticCast<Achievment*>(menu()->GetAcceptData());
				...
				const AchievmentBox* box = (sender->GetData() || sender->GetParent() && sender->GetParent()->GetData())
					                           ? GetAchievment(static_cast<Achievment*>(sender->GetData()))
					                           : nullptr;
				if (box)
				{
			...
			RaceMainFrame::RaceMainFrame(Menu* menu, RaceMenu* raceMenu, gui::Widget* parent): MenuFrame(menu, parent),
				_raceMenu(raceMenu)
			{
				...
				const std::string menuItemsIcon[cMenuItemEnd] = {
					"GUI\\icoStart.png", "GUI\\icoWorkshop.png", "GUI\\icoGarage.png", "GUI\\icoSpace.png",
					"GUI\\icoAchivment.png", "GUI\\icoOptions.png", "GUI\\icoExit.png"
				};

				StringValue strLabels[cLabelEnd] = {
					svDivision, svNull, svNull, svNull, svTrackMaps, svMoney, svKills, svDeads, svPoints, svRequest
				};
				std::string fontLabels[cLabelEnd] = {
					"Small", "Small", "Small", "Small", "Small", "Item", "SmallBold", "SmallBold", "SmallBold",
					"SmallBold"
				};
				gui::Text::HorAlign horLabels[cLabelEnd] = {
					gui::Text::haCenter, gui::Text::haCenter, gui::Text::haCenter, gui::Text::haCenter,
					gui::Text::haCenter, gui::Text::haRight, gui::Text::haLeft, gui::Text::haLeft, gui::Text::haLeft,
					gui::Text::haLeft
				};
				gui::Text::VertAlign vertLabels[cLabelEnd] = {
				...
				D3DXCOLOR colorLabels[cLabelEnd] = {
					color1, color1, color1, color1, color1, clrWhite, color2, color2, color2, color2
				};
				...
				_bottomPanel = menu->CreatePlane(root(), this, "GUI\\bottomPanel.png", true, IdentityVec2,
				                                 gui::Material::bmTransparency);
				_bottomPanel->SetAnchor(gui::Widget::waBottom);
				_bottomPanel->SetAlign(gui::Widget::waBottom);
				...
				_stateBg = menu->CreatePlane(_bottomPanel, this, "GUI\\statFrame.png", true, IdentityVec2,
				                             gui::Material::bmTransparency);
				_stateBg->SetAlign(gui::Widget::waLeftBottom);

				_speedBar = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar.png", "");
				_armorBar = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar.png", "");
				_damageBar = menu->CreateBar(_stateBg, nullptr, "GUI\\statBar.png", "");

				_armorBarValue = menu->CreateLabel("", _armorBar, "Small", NullVec2, gui::Text::haRight,
				                                   gui::Text::vaCenter, color1);
				...
				_viewportCar = menu->CreateViewPort3d(_topPanel, nullptr, "", gui::ViewPort3d::msAnim);
				D3DXQUATERNION quat;
				D3DXQuaternionRotationAxis(&quat, &ZVector, D3DX_PI / 2.0f);
				_viewportCar->SetRot3dSpeed(quat);

				_playerGrid = menu->CreateDummy(root(), nullptr);

				for (int i = 0; i < 2; ++i)
				{
					_photoImages[i] = menu->CreatePlane(_topPanel, nullptr, "", true, IdentityVec2,
					                                    gui::Material::bmTransparency);
				...
				for (int i = 0; i < 3; ++i)
				{
					_frameImages[i] = menu->CreatePlane(
						i == 2 ? static_cast<gui::Widget*>(_viewportCar) : static_cast<gui::Widget*>(_photoImages[i]),
						nullptr, "GUI\\imageFrame1.png", true, IdentityVec2, gui::Material::bmTransparency);
					_frameImages[i]->GetMaterial().GetSampler().SetFiltering(graph::BaseSampler::sfLinear);
				}

				for (int i = 0; i < 6; ++i)
				{
					_chargeBars[i] = menu->CreateBar(_topPanel, nullptr, "GUI\\chargeBar1.png", "",
					                                 gui::ProgressBar::psHorizontal); //psVertical
				...
				{
					_slots[i] = menu->
						CreateMesh3dBox(_topPanel, nullptr, nullptr, nullptr, gui::ViewPort3d::msSlowAnim);
					_slots[i]->SetSize(D3DXVECTOR2(64.0f, 64.0f));
				...
				gui::Widget* labelsParent[cLabelEnd] = {
					_topPanel, _topPanel, _topPanel, _topPanel, _topPanel, _moneyBg, _topPanel, _topPanel, _topPanel,
					_topPanel
				};
				for (int i = 0; i < cLabelEnd; ++i)
					_labels[i] = menu->CreateLabel(strLabels[i], labelsParent[i], fontLabels[i], NullVec2, horLabels[i],
			...
			RaceMainFrame::PlayerBox* RaceMainFrame::AddPlayer(NetPlayer* netPlayer, unsigned index)
			{
				...
				PlayerBox* box = index < _players.size() ? &_players[index] : nullptr;
				if (box == nullptr)
				{
					PlayerBox newBox;

					newBox.bgRoot = menu()->CreateDummy(_playerGrid, nullptr);

					newBox.bg = menu()->CreatePlane(newBox.bgRoot, nullptr, "GUI\\netPlayerFrame.png", true,
					                                IdentityVec2, gui::Material::bmTransparency);

					newBox.photo = menu()->CreatePlane(newBox.bgRoot, nullptr, "", false, IdentityVec2,
					                                   gui::Material::bmTransparency);

					newBox.name = menu()->CreateLabel(svNull, newBox.bgRoot, "VerySmall", NullVec2, gui::Text::haLeft,
					                                  gui::Text::vaCenter, color1);
					newBox.name->SetAlign(gui::Widget::waLeft);

					newBox.readyLabel = menu()->CreateLabel(svNull, newBox.bgRoot, "VerySmall", NullVec2,
					                                        gui::Text::haLeft, gui::Text::vaCenter, color1);
					newBox.readyLabel->SetAlign(gui::Widget::waLeft);

					newBox.kick = menu()->CreateMenuButton(svNull, "", "GUI\\netPlayerKick.png",
					                                       "GUI\\netPlayerKickSel.png", newBox.bgRoot, this,
					                                       IdentityVec2, gui::Button::bsSelAnim);

					newBox.readyState = menu()->CreatePlane(newBox.bgRoot, nullptr, "", false, IdentityVec2,
					                                        gui::Material::bmTransparency);

					newBox.viewportCar = menu()->CreateViewPort3d(newBox.bgRoot, nullptr, "", gui::ViewPort3d::msAnim);
					D3DXQUATERNION quat;
					D3DXQuaternionRotationAxis(&quat, &ZVector, D3DX_PI / 2.0f);
				...
				box->readyState->GetMaterial().GetSampler().SetTex(GetTexture(
					netPlayer->IsRaceReady() ? "GUI\\netPlayerReadyStateSel.png" : "GUI\\netPlayerReadyState.png"));
			...
			void RaceMainFrame::AdjustPlayer(PlayerBox* box, bool invert)
			{
				...
				box->photo->SetSize(menu()->StretchImage(box->photo->GetMaterial(), D3DXVECTOR2(100.0f, 97.0f), true,
				                                         false, true, false));
			...
			void RaceMainFrame::UpdatePlayers()
			{
				...
				for (auto iter = net()->netOpponents().begin(); iter != net()->netOpponents().end(); ++iter, ++index)
				{
			...
			void RaceMainFrame::OnShow(bool value)
			{
				...
				Menu::NavElement elements[] = {
					{
						_menuItems[miStartRace], {_menuItems[miExit], _menuItems[miWorkshop], nullptr, nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					},
					{
						_menuItems[miWorkshop], {_menuItems[miStartRace], _menuItems[miGarage], nullptr, nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					},
					{
						_menuItems[miGarage], {_menuItems[miWorkshop], _menuItems[miSpace], nullptr, nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					},
					{
						_menuItems[miSpace], {_menuItems[miGarage], _menuItems[miAchivments], nullptr, nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					},
					{
						_menuItems[miAchivments], {_menuItems[miSpace], _menuItems[miOptions], nullptr, nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					},
					{
						_menuItems[miOptions], {_menuItems[miAchivments], _menuItems[miExit], nullptr, nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					},
					{
						_menuItems[miExit], {_menuItems[miOptions], _menuItems[miStartRace], nullptr, nullptr},
						{cVirtualKeyEnd, cVirtualKeyEnd}
					}
				};
			...
			void RaceMainFrame::OnAdjustLayout(const D3DXVECTOR2& vpSize)
			{
				...
				for (int i = 0; i < cMenuItemEnd; ++i)
				{
					float offsX = -(cMenuItemEnd * _menuItems[i]->GetSize().x + (cMenuItemEnd - 1) * menuItemSpaceX) / 2
						+ _menuItems[i]->GetSize().x / 2;
					_menuItems[i]->SetPos(offsX + i * (_menuItems[i]->GetSize().x + menuItemSpaceX),
					                      -_menuItems[i]->GetSize().y / 2 - 25);
				}

				const float frameImagesPos[3] = {-590.0f, 475.0f, 590.0f};
				for (int i = 0; i < 3; ++i)
				...
				for (int i = 0; i < 6; ++i)
					...
					{
						_chargeBars[i]->SetPos(-100.0f + i * 95, 122);
						_slots[i]->SetPos(-100.0f + i * 95, 80);
					}


				_netInfo->SetPos(-vpSize.x / 2 + 20.0f, _topPanel->GetSize().y + 20.0f);

				_viewportCar->SetPos(D3DXVECTOR2(590.0f, 87.0f));
				...
				for (int i = 0; i < static_cast<int>(_players.size()); ++i)
				{
					...
					_players[i].bgRoot->SetPos(D3DXVECTOR2(size.x / 2 + 8.0f + col * (vpSize.x - size.x - 16.0f),
					                                       size.y / 2 + (size.y + space) * row));
				}
				_playerGrid->SetPos(D3DXVECTOR2(-1, _topPanel->GetSize().y + 30.0f));

				menu()->GetUserChat()->inputPos(D3DXVECTOR2(_stateBg->GetSize().x,
				                                            vpSize.y - _bottomPanel->GetSize().y - 10.0f));
				menu()->GetUserChat()->inputSize(D3DXVECTOR2(vpSize.x - _stateBg->GetSize().x - _moneyBg->GetSize().x,
				                                             300.0f));
				...
				menu()->GetUserChat()->linesSize(D3DXVECTOR2(vpSize.x / 3,
				                                             vpSize.y - _topPanel->GetSize().y - _bottomPanel->GetSize()
				                                             .y - _moneyBg->GetSize().y));
			}
			...
			void RaceMainFrame::OnInvalidate()
			{
				if (net()->isStarted())
				{
					...
					for (auto iter = net()->players().begin(); iter != net()->players().end(); ++iter)
					{
						NetPlayer* netPlayer = (*iter);
						text += StrFmt("player(%d) connected ready=%d car=%s\n", netPlayer->id(),
						               static_cast<DWORD>(netPlayer->IsRaceReady()),
						               netPlayer->GetCar() ? GetString(netPlayer->GetCar()->GetName()).c_str() : "");
					}
				...
				for (int i = 0; i < 6; ++i)
				{
					...
					{
						_chargeBars[i]->SetVisible(true);
						_slots[i]->SetVisible(true);

						WeaponItem& wpn = slot->GetItem<WeaponItem>();

						//massmod идея на будущее. Работающий фикс для исправления прогресс бара оружия.
						if (wpn.GetName() != "scBulletGun")
							_chargeBars[i]->SetProgress(ClampValue(wpn.GetCntCharge() / 7.0f, 0.0f, 1.0f));
						else
							_chargeBars[i]->SetProgress(ClampValue(wpn.GetCntCharge() / 28.0f, 0.0f, 1.0f));

						auto mesh3d = static_cast<gui::Mesh3d*>(_slots[i]->GetBox()->GetChildren().front());
					...
					{
						_chargeBars[i]->SetVisible(false);
				...
				_labels[mlMoney]->SetPos(D3DXVECTOR2(-53.0f, -29.0f));
				_labels[mlKills]->SetPos(D3DXVECTOR2(-515.0f, 72.0f));
			...
			void RaceMainFrame::OnReceiveCmd(const net::NetMessage& msg, const net::NetCmdHeader& header,
			...
			RaceMenu::RaceMenu(Menu* menu, gui::Widget* parent, Player* player): _menu(menu), _player(player),
				_state(msMain), _lastState(cStateEnd)
			{
			...
			void RaceMenu::OnReceiveCmd(const net::NetMessage& msg, const net::NetCmdHeader& header, const void* data,
			                            unsigned size)
			{
				if (_mainFrame->visible())
			...
			void RaceMenu::OnProcessNetEvent(unsigned id, NetEventData* data)
			{
				...
				if (id == cNetPlayerSetColor && data->failed)
				{
					...
					_menu->ShowMessage(_menu->GetString(svWarning), _menu->GetString(svHintSetColorFailed),
					                   _menu->GetString(svOk), _menu->GetGUI()->GetVPSize() / 2, gui::Widget::waCenter,
					                   0.0f);
				}
			}

			gui::Button* RaceMenu::CreateMenuButton(const std::string& icon, gui::Widget* parent,
			                                        gui::Widget::Event* guiEvent)
			{
				gui::Button* button = _menu->CreateMenuButton(svNull, "", "GUI\\buttonBg1.png", "GUI\\buttonBgSel1.png",
				                                              parent, guiEvent, IdentityVec2, gui::Button::bsSelAnim,
				                                              clrWhite, Menu::ssButton2);

				_menu->CreatePlane(button, nullptr, icon, true, IdentityVec2, gui::Material::bmTransparency);

				return button;
			}

			gui::Button* RaceMenu::CreateMenuButton2(StringValue name, gui::Widget* parent, const D3DXCOLOR& textColor,
			                                         gui::Widget::Event* guiEvent)
			{
				return _menu->CreateMenuButton(name, "Header", "GUI\\buttonBg2.png", "GUI\\buttonBgSel2.png", parent,
				                               guiEvent, IdentityVec2, gui::Button::bsSelAnim, textColor,
				                               Menu::ssButton1);
			}
			...
			gui::Button* RaceMenu::CreateUpArrow(gui::Widget* parent, gui::Widget::Event* guiEvent)
			{
				return _menu->CreateUpArrow(parent, guiEvent);
			}

			gui::Button* RaceMenu::CreateUpArrowR(gui::Widget* parent, gui::Widget::Event* guiEvent)
			{
				return _menu->CreateUpArrowR(parent, guiEvent);
			}

			gui::Button* RaceMenu::CreateDWNArrow(gui::Widget* parent, gui::Widget::Event* guiEvent)
			{
				return _menu->CreateDWNArrow(parent, guiEvent);
			}

			gui::Button* RaceMenu::CreateDWNArrowR(gui::Widget* parent, gui::Widget::Event* guiEvent)
			{
				return _menu->CreateDWNArrowR(parent, guiEvent);
			}

			gui::Button* RaceMenu::CreateArrow2(gui::Widget* parent, gui::Widget::Event* guiEvent)
			{
				return _menu->CreateMenuButton(svNull, "", "GUI\\arrow2.png", "GUI\\arrowSel2.png", parent, guiEvent,
				                               IdentityVec2, gui::Button::bsSelAnim);
			}

			gui::Button* RaceMenu::CreatePlusButton(gui::Widget* parent, gui::Widget::Event* guiEvent)
			{
				return _menu->CreateMenuButton(svNull, "", "GUI\\chargeButton.png", "GUI\\chargeButtonSel.png", parent,
				                               guiEvent, IdentityVec2, gui::Button::bsSelAnim, clrWhite,
				                               Menu::ssButton3);
			}

			gui::Button* RaceMenu::CreateUpgBtn(gui::Widget* parent, gui::Widget::Event* guiEvent)
			{
				return _menu->CreateMenuButton(svNull, "", "GUI\\UpgBtn.png", "GUI\\UpgBtnSel.png", parent, guiEvent,
				                               IdentityVec2, gui::Button::bsSelAnim, clrWhite, Menu::ssButton3);
			}

			void RaceMenu::CreateCar(gui::ViewPort3d* viewport, Garage::Car* car, const D3DXCOLOR& color,
			                         Slot* slots[Player::cSlotTypeEnd])
			{
				...
				if (car)
				{
					for (auto iter = car->GetBodies().begin(); iter != car->GetBodies().end(); ++iter)
						if (iter->mesh)
						{
					...
					if (!car->GetWheels().empty() && car->GetWheel())
					{
						...
						{
							gui::Mesh3d* wheel = _menu->CreateMesh3d(viewport, car->GetWheel(),
							                                         !car->GetBodies().empty()
								                                         ? car->GetBodies().front().texture
								                                         : nullptr);
							...
							//фикс протекторов колес для джипов:
							if (car->GetName() == "scMarauder" || car->GetName() == "scDirtdevil" || car->GetName() ==
								"scMonsterTruckBoss")
							{
								if (wheels[i].y > 0)
								{
				...
				if (car && slots)
				{
					for (int i = 0; i < Player::cSlotTypeEnd; ++i)
					{
						...
						{
							WeaponItem* wpn = slot->GetItem().IsWeaponItem();
							if (wpn != nullptr && wpn->GetMapObj() == nullptr)
								continue;

							D3DXVECTOR3 pos = placeSlot.pos + placeItem->offset;
							D3DXQUATERNION rot = placeItem ? placeItem->rot : NullQuaternion;

							gui::Mesh3d* carMesh = _menu->CreateMesh3d(viewport, slot->GetItem().GetMesh(),
							                                           slot->GetItem().GetTexture());
			...
			{
				Garage::Car* car = player && player->GetCar().record
					                   ? _menu->GetRace()->GetGarage().FindCar(player->GetCar().record)
					                   : nullptr;
				...
				CreateCar(viewport, car, player != nullptr ? player->GetColor() : clrWhite, slots);
			}
			...
			void RaceMenu::UpdateStats(Garage::Car* car, Player* player, gui::ProgressBar* armorBar,
			                           gui::ProgressBar* speedBar, gui::ProgressBar* damageBar,
			                           gui::Label* armorBarValue, gui::Label* damageBarValue, gui::Label* speedBarValue)
			{
				...
				if (armorBarValue)
					armorBarValue->SetText(StrFmt("%0.0f/%0.0f", armor, maxArmor));
				if (damageBarValue)
					damageBarValue->SetText(StrFmt("%0.0f/%0.0f", damage, maxDamage));
				if (speedBarValue)
					speedBarValue->SetText(StrFmt("%0.0f/%0.0f", speedBar->GetProgress() * 300.0f, 300.0f));
			}
