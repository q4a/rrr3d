	...
	namespace game
	{
		...
		private:
			struct ShotSlots
			{
				...
				ShotSlots(): hyperdrive(0), mine(0), wpn1(0), wpn2(0), wpn3(0), wpn4(0), reserved(0)
				{
				}
				...
				{
					return Get(static_cast<unsigned>(slot - Player::stHyper));
				}

				void Set(Player::SlotType slot, bool val)
				{
					Set(static_cast<unsigned>(slot - Player::stHyper), val);
				}

				Player::SlotType AsWpn(unsigned i) const
				{
					return static_cast<Player::SlotType>(Player::stHyper + i);
				}
			...
			using CoordList = std::vector<D3DXVECTOR3>;
		public:
			struct DescData
			{
				BYTE playerId;
		...
		protected:
			...
			void OnSerialize(const net::NetMessage& msg, net::BitStream& stream) override;
		public:
			...
			void TakeBonus(MapObj* bonus, GameObject::BonusType type, float value, bool destroy);
