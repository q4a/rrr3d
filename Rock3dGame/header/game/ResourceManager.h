...
namespace r3d
{
	namespace game
	{
		...
		template <class _Resource, class _ComplexLib>
		class ComplexResCollection : public ComCollection<_Resource, void, void, void>
		{
		private:
			using _MyBase = ComCollection<_Resource, void, void, void>;
		private:
		...
		public:
			...
			Item* Find(const std::string& name) override
			{
			...
			bool ValidateName(const std::string& name) override
			{
		...
		class ComplexMeshLib : public Collection<ComplexMesh, void, ComplexMeshLib*, ComplexMeshLib*>, public Component
		{
			friend ComplexMesh;
		private:
			using _MyBase = Collection<ComplexMesh, void, ComplexMeshLib*, ComplexMeshLib*>;

			using _MeshLib = ComplexResCollection<res::MeshData, ComplexMeshLib>;
			using _IVBMeshLib = ComplexResCollection<graph::IndexedVBMesh, ComplexMeshLib>;
			using _MeshXLib = ComplexResCollection<graph::MeshX, ComplexMeshLib>;
			using _PxMeshLib = ComplexResCollection<px::TriangleMesh, ComplexMeshLib>;
			using MeshList = List<ComplexMesh*>;
		public:
		...
		public:
			...
			~ComplexMeshLib() override;

			CollectionItem* ComplexFind(CollectionTraits* traits, const std::string& name);
			ComplexMesh* Find(const std::string& name);
			ComplexMesh& Get(const std::string& name);
			const MeshList& GetMeshList(int tag);

			ComplexMesh& LoadMesh(const std::string& name, bool buildTBN = false, bool loadData = false,
			                      bool initIVB = false, int tag = -1);
		...
		class ComplexImageLib : public Collection<ComplexImage, void, ComplexImageLib*, ComplexImageLib*>,
		                        public Component
		{
			friend ComplexImage;
		private:
			using _MyBase = Collection<ComplexImage, void, ComplexImageLib*, ComplexImageLib*>;

			using _ImageLib = ComplexResCollection<res::ImageResource, ComplexImageLib>;
			using _Tex2dLib = ComplexResCollection<graph::Tex2DResource, ComplexImageLib>;
			using _CubeImageLib = ComplexResCollection<res::CubeImageResource, ComplexImageLib>;
			using _CubeTexLib = ComplexResCollection<graph::TexCubeResource, ComplexImageLib>;
			using ImageList = List<ComplexImage*>;
		public:
		...
		public:
			...
			~ComplexImageLib() override;

			CollectionItem* ComplexFind(CollectionTraits* traits, const std::string& name);
			ComplexImage* Find(const std::string& name);
			ComplexImage& Get(const std::string& name);

			ComplexImage& LoadImage(const std::string& name, unsigned levelCnt, bool initTex2d = false,
			                        bool initCubeTex = false, int tag = -1, bool gui = false);
		...
		public:
			...
			graph::LibMaterial& LoadLibMat(const std::string& name, bool sprite = false,
			                               graph::Material::Blending blending = graph::Material::bmOpaque,
			                               const FloatRange& alpha = 1.0f, const ColorRange& color = clrWhite,
			                               const ColorRange& ambient = clrWhite);
			void SetAlphaTestTo(graph::LibMaterial& libMat, graph::Material::AlphaTest alphaTest,
			                    const FloatRange& alphaRef);
			graph::Sampler2d& AddSampler2dTo(graph::LibMaterial& libMat, const std::string& imgName,
			                                 graph::Sampler2d::Mode alphaMode = graph::Sampler2d::tmDefault,
			                                 graph::BaseSampler::Filtering filtering = graph::BaseSampler::sfLinear);
		};

		class ComplexTextFontLibrary : public ComCollection<graph::TextFont, void, void, void>
		{
			using _MyBase = ComCollection<graph::TextFont, void, void, void>;
		private:
			ResourceManager* _resManager;
		public:
			...
			graph::TextFont& LoadFont(const std::string& name, int height, unsigned weight, bool italic, DWORD charset,
		...
		using StringValList = std::vector<StringValue>;
		...
		using Languages = Vector<Language>;
		...
		using CommentatorStyles = Vector<CommentatorStyle>;

		class StringLibrary : public Component
		{
		public:
			static const wchar_t cInfinite = static_cast<wchar_t>(0x221E);
		...
		public:
			...
			void Set(const std::string& id, const string& value);
			bool Has(const std::string& id) const;

			const std::string& Get(StringValue value) const;
			void Set(StringValue id, const string& value);
		...
		public:
			...
			using MeshLib = ComplexMeshLib;
			using ImageLib = ComplexImageLib;
			using ShaderLib = ResourceCollection<graph::Shader, ShaderType, void, void>;
			using MatLib = ComplexMatLib;
			using TextFontLib = ComplexTextFontLibrary;
			using StringLib = StringLibrary;
			using SoundLib = snd::SoundLib;
		...
		private:
			...
			ComplexMesh& LoadMesh(const std::string& name, bool buildTBN = false, bool loadData = false,
			                      bool initIVB = false, int tag = -1);
			ComplexImage& LoadImage(const std::string& name, unsigned levelCnt = 1, bool initTex2d = false,
			                        bool initCubeTex = false, int tag = -1, bool gui = false);
			...
			graph::LibMaterial& LoadImage2dLibMat(const std::string& name, const std::string& imgName,
			                                      ...
			                                      graph::Material::Blending blending = graph::Material::bmOpaque,
			                                      const FloatRange& alpha = 1.0f, const ColorRange& color = clrWhite,
			                                      const Vec2Range& texCoord = Vec2Range(NullVec2, IdentityVec2),
			                                      const Point2U& tileCnt = Point2U(1, 1));
			graph::LibMaterial& LoadImage2dLibMatAnim(const std::string& name, const std::string& imgName,
			                                          bool sprite = false,
			                                          graph::Material::Blending blending = graph::Material::bmOpaque,
			                                          const FloatRange& alpha = 1.0f,
			                                          const ColorRange& color = clrWhite,
			                                          const Vec3Range& texOffset = NullVector,
			                                          const Vec3Range& texScale = IdentityVector,
			                                          const QuatRange& texRot = NullQuaternion);
			graph::LibMaterial& LoadAlphaTestLibMat(const std::string& name, const std::string& imgName,
			                                        bool sprite = false, float alphaRef = cAlphaTestRef);
			graph::LibMaterial& LoadSpecLibMat(const std::string& name, const std::string& imgName,
			                                   bool sprite = false);
			graph::LibMaterial& LoadBumpLibMat(const std::string& name, const std::string& imgName,
			                                   const std::string& normMap, bool sprite = false,
			                                   graph::Material::Blending blending = graph::Material::bmOpaque,
			                                   float alpha = 1.0f);
			graph::LibMaterial& LoadAnimImage2dLibMat(const std::string& name, const std::string& imgName,
			                                          const Vec3Range& offset = NullVector,
			                                          const Vec3Range& scale = IdentityVector,
			                                          const QuatRange& rot = NullQuaternion, bool sprite = false,
			                                          graph::Material::Blending blending = graph::Material::bmOpaque,
			                                          const FloatRange& alpha = 1.0f,
			                                          const ColorRange& color = clrWhite);

			graph::TextFont& LoadFont(const std::string& name, int height, unsigned weight, bool italic,
			                          const std::string& faceName);

			snd::Sound* LoadSound(const std::string& name, const std::string& pathRoot, const std::string& path,
			                      float volume = 1.0f, float distScaller = 0.0f, bool load = false,
			                      bool loadCheckExists = false);
			snd::Sound* LoadSound(const std::string& name, float volume = 1.0f, float distScaller = 0.0f,
			                      bool load = false, bool loadCheckExists = false);
		...
		public:
			...
			void LoadCarLibMat(graph::LibMaterial* libMat, graph::Tex2DResource* tex, const D3DXCOLOR& color,
			                   const std::string& normMap = "");
			void LoadCarLibMat(graph::LibMaterial* libMat, const std::string& imgName, const std::string& normMap = "");
			graph::LibMaterial& LoadCarLibMat(const std::string& name, const std::string& imgName,
			                                  const std::string& normMap = "");
